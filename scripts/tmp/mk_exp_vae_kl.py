"""
mk_exp_vae_kl.py — Generate VAE β-sweep experiment files for MLP-shallow encoder.

Creates one experiment file per β value. Each trains VAE on MNIST for 200 epochs
and evaluates linear probe on both MNIST and Fashion-MNIST. Results saved to
projects/ssl/results_vae_kl.jsonl.

Usage:
    uv run python scripts/tmp/mk_exp_vae_kl.py
"""
...
TEMPLATE = '''\
"""
{exp_name} — MLP-shallow VAE with BETA={beta} KL weight.
...
for beta in BETAS:
    # Make a clean name: beta0, beta1e-4, beta1e-3, beta1e-2, beta1e-1, beta1
    if beta == 0.0:
        suffix = "b0"
    elif beta == 0.0001:
        suffix = "b1e4"
        ...
        suffix = str(beta).replace(".", "p")

    exp_name = f"exp_vae_kl_{suffix}"
    out = SSL / f"{exp_name}.py"
    out.write_text(TEMPLATE.format(exp_name=exp_name, beta=repr(beta)))
    print(f"Wrote {out}")

print("Done — 6 VAE KL sweep experiments generated.")
