"""
Generate a self-contained HTML report for exp34 (CIFAR-10, LATENT=2048, N_VAR=256, B=32).

Usage:
    uv run python scripts/tmp/gen_html_report_exp34.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (CIFAR-10, LATENT={latent_dim}, N_VAR={n_var})", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Image Variations — {EXP} (CIFAR-10, N_VAR={n_var})</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; CNN encoder + wide deconvnet, LATENT={latent_dim}, N_VAR={n_var}, B={batch_size}, 200 epochs &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">Coverage ratio</div><div class="value">{n_var/k_nn:.0f}×</div></div>
    <div class="metric"><div class="label">Training time</div><div class="value">{int(time_s//60)} m {int(time_s%60)} s</div></div>
  </div>
  <p>
    exp34 doubles N_VAR from 128 to {n_var} (halving batch size to {batch_size} to keep B×N_VAR=8,192 constant).
    This matches training coverage to eval coverage (both {n_var/k_nn:.0f}× over K_NN={k_nn} neighbors),
    testing whether the recall bottleneck is a coverage mismatch.
  </p>
</div>
...
<table>
  ...
  <tr><td>Variation candidates / step</td><td><strong>{n_var}</strong> random style codes (was 128 in exp33)</td></tr>
  <tr><td>Coverage ratio</td><td><strong>{n_var/k_nn:.0f}×</strong> (was 8× in exp33, matching eval)</td></tr>
  <tr><td>ALPHA (variation loss weight)</td><td>{alpha}</td></tr>
  <tr><td>Batch size</td><td><strong>{batch_size}</strong> (was 64 in exp33, keep B×N_VAR=8,192)</td></tr>
  <tr><td>Decoder channels C1/C2/C3</td><td>{dc_c1} / {dc_c2} / {dc_c3}</td></tr>
  <tr><td>Decoder stem width</td><td>{dc_stem}</td></tr>
  <tr><td>Epochs</td><td>200, cosine LR 3×10⁻⁴ → 1.5×10⁻⁵</td></tr>
...
<footer>
  ...
  CIFAR-10 &nbsp;·&nbsp; LATENT={latent_dim} N_VAR={n_var} wide-deconvnet &nbsp;·&nbsp; generated 2026-04-19
</footer>
</body>
</html>
"""
