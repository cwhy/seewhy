"""
Generate a self-contained HTML report for exp49
(Local IMLE, VOCAB=32, N_VAR=128, 300ep).
Usage: uv run python scripts/tmp/gen_html_report_exp49.py
"""
...
e_dim        = result.get("e_dim", result.get("cond_dim", 64) // n_cat)
...
class_sections = "".join(f"""
    <div class="class-block">
      ...
      <p class="caption">
        ...
        <strong>Bottom:</strong> 16&times;16 mosaic &mdash; cat1&times;cat2, quadrants = cat3&times;cat4 {OUTER_VALS_TEXT}.
      </p>
...
<head>
  <meta charset="UTF-8">
  <title>{EXP} &mdash; Local IMLE VOCAB=32 N_VAR=128 300ep CIFAR-10</title>
  <style>
    ...
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    .best{{border-left:4px solid var(--green);background:#f0fdf4;border-radius:0 8px 8px 0;padding:0.85rem 1.2rem;margin:1rem 0 1.2rem;}}
...
<body>
<h1>Local IMLE &mdash; VOCAB=32, N_VAR=128, 300-epoch schedule</h1>
<p class="subtitle">
  {EXP} &nbsp;&middot;&nbsp; CIFAR-10 &nbsp;&middot;&nbsp; CNN encoder + wide deconvnet &nbsp;&middot;&nbsp;
  {n_var} samples | {k_nn} images, {n_cat} var &times; {vocab_size} vocabs &times; {e_dim} dims + {latent_dim} latent &nbsp;&middot;&nbsp;
  {int(time_s//60)} min on RTX 4090 &nbsp;&middot;&nbsp; {n_params_val:,} parameters
</p>
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">exp45 recall (VOCAB=8, 150ep)</div><div class="value">{fmt(recall_f("exp45"))}</div></div>
    <div class="metric"><div class="label">exp35 recall (warm-start)</div><div class="value">{fmt(recall_f("exp35"))}</div></div>
    <div class="metric"><div class="label">Training time</div><div class="value">{int(time_s//60)} min</div></div>
  </div>
  <p>
    exp49 = exp45 + <strong>VOCAB_SIZE 8&rarr;32</strong> (32<sup>4</sup> &asymp; 1M conditioning codes vs 8<sup>4</sup> = 4096)
    over 300 epochs. The larger vocabulary gives the decoder far more distinct conditioning inputs to learn from.
    Early training is slower (sparse embedding gradients), but the model converges to better final recall given
    enough epochs. Near-zero compute overhead: only the embedding table grows (4&times;32&times;16 = 2048 params).
  </p>
  <div class="best">
    <strong>Result:</strong> recall = {final_recall:.5f} &mdash; <strong>new overall best</strong>, beating exp35
    ({fmt(recall_f("exp35"))}, warm-start from 200 pre-trained epochs) by {(float(fmt(recall_f("exp35"))) - final_recall)*100/float(fmt(recall_f("exp35"))):.1f}%.
    VOCAB=32 + 300ep outperforms VOCAB=8 + 400ep warm-start equivalent.
  </div>
</div>
...
<p class="caption">
  ...
  <strong>Centre:</strong> eval recon MSE and recall MSE &mdash; slow early progress, then sustained improvement through ep299.
  <strong>Right:</strong> per-rank recall MSE across training.
</p>
...
<h2>Variation grids &mdash; per class</h2>
{class_sections}

<h2>Comparison &mdash; key experiments</h2>
<table>
  <tr><th>Exp</th><th>VOCAB</th><th>COND_DIM</th><th>Epochs</th><th>Recall MSE</th><th>Recon MSE</th><th>Time</th></tr>
  <tr><td>exp41</td><td>8</td><td>16</td><td>150ep</td><td>{fmt(recall_f("exp41"))}</td><td>{fmt(recon_f("exp41"))}</td><td>65 min</td></tr>
  <tr><td>exp45</td><td>8</td><td>64</td><td>150ep</td><td>{fmt(recall_f("exp45"))}</td><td>{fmt(recon_f("exp45"))}</td><td>107 min</td></tr>
  <tr><td>exp35</td><td>8</td><td>16</td><td>200+200ep warm-start</td><td>{fmt(recall_f("exp35"))}</td><td>{fmt(recon_f("exp35"))}</td><td>8.2h</td></tr>
  <tr style="background:#f0fdf4"><td><strong>exp49</strong></td><td><strong>32</strong></td><td><strong>64</strong></td><td><strong>300ep full run</strong></td><td><strong>{final_recall:.5f}</strong></td><td><strong>{final_recon:.5f}</strong></td><td><strong>{int(time_s//60)} min</strong></td></tr>
</table>

<div class="callout">
  <strong>Key finding:</strong> VOCAB=32 is slow to start (ep5 recall 0.00949 vs exp45&rsquo;s 0.00929)
  due to sparser embedding gradients, but converges to better final recall with enough epochs.
  The larger code space gives the decoder more expressive conditioning, enabling finer-grained
  neighbourhood coverage. Next: try VOCAB=64 or 128 with proportionally more epochs.
</div>

<h2>Configuration</h2>
<table>
  <tr><th>Setting</th><th>Value</th></tr>
  <tr><td>kNN search</td><td>On-the-fly B&times;50k GEMM + top_k(K+1), stop_gradient on kNN targets</td></tr>
  <tr><td>IMLE matching</td><td>Inverse: K=16 neighbours &rarr; argmin over N_VAR={n_var} variations</td></tr>
  <tr><td>Conditioning</td><td>{n_cat} categorical codes &times; VOCAB={vocab_size}, E_DIM={e_dim}, COND_DIM={n_cat*e_dim}</td></tr>
  <tr><td>Loss function</td><td>L_imle(local-inverse) + {alpha}&middot;L_recon</td></tr>
  <tr><td>LR schedule</td><td>Cosine 3&times;10<sup>&minus;4</sup>&rarr;1.5&times;10<sup>&minus;5</sup> over {epochs_run} epochs (no early stopping)</td></tr>
  <tr><td>Total parameters</td><td>{n_params_val:,}</td></tr>
  <tr><td>Encoder</td><td>CNN: {enc_c1}/{enc_c2}/{enc_c3} ch, stride-2 conv, FC {enc_fc1}&rarr;{latent_dim}</td></tr>
  <tr><td>Decoder</td><td>Wide deconvnet: MLP stem {latent_dim+n_cat*e_dim}&rarr;{dc_stem}&rarr;{dc_c1}&times;4&times;4, ConvT {dc_c1}/{dc_c2}/{dc_c3} ch</td></tr>
</table>
<footer>
  Conditioned variation model &nbsp;&middot;&nbsp; {EXP} &nbsp;&middot;&nbsp; CIFAR-10 &nbsp;&middot;&nbsp;
  Local IMLE on-the-fly kNN, VOCAB=32, N_VAR=128 &nbsp;&middot;&nbsp; 2026-04-26
</footer>
</body></html>"""
