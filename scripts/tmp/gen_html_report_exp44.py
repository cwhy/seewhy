"""
Generate a self-contained HTML report for exp44
(Local IMLE, N_VAR=96, 150ep schedule, stop_gradient on kNN).
Usage: uv run python scripts/tmp/gen_html_report_exp44.py
"""
...
<body>
<h1>Local IMLE On-the-fly kNN — COND_DIM=64, 150-epoch schedule</h1>
<p class="subtitle">
...
<div class="summary-card">
  ...
  <p>
    exp44 = exp43 with one change: <strong>E_DIM 4→16</strong> (COND_DIM 16→64). The conditioning space
    given to the decoder is 4× wider, giving variations more room to differ from each other. All other
    settings identical: N_VAR=96, 150ep cosine schedule, stop_gradient on kNN, MIN_DELTA=2×10⁻⁴.
  </p>
  <div class="callout">
    <strong>Result:</strong> recall = {final_recall:.5f} — 3.2% better than exp43 ({fmt(recall_f("exp43"))}).
    COND_DIM=64 helps: wider conditioning lets the decoder steer outputs further across the local neighbourhood.
    Remaining gap to exp35 ({fmt(recall_f("exp35"))}) is ~7%.
  </div>
</div>
...
<table>
  ...
  <tr><td>exp43</td><td>96</td><td>150ep, early stop</td><td>Yes</td><td>{fmt(recall_f("exp43"))}</td><td>{fmt(recon_f("exp43"))}</td><td>118 min</td></tr>
  <tr style="background:#eff6ff"><td><strong>exp44</strong></td><td><strong>96</strong></td><td><strong>150ep, early stop @ep{epochs_run - 1}</strong></td><td><strong>Yes</strong></td><td><strong>{final_recall:.5f}</strong></td><td><strong>{final_recon:.5f}</strong></td><td><strong>{int(time_s//60)} min</strong></td></tr>
</table>

<div class="callout">
  <strong>Finding:</strong> COND_DIM=64 improves recall by 3.2% over COND_DIM=16 — the wider conditioning
  space gives the decoder more leverage to steer outputs across the local neighbourhood. Next: N_VAR=128
  via two-phase chunked search (exp45), which unlocks larger variation search without OOM.
</div>
