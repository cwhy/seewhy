"""
Generate a self-contained HTML report for exp45
(Local IMLE, N_VAR=96, 150ep schedule, stop_gradient on kNN).
Usage: uv run python scripts/tmp/gen_html_report_exp45.py
"""
...
<body>
<h1>Local IMLE On-the-fly kNN — N_VAR=128, COND_DIM=64, 150-epoch schedule</h1>
<p class="subtitle">
...
<div class="summary-card">
  ...
  <p>
    exp45 = exp44 + two changes: <strong>N_VAR 96→128</strong> (33% more candidates) via a two-phase chunked
    IMLE search that avoids OOM. Phase 1 decodes N_VAR in chunks of 64 under stop_gradient (no activation
    storage); Phase 2 re-decodes only the K=16 winners with full gradient (1,024 items vs 8,192). Same speed
    as exp44 (~44s/epoch) — the extra candidates cost nothing in backward time.
  </p>
  <div class="callout">
    <strong>Result:</strong> recall = {final_recall:.5f} — new best, 1.7% better than exp44 ({fmt(recall_f("exp44"))}).
    Gap to exp35 ({fmt(recall_f("exp35"))}, precomputed kNN): <strong>5.6%</strong>.
    Both COND_DIM=64 and N_VAR=128 contribute: each step we scale up closes the gap further.
  </div>
</div>
...
<p class="caption">
  ...
  <strong>Centre:</strong> eval recon MSE and recall MSE — recall converges, early stop at epoch {epochs_run - 1}.
  <strong>Right:</strong> per-rank recall MSE — clear improvement, plateau visible before stop.
</p>
...
<table>
  <tr><th>Exp</th><th>N_VAR</th><th>COND_DIM</th><th>Schedule</th><th>Recall MSE</th><th>Recon MSE</th><th>Time</th></tr>
  <tr><td>exp41</td><td>64</td><td>16</td><td>150ep early stop</td><td>{fmt(recall_f("exp41"))}</td><td>{fmt(recon_f("exp41"))}</td><td>65 min</td></tr>
  <tr><td>exp43</td><td>96</td><td>16</td><td>150ep early stop</td><td>{fmt(recall_f("exp43"))}</td><td>{fmt(recon_f("exp43"))}</td><td>118 min</td></tr>
  <tr><td>exp44</td><td>96</td><td>64</td><td>150ep early stop</td><td>{fmt(recall_f("exp44"))}</td><td>{fmt(recon_f("exp44"))}</td><td>99 min</td></tr>
  <tr style="background:#eff6ff"><td><strong>exp45</strong></td><td><strong>128</strong></td><td><strong>64</strong></td><td><strong>150ep, early stop @ep{epochs_run - 1}</strong></td><td><strong>{final_recall:.5f}</strong></td><td><strong>{final_recon:.5f}</strong></td><td><strong>{int(time_s//60)} min</strong></td></tr>
  <tr><td>exp35</td><td>128</td><td>16</td><td>200ep full run (precomputed kNN)</td><td>{fmt(recall_f("exp35"))}</td><td>{fmt(recon_f("exp35"))}</td><td>8.2h</td></tr>
</table>

<div class="callout">
  <strong>Finding:</strong> each scaling axis improves recall independently — COND_DIM 16→64 saved 3.2%,
  N_VAR 96→128 saved another 1.7%. Gap to exp35 now 5.6%. Next levers: K_NN, N_CAT/VOCAB_SIZE, decoder capacity.
</div>
...
<table>
  ...
  <tr><td>LR schedule</td><td>Cosine 3×10⁻⁴→1.5×10⁻⁵ over 150 epochs (ran {epochs_run} epochs)</td></tr>
  <tr><td>Early stopping</td><td>patience=8 evals, min_delta=2×10⁻⁴ (triggered at epoch {epochs_run - 1})</td></tr>
