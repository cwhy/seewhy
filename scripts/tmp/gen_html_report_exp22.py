"""
Generate a self-contained HTML report for exp22 (CIFAR-10, N_VAR=64 K_NN=16 + Gaussian style).

Usage:
    uv run python scripts/tmp/gen_html_report_exp22.py
"""
...
style_dim    = result.get("style_dim", 16)
style_std    = result.get("style_std", 1.0)
dc_c1        = result["dc_c1"]
dc_c2        = result["dc_c2"]
dc_c3        = result["dc_c3"]
...
ax.set_xlabel("epoch"); ax.set_ylabel("MSE")
ax.set_title("Eval metrics", fontweight="bold")
ax.legend(fontsize=9); ax.grid(alpha=0.25)
...
ax.set_xlabel("neighbor rank"); ax.set_ylabel("MSE")
ax.set_title("Recall by rank (blue→red)", fontweight="bold")
ax.legend(fontsize=8); ax.grid(alpha=0.25)

fig.suptitle(f"Learning dynamics — {EXP} (N_VAR={n_var} K_NN={k_nn} Gaussian)", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  <meta charset="UTF-8">
  <title>Variations — {EXP} (N_VAR={n_var} K_NN={k_nn} Gaussian style)</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; N_VAR={n_var} K_NN={k_nn} (coverage {n_var/k_nn:.1f}×) + Gaussian style &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">Coverage</div><div class="value">{n_var/k_nn:.1f}×</div></div>
    <div class="metric"><div class="label">Style</div><div class="value">Gaussian {style_dim}-d</div></div>
    <div class="metric"><div class="label">Training time</div><div class="value">{int(time_s//60)} m {int(time_s%60)} s</div></div>
  </div>
  <p>
    exp22 combines best IMLE config (N_VAR={n_var}, K_NN={k_nn}, coverage {n_var/k_nn:.1f}×) with
    continuous Gaussian style from exp21 (STYLE_DIM={style_dim}). Both experiments share the same
    upsample+conv decoder (bilinear, no checkerboard).
  </p>
</div>
...
<table>
  ...
  <tr><td>Style</td><td>Gaussian {style_dim}-d, std={style_std}</td></tr>
  <tr><td>N_VAR / K_NN</td><td>{n_var} / {k_nn} (coverage {n_var/k_nn:.1f}×)</td></tr>
  <tr><td>Decoder</td><td>upsample+conv: C1={dc_c1}, C2={dc_c2}, C3={dc_c3}, stem={dc_stem}</td></tr>
  <tr><td>Total parameters</td><td>{n_params_val:,}</td></tr>
</table>

<h2>Variation grids</h2>

{class_sections}

<footer>{EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; generated 2026-04-16</footer>
</body>
</html>
"""
