"""
Generate a self-contained HTML report for exp43
(Local IMLE, N_VAR=96, 150ep schedule, stop_gradient on kNN).
Usage: uv run python scripts/tmp/gen_html_report_exp43.py
"""
...
<body>
<h1>Local IMLE On-the-fly kNN — N_VAR=96, 150-epoch schedule</h1>
<p class="subtitle">
...
<div class="summary-card">
  ...
  <p>
    exp43 = exp42 with one change: <strong>MAX_EPOCHS 100→150</strong> to fix the underfitting seen in exp42
    (model still improving at epoch 99). All other settings identical: N_VAR=96, stop_gradient on kNN targets,
    MIN_DELTA=2×10⁻⁴. Early stopping triggered at epoch {epochs_run - 1}, confirming the model fully converged.
  </p>
  <div class="callout">
    <strong>Result:</strong> recall = {final_recall:.5f} — converged 6% better than exp42 ({fmt(recall_f("exp42"))})
    and matches exp41 ({fmt(recall_f("exp41"))}) using 50% more variation samples (N_VAR 64→96).
    N_VAR=96 does not improve over N_VAR=64 — the bottleneck is elsewhere (LR schedule length, not variation count).
  </div>
</div>
...
<table>
  ...
  <tr><td>exp35</td><td>64</td><td>200ep (precomputed kNN)</td><td>N/A</td><td>{fmt(recall_f("exp35"))}</td><td>{fmt(recon_f("exp35"))}</td><td>~8.2h</td></tr>
  ...
  <tr><td>exp42</td><td>96</td><td>100ep (ran to end)</td><td>Yes</td><td>{fmt(recall_f("exp42"))}</td><td>{fmt(recon_f("exp42"))}</td><td>~48 min</td></tr>
  <tr style="background:#eff6ff"><td><strong>exp43</strong></td><td><strong>96</strong></td><td><strong>150ep, early stop @ep{epochs_run - 1}</strong></td><td><strong>Yes</strong></td><td><strong>{final_recall:.5f}</strong></td><td><strong>{final_recon:.5f}</strong></td><td><strong>{int(time_s//60)} min</strong></td></tr>
</table>

<div class="callout">
  <strong>Finding:</strong> N_VAR=96 converges to the same recall as N_VAR=64 (exp41). The 150-epoch schedule
  was necessary for convergence (100ep was insufficient), but the 50% increase in variation samples provides
  no measurable benefit. The remaining gap to exp35 (precomputed kNN, 0.00666) is ~11% and is not closed
  by more variation samples alone. Next direction: investigate whether the gap is due to on-the-fly kNN
  noise vs precomputed, or encoder capacity.
</div>
