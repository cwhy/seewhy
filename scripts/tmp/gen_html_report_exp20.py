"""
Generate a self-contained HTML report for exp20 (CIFAR-10, N_VAR=64 K_NN=16).
All plots are embedded as base64 PNGs. Uploads to R2.

Usage:
    uv run python scripts/tmp/gen_html_report_exp20.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (CIFAR-10, N_VAR={n_var} K_NN={k_nn})", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Image Variations — {EXP} (CIFAR-10, N_VAR={n_var} K_NN={k_nn})</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; IMLE coverage fix: N_VAR={n_var}, K_NN={k_nn}, coverage={n_var/k_nn:.1f}× &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">Coverage ratio</div><div class="value">{n_var/k_nn:.1f}×</div></div>
    <div class="metric"><div class="label">Training time</div><div class="value">{int(time_s//60)} m {int(time_s%60)} s</div></div>
  </div>
  <p>
    exp20 fixes IMLE coverage: N_VAR={n_var} candidates / K_NN={k_nn} neighbors = {n_var/k_nn:.1f}× ratio
    (was 0.25× in exp17). Architecture identical to exp17 (wide CNN encoder + deconvnet decoder,
    LATENT={latent_dim}, DC_STEM={dc_stem}).
  </p>
</div>
...
<table>
  ...
  <tr><td>Variation candidates / step</td><td>{n_var} random style codes (coverage {n_var/k_nn:.1f}×)</td></tr>
...
<footer>
  ...
  CIFAR-10 &nbsp;·&nbsp; N_VAR={n_var} K_NN={k_nn} coverage={n_var/k_nn:.1f}× &nbsp;·&nbsp; generated 2026-04-16
</footer>
</body>
</html>
"""
