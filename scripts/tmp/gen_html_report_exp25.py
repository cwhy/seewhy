"""
Generate a self-contained HTML report for exp25 (CIFAR-10, LATENT=512 + ALPHA=3.0).

Usage:
    uv run python scripts/tmp/gen_html_report_exp25.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (LATENT={latent_dim} ALPHA={alpha})", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  <meta charset="UTF-8">
  <title>Variations — {EXP} (LATENT={latent_dim} + ALPHA={alpha})</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; LATENT={latent_dim} + ALPHA={alpha} (stronger variation) &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">ALPHA</div><div class="value">{alpha}</div></div>
  ...
  <p>
    exp25 = exp23 (LATENT={latent_dim}) + ALPHA={alpha}. With 512-d latent providing sufficient
    reconstruction capacity, increasing the variation loss weight from 1.0 to {alpha} should
    force the decoder to cover more output modes. Trade-off: may hurt reconstruction MSE.
  </p>
</div>
...
<table>
  ...
  <tr><td>ALPHA (variation loss weight)</td><td><strong>{alpha}</strong> (was 1.0 in exp23)</td></tr>
  ...
  <tr><td>kNN type</td><td>pixel-space L2 (sliced from k64)</td></tr>
...
<footer>{EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; LATENT={latent_dim} ALPHA={alpha} &nbsp;·&nbsp; generated 2026-04-16</footer>
</body>
</html>
"""
