"""
Generate a self-contained HTML report for exp11.
All plots are embedded as base64 PNGs. Uploads to R2.

Usage:
    uv run python scripts/tmp/gen_html_report_exp11.py
"""
...
fig.suptitle("Learning dynamics — exp11", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Handwriting Variations — exp11</title>
  <style>
    :root {{
      --blue:   #2563eb;
      --green:  #16a34a;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric">
      <div class="label">Recall MSE (final)</div>
      <div class="value">0.0160</div>
    </div>
    <div class="metric">
  ...
  <p>
    The model accurately reconstructs unseen test images and learns a structured, controllable
    style space: turning four categorical "knobs" smoothly changes stroke thickness, slant, and
    pen pressure while preserving digit identity. Both reconstruction quality and the model's
    ability to cover real handwriting variety improve steadily over 100 epochs.
  </p>
...
<div class="diagram">
  image ──► <b>Encoder</b> ──► content vector (64-d) ──────────────────────┐
                                                                             ▼
...
<p>
  A useful mental model: imagine {n_var} painters each given a style specification and asked
  to paint the same digit. A teacher grades each painter by finding, for each of {k_nn} reference
  samples, the painter whose output is closest. The grade is the average distance between each
  reference and its closest match. The painters (style codes) improve by diversifying — a painter
  who copies another painter's output is always redundant and always loses marks.
</p>
...
<p>
  ...
  <code>{final_mse:.4f}</code>. Recall MSE drops from 0.0225 at epoch 4 to 0.0160 at epoch 99,
  showing the variation space progressively covers more of the real data distribution.
