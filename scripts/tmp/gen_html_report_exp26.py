"""
Generate a self-contained HTML report for exp26 (CIFAR-10, DeltaNet decoder).

Usage:
    uv run python scripts/tmp/gen_html_report_exp26.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (CIFAR-10, DeltaNet, LATENT={latent_dim})", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Image Variations — {EXP} (CIFAR-10, DeltaNet)</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; CNN encoder + DeltaNet autoregressive decoder &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">vs exp23 baseline</div><div class="value">AR decoder</div></div>
  ...
  <p>
    exp26 replaces the parallel deconvnet decoder with a DeltaNet autoregressive decoder.
    Images are processed as {t_seq} patch tokens (4×4 RGB = 48-d each). The delta-rule
    linear RNN maintains a matrix-valued state W ∈ ℝ<sup>{dm}×{dk}</sup> per layer,
    capturing inter-patch dependencies. Training uses teacher forcing; sampling is
    truly O(T) with constant O(d²) memory.
  </p>
</div>

<div class="callout">
  <strong>DeltaNet core update:</strong>
  W<sub>t</sub> = W<sub>t−1</sub> + β<sub>t</sub> · outer(v<sub>t</sub> − W<sub>t−1</sub>k<sub>t</sub>, k<sub>t</sub>)
  &nbsp;&nbsp;→&nbsp;&nbsp; x<sub>t</sub> += W<sub>t</sub>q<sub>t</sub>
</div>
...
<p class="caption">Original (top) vs autoregressive reconstruction (bottom) for held-out test images.</p>
...
<table>
  ...
  <tr><td>Decoder</td><td>DeltaNet: DM={dm}, DK={dk}, {n_layers} layers, T={t_seq} tokens (4×4 patches)</td></tr>
  ...
  <tr><td>Variation candidates / step</td><td>{n_var} random style codes (AR sampled, no gradient)</td></tr>
...
<footer>
  ...
  CIFAR-10 &nbsp;·&nbsp; DeltaNet autoregressive decoder &nbsp;·&nbsp; generated 2026-04-16
</footer>
</body>
</html>
"""
