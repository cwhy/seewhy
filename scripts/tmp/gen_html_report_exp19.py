"""
Generate a self-contained HTML report for exp19 (CIFAR-10, feature-space kNN).
All plots are embedded as base64 PNGs. Uploads to R2.

Usage:
    uv run python scripts/tmp/gen_html_report_exp19.py
"""
...
k_nn            = result["k_nn"]
knn_update_every = result["knn_update_every"]
...
# Mark kNN refresh epochs
for ep in range(knn_update_every, len(epochs), knn_update_every):
    ax.axvline(x=ep, color="#7c3aed", lw=0.8, alpha=0.4, ls=":")
ax.set_xlabel("epoch", fontsize=11); ax.set_ylabel("MSE", fontsize=11)
ax.set_title("Training losses (purple lines = kNN refresh)", fontsize=12, fontweight="bold")
...
fig.suptitle("Learning dynamics — exp19 (CIFAR-10, feature-space kNN)", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Image Variations — exp19 (CIFAR-10, feature-space kNN)</title>
  <style>
...
<p class="subtitle">
  exp19 &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; CNN encoder + wide decoder + feature-space kNN &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="label">vs exp17 (pixel kNN)</div><div class="value">Δ MSE</div></div>
  ...
  <p>
    exp19 uses the same wide decoder as exp17 but replaces static pixel-space kNN with
    dynamic encoder-embedding kNN. Every {knn_update_every} epochs, all 50k training images are
    encoded to {latent_dim}-d vectors and cosine nearest neighbours are recomputed via FAISS.
    Bootstrap uses the cached pixel-space kNN for the first {knn_update_every} epochs.
    Hypothesis: semantically consistent neighbours produce better variation targets.
  </p>
...
<h3>CNN Encoder (same as exp17)</h3>
<div class="diagram">
  x_flat (B, 3072)
...
<h3>Wide Decoder (same as exp17)</h3>
<div class="diagram">
...
<h3>Dynamic Feature-Space kNN (new in exp19)</h3>
<div class="diagram">
  Every {knn_update_every} epochs:
    E ← encode_all(params, X_train)            # (50000, {latent_dim})
    E_norm ← E / ‖E‖₂                          # L2-normalise
    index ← faiss.IndexFlatIP({latent_dim})    # cosine similarity
    index.add(E_norm)
    _, I ← index.search(E_norm, K+1)           # K={k_nn} neighbours
    knn_idx ← I[:, 1:]                          # exclude self
</div>
...
<p class="caption">
  <strong>Left:</strong> training losses (purple dotted lines mark kNN refresh epochs). &nbsp;
...
<table>
  ...
  <tr><td>Nearest neighbors</td><td>{k_nn} per training image — dynamic encoder cosine (FAISS), refreshed every {knn_update_every} epochs</td></tr>
  <tr><td>Variation candidates / step</td><td>{n_var} random style codes</td></tr>
  <tr><td>Batch size</td><td>{batch_size}</td></tr>
  <tr><td>Decoder channels C1/C2/C3</td><td>{dc_c1} / {dc_c2} / {dc_c3}  (wide, same as exp17)</td></tr>
...
<ul>
  <li><strong>Top strip:</strong> training image + 10 nearest pixel-space neighbors (shown for visual reference).</li>
...
<footer>
  Conditioned variation model &nbsp;·&nbsp; exp19 &nbsp;·&nbsp;
  CIFAR-10 &nbsp;·&nbsp; CNN encoder + wide decoder + feature-space kNN &nbsp;·&nbsp; generated 2026-04-15
</footer>
</body>
</html>
"""
