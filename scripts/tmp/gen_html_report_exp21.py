"""
Generate a self-contained HTML report for exp21 (CIFAR-10, continuous Gaussian style).

Usage:
    uv run python scripts/tmp/gen_html_report_exp21.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (CIFAR-10, Gaussian style)", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
...
<head>
  ...
  <title>Learning to Generate Image Variations — {EXP} (CIFAR-10, Gaussian style)</title>
  <style>
...
<p class="subtitle">
  {EXP} &nbsp;·&nbsp; CIFAR-10 &nbsp;·&nbsp; CNN encoder + upsample+conv decoder, continuous Gaussian style &nbsp;·&nbsp;
...
<div class="summary-card">
  ...
  <p>
    exp21 replaces categorical discrete style (4096 modes) with a continuous Gaussian:
    z_style ~ N(0, {style_std}²·I), STYLE_DIM={style_dim}. Reconstruction uses style=0.
    Decoder changed to upsample+conv (bilinear resize) to eliminate checkerboard artifacts.
  </p>
</div>
...
<p class="caption">Original (top) vs reconstruction (style=0, bottom) for held-out test images.</p>
...
<table>
  ...
  <tr><td>Style variables</td><td>Gaussian {style_dim}-d, std={style_std} (continuous)</td></tr>
  <tr><td>Nearest neighbors</td><td>{k_nn} per training image (pixel space, precomputed)</td></tr>
  <tr><td>Variation candidates / step</td><td>{n_var} random Gaussian samples</td></tr>
...
<footer>
  ...
  CIFAR-10 &nbsp;·&nbsp; Gaussian style &nbsp;·&nbsp; generated 2026-04-16
</footer>
</body>
</html>
"""
