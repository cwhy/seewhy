"""
Generate a self-contained HTML report for exp38 (Global IMLE, CIFAR-10, no recon loss).
Requires plots already generated by:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/scripts/gen_report_exp38.py

Usage:
    uv run python scripts/tmp/gen_html_report_exp38.py
"""
...
<head>
  <meta charset="UTF-8">
  <title>{EXP} — Global IMLE CIFAR-10</title>
  <style>
    :root{{--blue:#2563eb;--red:#dc2626;--gray:#6b7280;--light:#f8fafc;--border:#e2e8f0;}}
...
<h1>Global IMLE — No Reconstruction Loss</h1>
<p class="subtitle">
...
<div class="summary-card">
  ...
  <p>
    exp38 replaces local IMLE (precomputed kNN per image) with <strong>global IMLE</strong>:
    each generated variation finds its nearest neighbour in the <em>full 50k training set</em>
    via exact GPU GEMM (JAX, no FAISS). No reconstruction loss, no <code>recon_emb</code>,
    no kNN precomputation. Both NN search and gradient update use the same style codes
    (the correct IMLE formulation — split codes cause trivial loss collapse).
  </p>
  <div class="warn">
    <strong>Key finding:</strong> without a reconstruction anchor the encoder has no incentive
    to learn input-specific structure. Both recall MSE and recon MSE freeze after ~4 epochs.
    Global recall (0.046) is 7&times; worse than exp35 local IMLE (0.007).
  </div>
</div>

<h2>Learning curves</h2>
<p class="caption">
  <strong>Left:</strong> IMLE training loss on log scale — collapses to ~10⁻⁸ by epoch 1, then plateaus.
  <strong>Centre:</strong> recon MSE (blue) and global recall MSE (red) — both flat from epoch 4.
  <strong>Right:</strong> recall by neighbour rank (blue = early → red = final) — negligible change.
</p>
...
<p class="caption">
  ...
  Recon MSE = {final_recon:.4f} — poor, as expected with no reconstruction loss.
</p>
...
<h2>Comparison vs exp33 / exp35</h2>
<table>
  <tr><th>Metric</th><th>exp33 (local IMLE, K=16)</th><th>exp35 (warm-start)</th><th>exp38 (global IMLE)</th></tr>
  <tr><td>Recall MSE (final)</td><td>{fmt(exp33_recall)}</td><td>{fmt(exp35_recall)}</td><td><strong>{final_recall:.5f}</strong></td></tr>
  <tr><td>Recon MSE (final)</td><td>{fmt(recon_final("exp33"))}</td><td>{fmt(recon_final("exp35"))}</td><td>{final_recon:.5f}</td></tr>
  <tr><td>kNN required</td><td>Yes (precomputed)</td><td>Yes (precomputed)</td><td><strong>No</strong></td></tr>
  <tr><td>Reconstruction loss</td><td>Yes (α=1.0)</td><td>Yes (α=1.0)</td><td><strong>No</strong></td></tr>
  <tr><td>NN search</td><td>Precomputed kNN array</td><td>Precomputed kNN array</td><td>Exact GPU GEMM</td></tr>
  <tr><td>Epoch time</td><td>~148s</td><td>~148s</td><td>~55s</td></tr>
  <tr><td>Total time</td><td>~8.2h</td><td>~8.2h</td><td>~3.1h</td></tr>
</table>

<div class="callout">
  <strong>Next step — exp38:</strong> global IMLE + reconstruction loss (α·L_recon + L_imle_global).
  Keeps global matching (no kNN precomputation) while anchoring the encoder via reconstruction,
  restoring local neighbourhood coverage.
</div>
...
<table>
  ...
  <tr><td>IMLE matching</td><td>Exact GPU GEMM ({batch_size*n_var}×50k×3072, ~8ms on RTX 4090)</td></tr>
  <tr><td>N_VAR / batch</td><td>{n_var} random codes → {batch_size * n_var} distance queries/step</td></tr>
  <tr><td>Loss function</td><td>IMLE only: MSE(Y, NN(Y)) — no reconstruction term</td></tr>
...
<footer>
  ...
  Global IMLE, exact GPU NN &nbsp;·&nbsp; 2026-04-22
</footer>
</body>
</html>"""
