"""Recall-Gen — exp16: shrink the STATE, hold the context fixed.
...
Here the state is **4,096 numbers against 16 x 784 = 12 544 numbers of
...
Usage:
    uv run python projects/recall-gen/scripts/run_experiments.py --bg exp16
"""
...
RN = Run(
    exp_name="smoke_exp16" if SMOKE else "exp16",
    name="recall training, M=16, state 4096 floats (dk=16, heads=16)",
    M=16, Q=4, mask_rows=14,
    batch=256, steps=200 if SMOKE else 12000, lr=3e-4, seed=0,
    train_mode="recall",
    cfg=Cfg(d_model=256, n_layers=4, dk=16, n_heads=16, n_tokens=20),
