"""Recall-Gen — exp15: shrink the STATE, hold the context fixed.
...
Here the state is **8,192 numbers against 16 x 784 = 12 544 numbers of
...
Usage:
    uv run python projects/recall-gen/scripts/run_experiments.py --bg exp15
"""
...
RN = Run(
    exp_name="smoke_exp15" if SMOKE else "exp15",
    name="recall training, M=16, state 8192 floats (dk=32, heads=8)",
    M=16, Q=4, mask_rows=14,
    batch=256, steps=200 if SMOKE else 12000, lr=3e-4, seed=0,
    train_mode="recall",
    cfg=Cfg(d_model=256, n_layers=4, dk=32, n_heads=8, n_tokens=20),
