"""Recall-Gen — exp38: train on Fashion-MNIST, test on MNIST, KDA layers frozen at init
...
carries predict-black and the ridge map alongside the average image.\n\nThe frozen arm: the four KDA layers keep their random initialisation forever\nand only the embedding and output head train.

Usage:
    uv run --no-sync python projects/recall-gen/scripts/run_experiments.py --bg exp38
"""
...
RN = Run(
    exp_name="smoke_exp38" if SMOKE else "exp38",
    name="cross-dataset: Fashion-MNIST to MNIST, recall training, KDA layers frozen at init",
