# Recall and completion on a synthetic prior, tested on Fashion-MNIST

Three networks were trained on a synthetic prior and never shown a real Fashion-MNIST
image. They are identical in size and shape and differ in one thing: what their
...
Then all three were asked to do both jobs on real Fashion-MNIST: find an item that is
present, and predict one that is absent.

| | finding, on Fashion-MNIST | predicting, on Fashion-MNIST |
|---|---|---|
| recall-trained | **0.269** | 1.646 |
| completion-trained | 0.093 | **1.952** |
| frozen layers | 0.081 | 1.519 |
...
Fashion-MNIST item the erased part is the bottom fourteen of twenty-eight rows.
...
worlds seen during training, items from worlds never seen, and real Fashion-MNIST — which
...
is what makes "infer this world" a question at all. The real Fashion-MNIST band is scored
...
Which reference is stated on each figure. For Fashion-MNIST the reference used here is the
average real Fashion-MNIST item, whose raw error is 0.0915.
...
![The three arms on Fashion-MNIST](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r18_bars_v5.svg)

The left panel puts real Fashion-MNIST beside the control that decides how to read
everything else: fresh worlds drawn from the prior the networks were actually
trained on.

The recall-trained network scores 0.631 there, against
...
d_model=256 scored 0.167 on that control. The section below is
about that, and it is the reason the arms here are the wider ones.

On real Fashion-MNIST the recall arm scores 0.269. The
completion-trained network scores 0.093 and the frozen
one 0.081 — both at chance. Across three independent
...
looks at the context and scores 1.371. The soft look-up uses
nothing but the context and scores 0.776. The best of the three
networks is **frozen layers** at 1.519.
...
context.** On Fashion-MNIST it scores 1.911 when the answer is
present and 1.952 when it is absent — the same number, to three
...
is 0.093 against chance of 0.063. That signature has
...
![What the three networks produce](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r18_grid_v5.png)

The bottom band is real Fashion-MNIST, and it is the one to look at. The prior's own
...
![Identification against training compute](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r18_scaling_v4.svg)
...
![Resemblance against transfer](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r18_ablation_v4.svg)

The same axis orders Fashion-MNIST. It has **0.598** of its coordinates
within 0.1 of 0 or 1, against 0.914 for MNIST and 1.000 for chess.
Removing the simplex mode hurts retrieval on Fashion-MNIST by
0.030, against a draw-to-draw spread of
0.015.
...
world it has never seen — 0.631, against chance of
0.063 — and carries part of that to real Fashion-MNIST, at
0.269. It needed 15M parameters to do it; at 4M it did
not.
...
at either setting of the simplex mode, predicts a real Fashion-MNIST item better than
drawing the average real Fashion-MNIST item.
...
the `baselines_synth*_to_fashion_mnist` rows carry the ridge, soft look-up and average-item
