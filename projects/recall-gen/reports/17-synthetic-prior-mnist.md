# Recall and completion on a synthetic prior, tested on MNIST

Three networks were trained on a synthetic prior and never shown a real MNIST
...
Then all three were asked to do both jobs on real MNIST: find an item that is
present, and predict one that is absent.

| | finding, on MNIST | predicting, on MNIST |
|---|---|---|
| recall-trained | **0.394** | 1.711 |
| completion-trained | 0.069 | **1.736** |
| frozen layers | 0.069 | 1.497 |
...
![The three arms on MNIST](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r17_bars_v5.svg)

The left panel puts real MNIST beside the control that decides how to read
...
d_model=256 scored 0.544 on that control. The section below is
about that, and it is the reason the arms here are the wider ones.

On real MNIST the recall arm scores 0.394. The
completion-trained network scores 0.069 and the frozen
one 0.069 — both at chance. Across three independent
...
looks at the context and scores 1.419. The soft look-up uses
nothing but the context and scores 0.993. The best of the three
networks is **frozen layers** at 1.497.
...
context.** On MNIST it scores 1.734 when the answer is
present and 1.736 when it is absent — the same number, to three
...
is 0.069 against chance of 0.063. That signature has
...
![What the three networks produce](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r17_grid_v5.png)

The bottom band is real MNIST, and it is the one to look at. The prior's own
...
![Identification against training compute](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r17_scaling_v4.svg)
...
![Resemblance against transfer](https://media.tanh.xyz/seewhy/26-09-03/recall-gen_r17_ablation_v4.svg)

The same axis orders MNIST. It has **0.914** of its coordinates
within 0.1 of 0 or 1, against 0.598 for Fashion-MNIST and 1.000 for chess.
Removing the simplex mode helps retrieval on MNIST by
0.124, against a draw-to-draw spread of
0.015.
...
0.063 — and carries part of that to real MNIST, at
0.394. It needed 15M parameters to do it; at 4M it did
not.
...
at either setting of the simplex mode, predicts a real MNIST item better than
drawing the average real MNIST item.
...
the `baselines_synth*_to_mnist` rows carry the ridge, soft look-up and average-item
