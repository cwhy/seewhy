"""Recall-Gen — exp54: recall with 4 heads of dk=128, same parameters as exp49.
...
Usage:
    uv run --no-sync python projects/recall-gen/scripts/run_experiments.py --bg exp54
"""
...
RN = Run(
    exp_name="smoke_exp54" if SMOKE else "exp54",
    name="synthetic prior, recall arm, 4 heads x dk=128",
    domain="synth",
    ...
    cfg=Cfg(d_model=512, n_layers=4, dk=128, n_heads=4, n_tokens=20, d_in=PAD_W),
