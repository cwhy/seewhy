"""Recall-Gen — exp27: frozen mixer, knn context with 64 ranks skipped.
...
with it (`baselines_M16_r14_knn*_Q1`, soft look-up on an absent target):
    ...
    offset   64    0.701     this run
    offset  512    0.886     exp28
...
Usage:
    uv run python projects/recall-gen/scripts/run_experiments.py --bg exp27
"""
...
RN = Run(
    exp_name="smoke_exp27" if SMOKE else "exp27",
    name="frozen mixer, knn context with 64 ranks skipped (retrievability dial)",
    M=16, Q=1, mask_rows=14,
    batch=256, steps=200 if SMOKE else 12000, lr=3e-4, seed=0,
    train_mode="recall", ctx_mode="knn", knn_offset=64, train_only=EMBED_HEAD,
