"""Recall-Gen — exp43: exp41 trained four times as long.
...
exp41 was still improving when it stopped: its identification accuracy on its
own training worlds ran 0.06, 0.16, 0.39, 0.44, 0.47, 0.48 and was rising at the
final step, against 1.000 reached inside 2000 steps by every run on a fixed real
dataset. Inferring a fresh world from sixteen items, under a mask that also
changes every episode, is a far harder objective than any this project has run,
and 12 000 steps does not reach it.

Nothing else differs from exp41. If the transfer numbers move, exp41 was
measuring optimisation rather than the prior.
...
RN = Run(
    exp_name="smoke_exp43" if SMOKE else "exp43",
    name="synthetic prior, many worlds, random masks, 48k steps",
    domain="synth",
