"""
Universal Transformer with Memory on Sudoku-Extreme (exp5).
Fix: memory tokens get no RoPE (positions are zeroed out) — they should be
"positional-invariant scratchpad tokens" per the paper, not spatially biased.
Same training setup as exp4 (bs=128, 8 epochs, no remat).
Usage: uv run python projects/utm/scripts/run_experiments.py exp5
"""
...
LR             = 3e-4
BATCH_SIZE     = 128                          # half of 256: ~8GB peak activations, fits without remat
N_EPOCHS       = 8                            # paper uses 4; extra epochs to close gap to 57%
EMA_DECAY      = 0.999
WEIGHT_DECAY   = 0.01
LAMBDA_PONDER  = 0.0
HALT_BIAS      = -3.0
...
# RoPE: grid cells get positions 0-80; memory tokens get position 0 (angle=0 → cos=1,
# sin=0 → identity rotation). This makes memory tokens positional-invariant — they
# attend to all grid positions without spatial bias, matching the paper's description.
_d      = HEAD_DIM // 2
_theta  = 10000.0 ** (-jnp.arange(_d, dtype=jnp.float32) * 2.0 / HEAD_DIM)
_grid_pos = jnp.arange(N_CELLS, dtype=jnp.float32)             # (81,)
_mem_pos  = jnp.zeros(N_MEM, dtype=jnp.float32)                # (8,) — all zeros
_pos      = jnp.concatenate([_grid_pos, _mem_pos])              # (89,)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM mem-no-rope bs=128 D={D_MODEL} H={N_HEADS} K={K_PONDER} T={N_MEM} 8ep",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
