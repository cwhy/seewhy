"""
Universal Transformer with Memory on Sudoku-Extreme (exp2).
Full-scale replication of arxiv 2604.21999 — same architecture as exp1 but trained
on the complete 3.83M training set for 4 epochs (~60K steps/epoch, ~240K total).
Expects ~50-60% exact accuracy matching the paper's reported 57.4%.
Usage: uv run python projects/utm/scripts/run_experiments.py exp2
"""
...
# Rematerialised block: discards internal FFN/attention activations during the
# forward pass and recomputes them during backprop. Saves ~500MB per ponder step
# (18 steps × batch=256 ≈ 9 GB), at the cost of 2× compute per step.
...
CHUNK_SIZE = 50   # batches per JIT/XLA call; tune if OOM
