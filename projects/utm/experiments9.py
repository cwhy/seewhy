"""
Universal Transformer with Memory on Sudoku-Extreme (exp9).
Same as exp4 but with ACT ponder penalty (lambda=0.001, linear warmup over 2 epochs).
Paper Table 4: lambda warmup achieves 57.0% using 34% fewer ponder steps.
Usage: uv run python projects/utm/scripts/run_experiments.py exp9
"""
...
EMA_DECAY         = 0.999
WEIGHT_DECAY      = 0.01
LAMBDA_PONDER_MAX = 0.001                        # paper Table 4 value
LAMBDA_WARMUP_EP  = 2                            # ramp lambda from 0 → max over 2 epochs
HALT_BIAS         = -3.0
N_TRAIN           = None
N_TEST            = 20_000
SEED              = 42
...
def _rope(x):
    """Apply RoPE to x: (seq, n_heads, head_dim)."""
    x1, x2 = x[..., :_d], x[..., _d:]
    c = _COS[:, None, :]
    s = _SIN[:, None, :]
...
def loss_fn(params, lambda_val, x, y):
    logits, ponder_cost = forward(params, x)
    xe = jnp.mean(optax.softmax_cross_entropy_with_integer_labels(logits, y - 1))
    return xe + lambda_val * ponder_cost, (xe, ponder_cost)


def batch_loss(params, lambda_val, Xb, yb):
    losses, (xes, pcs) = jax.vmap(loss_fn, in_axes=(None, None, 0, 0))(params, lambda_val, Xb, yb)
...
def make_chunk_fn(optimizer):
    def step_body(carry, batch):
        params, ema_params, opt_state, lambda_val = carry
        Xb, yb = batch
        (loss, aux), grads = jax.value_and_grad(batch_loss, has_aux=True)(params, lambda_val, Xb, yb)
        ...
        return (new_params, new_ema, new_opt_state, lambda_val), (loss, *aux)

    @jax.jit
    def chunk_fn(params, ema_params, opt_state, lambda_val, Xb_chunk, yb_chunk):
        (params, ema_params, opt_state, _), stats = jax.lax.scan(
            step_body, (params, ema_params, opt_state, lambda_val), (Xb_chunk, yb_chunk)
        )
...
def train(params, X_train, y_train, X_test, y_test):
    ...
    history = {"loss": [], "xe": [], "ponder": [], "exact_acc": [], "lambda": []}
    t0 = time.time()

    for epoch in range(N_EPOCHS):
        # Linear warmup: 0 → LAMBDA_PONDER_MAX over first LAMBDA_WARMUP_EP epochs
        lam = jnp.array(min(LAMBDA_PONDER_MAX * (epoch + 1) / LAMBDA_WARMUP_EP, LAMBDA_PONDER_MAX))
        ...
        for c in range(n_chunks):
            params, ema_params, opt_state, (losses, xes, pcs) = chunk_fn(
                params, ema_params, opt_state, lam, Xb_all[c], yb_all[c]
            )
        ...
        history["loss"].append(loss_m)
        history["xe"].append(xe_m)
        history["ponder"].append(pc_m)
        history["exact_acc"].append(acc)
        history["lambda"].append(lam_f)

        logging.info(
            f"Epoch {epoch+1}/{N_EPOCHS} | loss={loss_m:.4f} xe={xe_m:.4f} "
            f"ponder={pc_m:.2f} lam={lam_f:.4f} | exact_acc={acc:.4f} | {elapsed:.0f}s"
        )
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":        EXP_NAME,
        "name":              f"UTM lambda-warmup lam={LAMBDA_PONDER_MAX} warmup={LAMBDA_WARMUP_EP}ep bs=128 D={D_MODEL} K={K_PONDER} T={N_MEM}",
        "d_model":           D_MODEL,
        "n_heads":           N_HEADS,
        "head_dim":          HEAD_DIM,
        "ffn_dim":           FFN_DIM,
        "n_mem":             N_MEM,
        "k_ponder":          K_PONDER,
        "lambda_ponder_max": LAMBDA_PONDER_MAX,
        "lambda_warmup_ep":  LAMBDA_WARMUP_EP,
        "halt_bias":         HALT_BIAS,
        "n_train":           N_TRAIN,
