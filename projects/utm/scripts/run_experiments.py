"""
Experiment runner for UTM.

Runs experiments sequentially (default) or in parallel across GPUs.
Logs each experiment to projects/utm/logs/{name}.log and prints a
results table when done.

Usage:
    # Run sequentially (blocks until done)
    uv run python projects/utm/scripts/run_experiments.py exp1 exp2
    ...
    uv run python projects/utm/scripts/run_experiments.py --parallel exp1 exp2

    # Force a specific GPU for sequential runs
    uv run python projects/utm/scripts/run_experiments.py --gpu 1 exp1

    # Run viz after each experiment
    uv run python projects/utm/scripts/run_experiments.py --viz scripts/gen_viz_exp1.py exp1
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run UTM experiments")
