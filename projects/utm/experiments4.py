"""
Universal Transformer with Memory on Sudoku-Extreme (exp4).
Faster training: BATCH_SIZE=128 cuts peak activations to ~8GB (fits without gradient
checkpointing), CHUNK_SIZE=100 gives ~300 Python iters/epoch. Expected ~31 min/epoch
vs 62 min in exp2/3. 8 epochs to push past exp2's 32.6% toward the paper's 57.4%.
Usage: uv run python projects/utm/scripts/run_experiments.py exp4
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM no-remat bs=64 D={D_MODEL} H={N_HEADS} K={K_PONDER} T={N_MEM} n_tr={N_TRAIN}",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
