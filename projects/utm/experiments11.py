"""
Universal Transformer with Memory on Sudoku-Extreme (exp11).
Larger model: hidden=768 (~7M params) for direct comparison with TRM (7M).
Paper future work: "unknown whether deep-start initialization would rescue hidden=768."
Uses remat (gradient checkpointing) + bs=64 to fit 24GB VRAM.
Usage: uv run python projects/utm/scripts/run_experiments.py exp11
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM D={D_MODEL} remat bs={BATCH_SIZE} H={N_HEADS} K={K_PONDER} T={N_MEM} 7M-params",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
