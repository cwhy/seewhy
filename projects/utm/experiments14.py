"""
Universal Transformer with Memory on Sudoku-Extreme (exp14).
D=768 (~7M params) + lambda warmup (paper Table 4: lambda=0.001 over 2 epochs).
Combines the two future-work suggestions: larger model + ponder penalty.
Uses remat + bs=64 to fit 24GB VRAM.
Usage: uv run python projects/utm/scripts/run_experiments.py exp14
"""
...
# ─── Hyperparameters ──────────────────────────────────────────────────────────
# fmt: off
D_MODEL        = 768                          # paper future work: hidden=768 ~7M params
N_HEADS        = 8
HEAD_DIM       = D_MODEL // N_HEADS          # 96
FFN_DIM        = int(D_MODEL * 8 / 3)        # 2048 — SwiGLU intermediate width
...
LR             = 3e-4
BATCH_SIZE     = 64                           # smaller batch to fit 24GB with remat
N_EPOCHS       = 8
EMA_DECAY      = 0.999
WEIGHT_DECAY      = 0.01
LAMBDA_PONDER_MAX = 0.001                        # paper Table 4
LAMBDA_WARMUP_EP  = 2                            # linear ramp over 2 epochs
HALT_BIAS         = -3.0
N_TRAIN           = None
N_TEST            = 20_000
SEED              = 42
...
# D=768 + K=18 unrolled backward needs remat even at bs=64.
...
CHUNK_SIZE = 50   # 50 × batch=64 with remat: fits in 24GB for D=768
...
def train(params, X_train, y_train, X_test, y_test):
    ...
    for epoch in range(N_EPOCHS):
        ...
        logging.info(
            ...
            f"ponder={pc_m:.2f} lam={float(lam):.4f} | exact_acc={acc:.4f} | {elapsed:.0f}s"
        )
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM D={D_MODEL} lambda-warmup lam={LAMBDA_PONDER_MAX} remat bs={BATCH_SIZE} K={K_PONDER} T={N_MEM} 7M",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
