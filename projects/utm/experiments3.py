"""
Universal Transformer with Memory on Sudoku-Extreme (exp3).
Ablation: T=0 memory tokens (no memory). Paper claims T=0 always fails (~0% exact acc).
Full dataset, same training as exp2. Confirms memory tokens are necessary.
Usage: uv run python projects/utm/scripts/run_experiments.py exp3
"""
...
N_MEM          = 0                            # ablation: no memory tokens
K_PONDER       = 18                           # max ACT ponder steps
VOCAB_SIZE     = 10                           # 0=empty cell, 1-9=digit
N_CELLS        = 81                           # 9×9 grid
SEQ_LEN        = N_CELLS + N_MEM             # 81 total tokens (no memory)
LR             = 3e-4
BATCH_SIZE     = 256
N_EPOCHS       = 4
EMA_DECAY      = 0.999
...
N_TRAIN        = None                         # full 3.83M training set
N_TEST         = 20_000                       # subset for faster eval
SEED           = 42
EXP_NAME       = "exp3"
# fmt: on
...
def halt_router(params, x):
    """Scalar halt probability pooled from grid tokens (no memory tokens in exp3)."""
    grid_mean = jnp.mean(x[:N_CELLS], axis=0)   # (D,)
    return jax.nn.sigmoid(grid_mean @ params["halt"]["W"] + params["halt"]["b"])


def forward(params, puzzle):
    """
    Full forward pass with ACT. No memory tokens (ablation).
    puzzle: (N_CELLS,) int32, values 0-9
    Returns: logits (N_CELLS, 9), ponder_cost scalar (expected depth)
    """
    x = params["embed"][puzzle]   # (81, D) — no memory tokens
