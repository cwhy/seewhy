"""
Universal Transformer with Memory on Sudoku-Extreme (exp8).
Continuation of exp7: loads exp7's EMA checkpoint (56.37% after 24 epochs) and trains
for 8 more epochs with LR=1e-5. Targets 57%+ to match the paper.
Usage: uv run python projects/utm/scripts/run_experiments.py exp8
"""
...
N_EPOCHS       = 8                            # 8 more epochs on top of exp7's 24
LR             = 1e-5                         # lower LR for continued fine-tuning
EMA_DECAY      = 0.999
WEIGHT_DECAY   = 0.01
LAMBDA_PONDER  = 0.0
HALT_BIAS      = -3.0
...
SEED           = 42
EXP_NAME       = "exp8"
CHECKPOINT_FROM = "exp7"                      # warm-start from exp7's EMA params (56.37%)
# fmt: on
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM cont-from-exp7 lr=1e-5 bs=128 D={D_MODEL} K={K_PONDER} T={N_MEM} 8ep",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
