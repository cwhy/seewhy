"""
Universal Transformer with Memory on Sudoku-Extreme (exp16).
Continuation of exp15 (55.53% after 16 epochs, D=768 7M). LR=3e-5, 8 more epochs.
Mirrors the exp6→exp7 strategy that pushed D=512 from 55.46% to 56.37%.
Usage: uv run python projects/utm/scripts/run_experiments.py exp16
"""
...
LR             = 3e-5                         # further reduced LR, mirrors exp6→exp7
BATCH_SIZE     = 64
N_EPOCHS       = 8
EMA_DECAY      = 0.999
WEIGHT_DECAY   = 0.01
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM cont-exp15 lr=3e-5 D={D_MODEL} remat bs={BATCH_SIZE} K={K_PONDER} T={N_MEM} 7M",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
