"""
Universal Transformer with Memory on Sudoku-Extreme (exp15).
Continuation of exp11 (52.00% after 8 epochs, D=768 7M). LR=1e-4, 8 more epochs.
Mirrors the exp4→exp6 strategy that pushed D=512 from 51.49% to 55.46%.
Usage: uv run python projects/utm/scripts/run_experiments.py exp15
"""
...
LR             = 1e-4                         # lower LR for fine-tuning phase
BATCH_SIZE     = 64
N_EPOCHS       = 8
EMA_DECAY      = 0.999
WEIGHT_DECAY   = 0.01
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":     EXP_NAME,
        "name":           f"UTM cont-exp11 lr=1e-4 D={D_MODEL} remat bs={BATCH_SIZE} K={K_PONDER} T={N_MEM} 7M",
        "d_model":        D_MODEL,
        "n_heads":        N_HEADS,
        "head_dim":       HEAD_DIM,
        "ffn_dim":        FFN_DIM,
