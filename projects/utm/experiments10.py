"""
Universal Transformer with Memory on Sudoku-Extreme (exp10).
Continuation of exp9 (51.14%, ponder=1.71) with LR=1e-4. Lambda already at 0.001 max.
Usage: uv run python projects/utm/scripts/run_experiments.py exp10
"""
...
EMA_DECAY         = 0.999
WEIGHT_DECAY      = 0.01
LAMBDA_PONDER_MAX = 0.001                        # keep at max — already warmed up in exp9
LAMBDA_WARMUP_EP  = 0                            # no warmup: lambda is fixed at max from epoch 1
HALT_BIAS         = -3.0
N_TRAIN           = None
N_TEST            = 20_000
SEED              = 42
EXP_NAME          = "exp10"
CHECKPOINT_FROM   = "exp9"
# fmt: on
...
def train(params, X_train, y_train, X_test, y_test):
    ...
    for epoch in range(N_EPOCHS):
        # Lambda fixed at max (already warmed up in exp9)
        lam = jnp.array(LAMBDA_PONDER_MAX)
...
if __name__ == "__main__":
    ...
    ckpt_path = PROJ_DIR / f"params_{CHECKPOINT_FROM}.pkl"
    ...
    append_result({
        "experiment":        EXP_NAME,
        "name":              f"UTM cont-exp9 lam={LAMBDA_PONDER_MAX} lr=1e-4 bs=128 D={D_MODEL} K={K_PONDER} T={N_MEM}",
        "d_model":           D_MODEL,
        "n_heads":           N_HEADS,
        "head_dim":          HEAD_DIM,
        "ffn_dim":           FFN_DIM,
