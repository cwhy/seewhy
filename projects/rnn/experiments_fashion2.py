"""
Fashion-MNIST high-end sweep (fashion2).

Pushes gram_f3 and pure_mlp into the 136K–1.6M parameter range to find
the practical ceiling of each family and see if gram_f3 can match/beat
pure_mlp at equal total params.

Reference points from fashion1:
  gram_f3  K=4 mlp=256 noise15  69,610 total  → 89.64%
  gram_f3  K=4 d2  mlp=128 n30  35,562 total  → 89.58%
  pure_mlp 1L mlp=512           407,050 total  → 90.28%
  pure_mlp 2L 512×256           535,818 total  → 90.01%

Usage:
    uv run python projects/rnn/experiments_fashion2.py
"""
...
def make_pure_mlp_3layer(h1, h2, h3):
    kg = infinite_safe_keys_from_key(jax.random.PRNGKey(SEED))
    p = {
        "W1": jax.random.normal(next(kg).get(), (h1,  784)) / 784**0.5,
        "b1": jnp.zeros(h1),
        "W2": jax.random.normal(next(kg).get(), (h2,  h1))  / h1**0.5,
        "b2": jnp.zeros(h2),
        "W3": jax.random.normal(next(kg).get(), (h3,  h2))  / h2**0.5,
        "b3": jnp.zeros(h3),
        "W4": jax.random.normal(next(kg).get(), (10,  h3))  * 0.01,
        "b4": jnp.zeros(10),
    }
    def fwd(params, x):
        h = jax.nn.gelu(x / 255.0 @ params["W1"].T + params["b1"])
        h = jax.nn.gelu(h         @ params["W2"].T + params["b2"])
        h = jax.nn.gelu(h         @ params["W3"].T + params["b3"])
...
if __name__ == "__main__":
    ...
    # gram_f3: best noise from fashion1 was noise=15 for large models
    # non_emb = K*64*(3+mlp) + depth*128 + 11*mlp + 10
    experiments = [
        # ── gram_f3 ~136K total ───────────────────────────────────────────────
        # K=8 mlp=256: 135,914 total
        ("gram_f3 K=8 mlp=256 +noise15",
         *make_f3_bilinear(4,4,4, 256, n_queries=8), 15.0, "gram_f3"),
        ("gram_f3 K=8 depth=2 mlp=256 +noise15",
         *make_f3_bilinear(4,4,4, 256, n_queries=8, depth=2), 15.0, "gram_f3"),
        # K=4 mlp=512: 137,962 total
        ("gram_f3 K=4 mlp=512 +noise15",
         *make_f3_bilinear(4,4,4, 512, n_queries=4), 15.0, "gram_f3"),
        ("gram_f3 K=4 depth=2 mlp=512 +noise15",
         *make_f3_bilinear(4,4,4, 512, n_queries=4, depth=2), 15.0, "gram_f3"),

        # ── gram_f3 ~270K total ───────────────────────────────────────────────
        # K=8 mlp=512: 269,802 total
        ("gram_f3 K=8 mlp=512 +noise15",
         *make_f3_bilinear(4,4,4, 512, n_queries=8), 15.0, "gram_f3"),
        ("gram_f3 K=8 depth=2 mlp=512 +noise15",
         *make_f3_bilinear(4,4,4, 512, n_queries=8, depth=2), 15.0, "gram_f3"),
        # K=4 mlp=1024: 274,666 total
        ("gram_f3 K=4 mlp=1024 +noise15",
         *make_f3_bilinear(4,4,4, 1024, n_queries=4), 15.0, "gram_f3"),

        # ── gram_f3 ~537K total ───────────────────────────────────────────────
        # K=8 mlp=1024: 537,578 total  (matches pure_mlp-512 budget at 407K)
        ("gram_f3 K=8 mlp=1024 +noise15",
         *make_f3_bilinear(4,4,4, 1024, n_queries=8), 15.0, "gram_f3"),

        # ── pure_mlp 1-layer ─────────────────────────────────────────────────
        # mlp=1024: 814,090 total
        ("pure_mlp 1L mlp=1024 +noise30",
         *make_pure_mlp(1024), 30.0, "pure_mlp"),
        # mlp=2048: 1,628,170 total
        ("pure_mlp 1L mlp=2048 +noise30",
         *make_pure_mlp(2048), 30.0, "pure_mlp"),

        # ── pure_mlp 2-layer ─────────────────────────────────────────────────
        # 512×512: 669,706 total
        ("pure_mlp 2L h=512x512 +noise30",
         *make_pure_mlp_2layer(512, 512), 30.0, "pure_mlp2"),
        # 1024×512: 1,333,770 total
        ("pure_mlp 2L h=1024x512 +noise30",
         *make_pure_mlp_2layer(1024, 512), 30.0, "pure_mlp2"),

        # ── pure_mlp 3-layer ─────────────────────────────────────────────────
        # 512×256×128: 537,482 total
        ("pure_mlp 3L h=512x256x128 +noise30",
         *make_pure_mlp_3layer(512, 256, 128), 30.0, "pure_mlp2"),
        # 1024×512×256: 1,334,282 total
        ("pure_mlp 3L h=1024x512x256 +noise30",
         *make_pure_mlp_3layer(1024, 512, 256), 30.0, "pure_mlp2"),
    ]
    ...
    for label, fwd, params, emb_keys, noise, family in experiments:
        ...
        tr_acc, te_acc, elapsed = train(
            ...
            noise_scale=noise)
        row = {
            "name": label, "experiment": "fashion2",
            "family": family,
