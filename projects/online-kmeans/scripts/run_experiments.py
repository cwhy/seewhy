"""
Experiment runner for online-kmeans.

Runs experiments sequentially (default) or in parallel across GPUs.
Logs each experiment to projects/online-kmeans/logs/{name}.log and prints a
results table when done.

Usage:
    # Run sequentially (blocks until done)
    uv run python projects/online-kmeans/scripts/run_experiments.py exp1 exp2
    ...
    uv run python projects/online-kmeans/scripts/run_experiments.py --gpu 0 exp1

    # Run viz after each experiment
    uv run python projects/online-kmeans/scripts/run_experiments.py --viz scripts/gen_viz_clustering.py exp1
"""
...
def print_results_table(names: list[str]):
    ...
    for name in names:
        ...
        if r:
            ...
            rows.append({"experiment": name, "final_match_l2": float("nan"),
                         "final_label_acc_hard": float("nan"),
                         "final_label_acc_soft": float("nan"),
    ...
    print("-" * len(header))
    for r in rows:
        l2   = r.get("final_match_l2", float("nan"))
        hard = r.get("final_label_acc_hard", float("nan"))
        soft = r.get("final_label_acc_soft", float("nan"))
        t    = r.get("time_s", float("nan"))
        l2_s   = f"{l2:.4f}"   if l2   == l2   else "  —   "
        hard_s = f"{hard:.4f}" if hard == hard else "  —   "
        soft_s = f"{soft:.4f}" if soft == soft else "  —   "
        t_s    = f"{t:.0f}s"   if t    == t    else "—"
        print(
...
def main():
    parser = argparse.ArgumentParser(description="Run online-kmeans experiments")
