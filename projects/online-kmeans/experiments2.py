"""
exp2 — Mini-batch K-means (BATCH_SIZE=256) on raw MNIST pixels.

Count-based incremental centroid update:
  new_counts = counts + batch_counts
  lr = batch_counts / new_counts          (per cluster)
  centroids += lr * (batch_mean - centroids)

Counts accumulate across the full training run. Effective lr decays as 1/t.
Dead clusters are reinitialised from random training samples once per epoch.

Centroid update uses JAX immutable scatter:
  sums  = jnp.zeros_like(centroids).at[assign].add(batch)
  new_c = centroids + lr * (batch_mean - centroids)    # no in-place mutations

Compare with:
  exp1 — full-batch Lloyd's (exact mean update)
  exp3 — small mini-batch K-means (BATCH_SIZE=32)

Usage:
    uv run python projects/online-kmeans/experiments2.py
"""
...
def batch_step(centroids, counts, batch):
    ...
    dists    = sq_dists(batch, centroids)                      # (BS, K)
    assign   = jnp.argmin(dists, axis=1)                      # (BS,)
    b_counts = jnp.zeros(N_CODES).at[assign].add(1.0)         # (K,)
    b_sums   = jnp.zeros_like(centroids).at[assign].add(batch) # (K, D)

    new_counts = counts + b_counts
    # lr per cluster: n_new / total — 0 where cluster had no batch assignments
...
def train(centroids, X_tr, X_te, Y_te, key):
    ...
    counts = jnp.ones(N_CODES)   # initialised to 1 to avoid div-by-zero at first batch
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for b in range(num_batches):
            ...
            # Compute per-batch loss for logging (on CPU to avoid blocking GPU)
...
if __name__ == "__main__":
    ...
    save_learning_curves(history, EXP_NAME,
                         subtitle=f"Mini-batch K-means  N_CODES={N_CODES}  BS={BATCH_SIZE}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"minibatch-kmeans N_CODES={N_CODES} BS={BATCH_SIZE}",
