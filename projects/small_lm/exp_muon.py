"""
exp_muon — Muon optimizer: Newton-Schulz orthogonalisation for 2D weight matrices.

Hypothesis: Muon's approximate Newton step on weight matrices improves convergence
speed, potentially reaching lower PPL in less wall-clock time.

What changed vs exp_baseline:
  - Optimizer: Muon (NS orthogonalisation, lr=0.02) for all 2D weight matrices;
    AdamW (lr=3e-4) for embeddings, biases, and LayerNorm params.
  - Architecture: identical to baseline (learned pos embed, weight-tied LM head).

Usage:
    uv run python projects/small_lm/scripts/run_experiments.py --bg exp_muon
"""
...
def newton_schulz(G: jnp.ndarray, steps: int = 5) -> jnp.ndarray:
    """Approximate matrix orthogonalisation via Newton-Schulz iteration.

    Maps G (m×n) to an approximately orthogonal matrix of the same shape.
    Works for both tall and wide matrices.
    """
    assert G.ndim == 2
...
def muon_update(updates: dict, params: dict) -> dict:
    """Apply Newton-Schulz orthogonalisation to 2D weight matrix gradients."""
    out = {}
    for k, g in updates.items():
        if g.ndim == 2 and k not in ("token_embed", "pos_embed"):
            out[k] = newton_schulz(g) * MUON_LR
        else:
            out[k] = g   # AdamW handles these
    return out


def make_optimizer(params: dict):
    """Multi-transform optimizer: Muon for 2D weights, AdamW for everything else."""
    lr_schedule = optax.warmup_cosine_decay_schedule(
        init_value=0.0,
        peak_value=1.0,    # scaled per-group below
        warmup_steps=WARMUP_STEPS,
        decay_steps=MAX_STEPS,
        end_value=0.1,
    )

    # Identify which keys get Muon vs AdamW
    muon_keys = {k for k in params if params[k].ndim == 2 and k not in ("token_embed", "pos_embed")}

    def param_labels(params):
        return {k: "muon" if k in muon_keys else "adam" for k in params}
    ...
    tx = optax.multi_transform(
        {
            "muon": optax.chain(
                ...
                # NS orthogonalisation applied in train_step after optimizer
            ),
            "adam": optax.chain(
                optax.clip_by_global_norm(GRAD_CLIP),
                optax.adamw(
...
# ── Training step with Muon ────────────────────────────────────────────────────

def make_train_step_fn(optimizer, muon_keys):
    """JIT-compiled step: gradients → (NS-ortho for 2D weights, AdamW for rest)."""
    def train_step(carry, token_ids_batch):
        params, opt_state = carry
        loss, grads = jax.value_and_grad(loss_fn)(params, token_ids_batch)
        # Apply Newton-Schulz to Muon-managed 2D weight grads before optimizer
        ns_grads = {
            k: (newton_schulz(g) if k in muon_keys else g)
            for k, g in grads.items()
        }
        updates, new_opt_state = optimizer.update(ns_grads, opt_state, params)
...
def train(params, train_tokens, eval_tokens):
    optimizer, muon_keys = make_optimizer(params)
    opt_state  = optimizer.init(params)
    train_step = make_train_step_fn(optimizer, muon_keys)
