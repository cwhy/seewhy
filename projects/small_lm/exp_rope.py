"""
exp_rope — RoPE positional encoding instead of learned positional embeddings.

Hypothesis: RoPE (Rotary Position Embedding) encodes relative positions directly
in the Q/K dot-product, which may generalise better and converge faster than
adding a learned absolute pos_embed to the token embeddings.

What changed vs exp_baseline:
  - Removed: p["pos_embed"] (saves MAX_SEQ_LEN × D_MODEL = 49,152 params)
  - Added: rope_rotate() applied to Q and K inside attention()
  - Optimizer: same AdamW as baseline.
  - Everything else: identical.

Usage:
    uv run python projects/small_lm/scripts/run_experiments.py --bg exp_rope
"""
...
# Precompute RoPE sin/cos tables: shape (MAX_SEQ_LEN, HEAD_DIM)
def _build_rope_tables(seq_len: int, head_dim: int):
    ...
    freqs = jnp.outer(t, theta)        # (seq_len, half)
    cos   = jnp.concatenate([jnp.cos(freqs), jnp.cos(freqs)], axis=-1)  # (seq_len, head_dim)
...
def rope_rotate(x: jnp.ndarray, T: int) -> jnp.ndarray:
    """Apply RoPE rotation to x of shape (B, H, T, head_dim).

    Rotates pairs of dimensions using the precomputed cos/sin tables.
    """
    cos = ROPE_COS[:T][None, None, :, :]  # (1, 1, T, head_dim)
    sin = ROPE_SIN[:T][None, None, :, :]

    # Rotate: split into two halves and apply the rotation matrix
    half = x.shape[-1] // 2
    x1, x2 = x[..., :half], x[..., half:]
...
def attention(params, x, layer_i):
    """Causal MHA with RoPE applied to Q and K."""
    B, T, _ = x.shape
...
def forward(params, token_ids):
    B, T = token_ids.shape
    # No positional embedding added — RoPE handles it in attention
    x = params["token_embed"][token_ids]   # (B, T, D_MODEL)
    for i in range(N_LAYERS):
        x = transformer_block(params, x, i)
    x = layer_norm(x, params["final_ln_s"], params["final_ln_b"])
    return x @ params["token_embed"].T     # weight-tied LM head
