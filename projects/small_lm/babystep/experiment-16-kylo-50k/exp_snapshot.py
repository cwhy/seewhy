...
The character had these recent exchanges:
{{exchanges_block}}

Extract up to 5 new facts revealed in these exchanges that are NOT already \
covered by the current facts above. If nothing new, return an empty list.
...
Generate {{n_questions}} diverse questions to learn more about this character. \
Prefer areas not yet covered by the known facts above. \
Ask directly to the character. \
...
def _update_facts_from_buffer(client, identity, current_facts, pairs_buffer, idx):
    """Two-step facts update from a buffer of (question, label) pairs.
...
def sample_pairs(all_pairs, n, pair_losses):
    ...
    # On-policy: most recent n pairs
    idx = list(range(N - n, N))
...
def main():
    ...
    # Buffer of (question, label) pairs accumulated since last facts update submission
    pairs_buffer: list[tuple[str, str]] = []
    ...
    for batch_idx in range(1, N_BATCHES + 1):
        ...
        pairs_buffer.append((question_cur, label))
        if batch_idx % FACTS_UPDATE_EVERY == 0:
            facts_snap  = list(all_facts)
            buffer_snap = list(pairs_buffer)
            pairs_buffer.clear()
            facts_fut = executor.submit(
                _update_facts_from_buffer, client, identity, facts_snap, buffer_snap, batch_idx)
