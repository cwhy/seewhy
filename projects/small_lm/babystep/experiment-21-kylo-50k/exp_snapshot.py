...
from ai_eval.openrouter import OpenRouterClient, BudgetExceededError
...
The character had these recent exchanges. Each shows the teacher's intended answer \
and what the model actually generated. Only extract facts from exchanges marked \
"→ aligned" — these indicate the model has genuinely learned that aspect of the \
character. Ignore "→ diverged" exchanges entirely.

{{exchanges_block}}

Extract up to 5 new facts from ALIGNED exchanges only that are NOT already covered \
by the current facts above. Write each fact so it can be understood at any education \
level. If nothing new, return an empty list.
    ...
    for attempt in range(3):
        try:
            ...
            if "exact_sentence" in data:
                ...
                return idx, sentence
            logging.warning(f"  [label {idx}] attempt {attempt+1}: missing 'exact_sentence', got keys={list(data.keys())} raw={raw[:120]}")
            user_msg += "\n\nReturn valid JSON only: {\"exact_sentence\": \"...\"}"
...
def _update_facts_from_buffer(client, identity, current_facts, pairs_buffer, pair_losses_snap, gen_outs_snap, idx):
    """Two-step facts update from a buffer of (pair_idx, question, label) tuples.

    Step 1: extract up to 5 new facts from aligned exchanges only.
    ...
    for pair_idx, q, a in pairs_buffer:
        gen_out   = gen_outs_snap.get(pair_idx, "")
        align_tag = "→ aligned" if is_aligned(a, gen_out) else "→ diverged"
        lines.append(f'Q: "{q}"\nTeacher: "{a}"\nModel: "{gen_out}"\n{align_tag}')
    exchanges_block = "\n\n".join(lines)
...
def is_aligned(label, gen_out, threshold=0.3):
    """True if model output has sufficient word overlap with the teacher label."""
    if not gen_out or not label or label.strip() == "...":
        return False
    label_words = set(label.lower().split())
    gen_words   = set(gen_out.lower().split())
    overlap = len(label_words & gen_words) / max(len(label_words), 1)
...
def refill_pending(n=20):
    ...
    try:
        ...
        existing = []
    # Exclude questions already sitting in the pending queue
    existing_set = set(existing)
    candidates = [q for q in all_qs if q not in existing_set]
    if not candidates:
        candidates = all_qs  # fallback if all questions are already pending
    weights = np.arange(1, len(candidates) + 1, dtype=float) ** 2
    weights /= weights.sum()
    chosen = list(np.random.choice(candidates, size=min(n, len(candidates)), replace=False, p=weights))
    with open(PENDING_FILE, "w") as f:
...
def main():
    ...
    # gen output keyed by pair_idx, for alignment checking in facts extraction
    gen_outs: dict[int, str] = {}
    ...
    try:
        credits_start = client.get_credits()
        logging.info(f"OpenRouter credit balance: ${credits_start:.4f}")
    except Exception as e:
        credits_start = None
        logging.warning(f"Could not fetch credit balance: {e}")
    ...
    for batch_idx in range(1, N_BATCHES + 1):
        ...
        try:
            ...
            logging.error(f"  [batch {batch_idx}] BUDGET EXCEEDED — stopping experiment: {e}")
            budget_exceeded = True
            break

        # ── Step 4: Record pair, accumulate buffer, submit facts update async ──
        if label.strip() == "...":
            logging.warning(f"  [batch {batch_idx}] skipping bad label, not adding to pairs")
        else:
        ...
        if batch_idx % FACTS_UPDATE_EVERY == 0 and pairs_buffer:
            facts_snap    = list(all_facts)
            buffer_snap   = list(pairs_buffer)
            losses_snap   = dict(pair_losses)
            gen_outs_snap = dict(gen_outs)
            pairs_buffer.clear()
            facts_fut = executor.submit(
                _update_facts_from_buffer, client, identity, facts_snap, buffer_snap, losses_snap, gen_outs_snap, batch_idx)
        ...
        if gen_future_cur is not None:
            try:
                ...
                if gen_out and all_pairs:
                    gen_outs[len(all_pairs) - 1] = gen_out
    ...
    # ── Cost reporting ────────────────────────────────────────────────────────
    try:
        credits_end = client.get_credits()
        cost = (credits_start - credits_end) if credits_start is not None else None
        cost_str = f"${cost:.4f}" if cost is not None else "n/a"
        tok_str  = f"{client.prompt_tokens:,}p + {client.completion_tokens:,}c tokens"
        logging.info(f"API cost this run: {cost_str}  ({tok_str})")
        with open(REPORT_FILE, "a") as f:
            f.write(f"\n## API Cost\n\n"
                    f"**Cost:** {cost_str}  \n"
                    f"**Tokens:** {tok_str}  \n"
                    f"**Credit balance remaining:** ${credits_end:.4f}\n")
    except Exception as e:
        logging.warning(f"Cost reporting failed: {e}")

    if budget_exceeded:
        logging.error("Experiment stopped early: OpenRouter budget exceeded.")
