"""
exp_kdyck — k-Dyck procedural pre-training followed by Guppy fine-tuning.

Hypothesis: Warming up on k-Dyck balanced-bracket sequences teaches the model
hierarchical dependency structure before it sees any semantic data.  This
should improve data efficiency on the Guppy task (lower final PPL or faster
convergence) compared to the cold-start baseline.

Two-phase training (per Shinnick et al. arXiv:2601.21725):
  Phase 1 — Procedural warm-up: PRETRAIN_STEPS steps on k-Dyck sequences.
             Constant LR, no eval (Guppy eval would be noise at this stage).
  Phase 2 — Fine-tuning: MAIN_STEPS steps on Guppy conversations.
             Same cosine+warmup schedule as exp_baseline.
             Optimizer state from phase 1 is KEPT (not reset).

k-Dyck token IDs: opens = [3..18], closes = [19..34] for k=16.
All 2k=32 bracket tokens are within our VOCAB_SIZE=4096.

Usage:
    uv run python projects/small_lm/scripts/run_experiments.py --bg exp_kdyck
"""
...
# k-Dyck warm-up
KDYCK_K             = 16     # bracket types (32 token IDs total)
PRETRAIN_STEPS      = 1_000  # phase 1 steps (~9% of total)
PRETRAIN_LR         = 3e-4   # constant LR during phase 1
KDYCK_EPOCH_SAMPLES = 10_000 # synthetic samples per epoch
...
# ── Parameter initialisation (identical to baseline) ──────────────────────────
...
# ── Core model (identical to baseline) ────────────────────────────────────────
...
def pretrain_kdyck(params, kdyck):
    """Train on k-Dyck sequences for PRETRAIN_STEPS steps.

    Returns (params, opt_state, history) where opt_state is kept for phase 2.
    """
    ...
    optimizer = optax.chain(
        ...
        optax.adamw(PRETRAIN_LR, weight_decay=0.0),
    )
...
def finetune_guppy(params, opt_state_phase1, train_tokens, eval_tokens):
    """Fine-tune on Guppy data, keeping the optimizer state from phase 1.

    Per the paper: optimizer state is NOT reset at the phase boundary.
    We do restart the LR schedule (cosine+warmup) for the main phase.
    """
    logging.info(f"Phase 2: Guppy fine-tuning for {MAIN_STEPS} steps")

    # New LR schedule for phase 2 (warmup+cosine), but seed the optimizer
    # with the momentum state from phase 1 by chaining into a new schedule.
    # We reconstruct with the same structure so the momentum buffers transfer.
    optimizer = optax.chain(
        optax.clip_by_global_norm(GRAD_CLIP),
        optax.adamw(
    ...
    # Re-initialise fresh (LR state doesn't carry over, but this is standard)
    opt_state  = optimizer.init(params)
    train_step = make_train_step_fn(optimizer)
    ...
    while step < MAIN_STEPS:
        ...
        for batch in tokens:
            ...
            if step % EVAL_EVERY == 0 or step == MAIN_STEPS:
                ...
                logging.info(
                    f"  [phase2] step {step:5d}  loss={float(loss):.4f}  "
...
if __name__ == "__main__":
    ...
    logging.info(f"k-Dyck: k={KDYCK_K}, {kdyck.n_tokens} token IDs "
                 f"(range {kdyck.open_ids[0]}..{kdyck.close_ids[-1]})")
    ...
    logging.info(f"Done. eval_ppl={final['eval_ppl']:.2f}  total={elapsed:.0f}s")
