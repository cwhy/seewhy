"""
exp31 — DiVeQ-PQ LATENT=32, G=8, D_g=4, K=512, interleaved grouping.

Same as exp29 (G=8, D_g=4, K=512, 200ep) but with interleaved dimension
assignment. Group g gets dims: g, g+G, g+2G, g+3G  (i.e. z[:, g::N_GROUPS]).

exp29 with contiguous slicing yielded 0.007328 (worse than LATENT=16).
Group 2 (dims 8-11) had only 69/512 utilization — contiguous slicing concentrated
a low-variance region into a single group. Interleaved grouping mixes dims from
across the full 32-dim space into each group.
"""
...
LATENT_DIM    = 32
N_GROUPS      = 8
GROUP_DIM     = LATENT_DIM // N_GROUPS   # = 4
CODEBOOK_SIZE = 512
SIGMA         = 0.01
HIDDEN        = 512
BATCH_SIZE    = 128
