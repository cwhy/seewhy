"""
exp35 — DiVeQ-PQ LATENT=32, G=16, D_g=2, K=128 + running-stats BN, 500 epochs.

Same as exp34 (running-stats batch normalization) but trained for 500 epochs.
exp34 (200ep) achieved eval_mse=0.00388 (7.2% above AE ceiling of 0.003618) with
near-perfect codebook utilization [478-512/512 across all 16 groups]. With 500
epochs and cosine LR decay, expect further convergence toward the AE ceiling.

exp34 fix recap: track population statistics (running mean/var), updated once per
epoch from a representative training sample. Eval uses population stats →
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"bits={total_bits}  sigma={SIGMA}  HIDDEN={HIDDEN}  (running-stats BN, K=128, 500ep)"
    )
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"DiVeQ-PQ+BN-running G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits} 500ep",
