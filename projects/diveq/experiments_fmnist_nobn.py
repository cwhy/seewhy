"""
fmnist_nobn32 — DiVeQ-PQ on Fashion-MNIST WITHOUT batch normalization.

Tests if BN is required for Fashion-MNIST at LATENT=32.
Hypothesis: FMNIST has higher intrinsic dimensionality (~20+ dims?), so the
encoder naturally uses all 32 dimensions without BN. If true, fmnist_nobn32
should achieve similar MSE to fmnist_vq32 (0.00711).
If lazy groups form, BN is universally required regardless of dataset.

Compare against:
- fmnist_vq32:   0.00711 (0.14% above ceiling) — with BN
- fmnist_ae32:   0.00710 — AE ceiling
- exp28 (MNIST): 0.004223 — 16.7% above ceiling without BN (lazy groups)
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":         EXP_NAME,
        "name":               f"FMNIST DiVeQ-PQ (no BN) G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits} 500ep",
