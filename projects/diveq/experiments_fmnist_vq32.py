"""
fmnist_vq32 — DiVeQ-PQ on Fashion-MNIST with running-stats BN.

Tests if the best MNIST config (LATENT=32, G=16, D_g=2, K=512, 500ep, BN)
generalizes to Fashion-MNIST.

Questions:
1. Does BN fix lazy groups on FMNIST? (FMNIST has higher intrinsic dim than MNIST)
2. How close can we get to the FMNIST AE32 ceiling?
3. Is the % above ceiling comparable to MNIST (2.8%)?

Compare against: fmnist_ae32 (AE ceiling for LATENT=32 on FMNIST)
"""
...
EXP_NAME = "fmnist_vq32"

# ── Hyperparameters (same as exp35/44 for MNIST) ──────────────────────────────
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":         EXP_NAME,
        "name":               f"FMNIST DiVeQ-PQ+BN G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits} 500ep",
