...
Usage:
    ...
    uv run python projects/diveq/scripts/poll_result.py exp1 --interval 5
    ...
    while True:
        for r in read_results():
            if r.get("experiment") == exp_name:
                ...
                t    = r.get("time_s", "?")
                n    = r.get("n_params", "?")
                print(f"{exp_name}: recon_mse={mse}  qe={qe}  codebook_use={cb}  time={t}s  params={n:,}")
                return r
        time.sleep(interval)
...
def show_all():
    ...
    for r in results:
        print(
            ...
            f" {r.get('final_recon_mse', 0):>10.6f}"
            f" {r.get('final_qe', 0):>10.6f}"
            f" {r.get('final_codebook_use', 0):>10}"
