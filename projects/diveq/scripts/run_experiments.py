"""
Experiment runner for diveq.

Runs experiments sequentially (default) or in parallel across GPUs.
Logs each experiment to projects/diveq/logs/{name}.log and prints a
...
Usage:
    ...
    uv run python projects/diveq/scripts/run_experiments.py --parallel exp1 exp2

    # Pin to a specific GPU
    uv run python projects/diveq/scripts/run_experiments.py --gpu 1 exp1
"""
...
def print_results_table(names: list[str]):
    ...
    header = f"{'exp':<8} {'recon_mse':>10} {'qe':>10} {'codebook':>10} {'time':>7} {'params':>10}  name"
    print("\n" + header)
    print("-" * len(header))
    for r in rows:
        mse = r.get("final_recon_mse", float("nan"))
        qe  = r.get("final_qe", float("nan"))
        cb  = r.get("final_codebook_use", 0)
        mse_str = f"{mse:.6f}" if mse == mse else "    —   "
        qe_str  = f"{qe:.6f}"  if qe  == qe  else "    —   "
...
def main():
    parser = argparse.ArgumentParser(description="Run diveq experiments")
