"""
exp45 — DiVeQ-PQ LATENT=64, G=32, D_g=2, K=512 + running-stats BN, 200 epochs.

exp43 established the LATENT=64 AE ceiling: MSE=0.00181 (vs 0.003618 for LATENT=32).
A 50% improvement in representational capacity — worth exploring quantization at this latent size.

Config: 16 groups × 9 bits = 144 total bits.
Same running-stats BN pattern as exp35 (the fix for lazy groups).
LATENT=64 will have even more lazy dims without BN; BN is mandatory here.

Hypothesis: if BN fixes lazy groups at LATENT=32, it should also fix LATENT=64.
Target: within ~5% of ceiling (0.00181) at 200ep → ≈0.0019.
"""
...
def train(params, X_tr, X_te):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if (epoch + 1) % EVAL_EVERY == 0:
            ...
            eval_str = f"  eval_mse={eval_mse:.5f}  cb_min={min(n_used)}"
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"bits={total_bits}  sigma={SIGMA}  HIDDEN={HIDDEN}  (running-stats BN, 200ep)"
    )
    ...
    append_result({
        "experiment":         EXP_NAME,
        "name":               f"DiVeQ-PQ+BN G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits} 200ep",
