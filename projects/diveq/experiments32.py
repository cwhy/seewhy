"""
exp32 — DiVeQ-PQ LATENT=32, G=16, D_g=2, K=512 + learned linear projection.

Adds a trainable (LATENT_DIM × LATENT_DIM) linear layer between the encoder
and PQ grouping, initialized as the identity matrix. This layer learns to
redistribute encoder variance equally across groups, eliminating lazy groups.

exp28/30 showed that 6/16 groups have <100/512 utilization regardless of
whether contiguous or interleaved grouping is used, because certain individual
latent dimensions carry very little variance. A learned rotation can mix dims
to equalize group information content.

Architecture:
    MLP encoder  784 → 512 → 512 → 32
    Linear proj  32 → 32  (trainable, initialized I)  ← new
    DiVeQ-PQ     G=16 contiguous groups of 2 dims, K=512 → 144 bits
    MLP decoder  32 → 512 → 512 → 784   (decodes from projected space)
"""
...
def init_params(key_gen):
    ...
    params = {
        ...
        # Learned projection: initialized as identity, no bias
        "proj":   jnp.eye(LATENT_DIM),
...
def encode(params, x):
    ...
    return z @ params["proj"]   # project into PQ space

def decode(params, zq):
    # Decoder receives zq in projected space — learns to decode directly from it
...
def loss_fn(params, x_batch, key):
    x = x_batch / 255.0
    z  = encode(params, x)       # already projected
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"bits={total_bits}  sigma={SIGMA}  HIDDEN={HIDDEN}  (learned projection)"
    )
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"DiVeQ-PQ+proj G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits}",
        ...
        "projection":       "learned_linear_32x32",
    })
