"""
exp30 — DiVeQ-PQ LATENT=32, G=16, D_g=2, K=512, interleaved grouping.

Same as exp28 (G=16, D_g=2, K=512, 200ep) but uses interleaved dimension
assignment instead of contiguous slices:

    Group g gets dims: g, g+G, g+2G, ...  (i.e. z[:, g::N_GROUPS])

Motivation: exp28 revealed that the MLP encoder concentrates variance unevenly
across the 32 latent dims. Contiguous slicing creates "lazy groups" (groups 0,1
with only 28-44/512 utilization). Interleaved grouping pairs each low-variance
dim with a high-variance dim, equalizing information across groups without any
new parameters.

Architecture:
    MLP encoder  784 → 512 → 512 → 32
    DiVeQ-PQ     G=16 interleaved groups of 2 dims each, K=512 → 144 bits
    MLP decoder  32 → 512 → 512 → 784
"""
...
# ── DiVeQ-PQ layer (interleaved grouping) ────────────────────────────────────
...
def diveq_pq(params, z, key, training=True):
    ...
    for g in range(N_GROUPS):
        z_g = z[:, g::N_GROUPS]                              # (B, D_g) interleaved
        C_g = codebooks[g]
    ...
    # Scatter groups back into interleaved positions
    zq = jnp.zeros_like(z)
    for g in range(N_GROUPS):
        zq = zq.at[:, g::N_GROUPS].set(zq_groups[g])
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"bits={total_bits}  sigma={SIGMA}  HIDDEN={HIDDEN}  (interleaved grouping)"
    )
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"DiVeQ-PQ interleaved G={N_GROUPS} K={CODEBOOK_SIZE} D_g={GROUP_DIM} bits={total_bits}",
