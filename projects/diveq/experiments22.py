"""
exp22 — DiVeQ with product quantization (G=4 groups, K=512 per group).
...
def init_params(key_gen):
    ...
    # Codebook init: normal(0, 0.1) — wide enough to match encoder output range for D_g=1
    params["codebooks"] = n((N_GROUPS, CODEBOOK_SIZE, GROUP_DIM), scale=0.1)
    return params

# ── Encoder / Decoder ─────────────────────────────────────────────────────────

def encode(params, x):
    """x (B, 784) → z (B, LATENT_DIM)."""
