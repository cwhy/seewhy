...
Compile the Typst report and push it to R2.
    ...
    uv run --no-sync python projects/sparse-attn-emergence/scripts/publish_report.py --status

    # fast local iteration on one section — compiles nothing else, uploads nothing
    uv run --no-sync python projects/sparse-attn-emergence/scripts/publish_report.py --section 04-content

    # whole report to a local PDF, still no upload
    uv run --no-sync python projects/sparse-attn-emergence/scripts/publish_report.py --preview

    # compile and push; prints the shareable URL
    uv run --no-sync python projects/sparse-attn-emergence/scripts/publish_report.py

Run `gen_report.py` first if results have changed — this script compiles what is on disk
and does not regenerate figures.
"""
...
from shared_lib.typst_report import (                      # noqa: E402
    changed_since_publish, preview, preview_section, publish_report, read_build_state,
)

REPORT = PROJECT / "report"
NAME = "sparse_attn_emergence_report"
TITLE = "What kind of pattern is hard to learn?"


def main() -> None:
    ap = argparse.ArgumentParser(description="Compile and publish the sparse-attn-emergence report")
