...
Whether the $C(S, s)$ difficulty law has an analogue for content-keyed patterns. On the linear
map, difficulty tracks the number of candidate position-subsets. For an induction circuit the
candidate space is a set of matching rules, and nothing measured here shows the two scale alike.
A content-keyed task with a tunable "sparsity" — say, recall where the key must match on $k$ of
$m$ attributes — would put both families on one axis.
