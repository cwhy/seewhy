# Experiment Workflow — Sparse-Attention Emergence

Small-scale replication of arXiv:2606.25010 — testing whether transformer capabilities
emerge abruptly and at seed-random times because a *sparse attention pattern* is hard to
find, on synthetic tasks where the correct pattern is known by construction.
...
├── proposal.md             # Claims under test (H1–H5), staged plan, deviations
├── workflow.md             # This file
├── concepts.md             # Task math, model, metric definitions, findings
...
├── params_expN.pkl         # Saved params, leading seed axis (gitignored)
├── history_expN.pkl        # Full-resolution per-step curves (gitignored)
├── .gitignore              # logs/, *.pkl, *.npy, *.json — results.jsonl exempted
├── logs/                   # Experiment and runner logs (gitignored)
├── lib/
│   ├── __init__.py
│   ├── tasks.py            # Linear map (and later CA) data generators
│   ├── models.py           # Flat-dict causal transformer, vmap-friendly
│   └── viz.py              # Seed curves, t* histogram, mechanism panel
└── scripts/
...
1. **Always `uv run --no-sync python`** — never raw `python` or `python3`, and never
   a bare `uv run`. The `.venv` has CUDA-specific packages; raw python may resolve a
   different interpreter, and a syncing `uv run` can silently upgrade packages and
   break GPU (see the root `CLAUDE.md`).
...
3. **Throwaway scripts go in `scripts/tmp/`** — never `python -c "..."` inline.
   ...
   `uv run python projects/sparse-attn-emergence/scripts/tmp/<name>.py`.
...
Experiments run on the remote GPU box `195.133.135.186` (2× RTX 4090), not the Mac —
...
Smoke-test any new experiment first with `SMOKE=1` (cuts steps and seeds, writes a
`*_smoke` row so the real run's skip-if-done guard stays clear).

All seeds of a config train **simultaneously** under one `jax.vmap` over a leading
param axis — `N_SEEDS=16` costs about as much as one seed at this model size, which is
what makes the seed-distribution claims (H1/H2) affordable.

Local invocations below are the generic runner interface (they work on the box):

```bash
# Fire and forget — detach to background, return immediately
uv run python projects/sparse-attn-emergence/scripts/run_experiments.py --bg exp1

# With auto-viz after completion
uv run python projects/sparse-attn-emergence/scripts/run_experiments.py --bg --viz scripts/gen_viz_exp1.py exp1

# Sequential blocking (safe — no XLA contention)
uv run python projects/sparse-attn-emergence/scripts/run_experiments.py exp1 exp2

# Parallel across GPUs
uv run python projects/sparse-attn-emergence/scripts/run_experiments.py --parallel exp1 exp2

# Pin to a specific GPU
uv run python projects/sparse-attn-emergence/scripts/run_experiments.py --gpu 1 exp1
```
...
tail -20 projects/sparse-attn-emergence/logs/exp1.log

# Poll until result appears in results.jsonl
uv run python projects/sparse-attn-emergence/scripts/poll_result.py exp1
uv run python projects/sparse-attn-emergence/scripts/poll_result.py --all
```
...
```python
with open("projects/sparse-attn-emergence/params_exp1.pkl", "rb") as f:
...
```bash
uv run python projects/sparse-attn-emergence/scripts/gen_viz_exp1.py
```
...
url = save_matplotlib_figure("sparse-attn-emergence_my_plot", fig, dpi=150)
plt.close(fig)
# In the report body:
...
url = save_report_file("sparse-attn-emergence_report_<name>", Path("projects/sparse-attn-emergence/report-<name>.md"))
print(url)
```
...
Run all with `uv run python projects/sparse-attn-emergence/scripts/tmp/<name>.py [args]`.
