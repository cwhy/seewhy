"""
Omniglot AR — exp9: the plateau-crossing recipe, 5-way real task.

exp8 at the original 5-way difficulty. See exp8's docstring for where the recipe
came from; the only change here is n_way 2 -> 5, with micro-batch traded against
accumulation to hold the effective batch at 64 while the token count doubles.

This is the headline claim of the project restated with enough optimisation
signal to cross the plateau: 5-way 1-shot in-context classification of Omniglot
characters from a class-disjoint split.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp9
"""
...
MICRO_BATCH, ACCUM = 16, 4          # effective batch = 64 episodes
LR, SEED = 1e-3, 0
