"""
Omniglot AR — exp5: exp3 plus an ink-biased observation pool.

exp3 fixes label ROUTING; this fixes the signal-to-noise of what gets routed.

Ink covers 18.7% of a drawing, so a uniform 196-pixel pool spends ~160 tokens on
background. A background token matches nearly every support drawing, so its
contribution to a content-matching vote is close to uniform noise — the query's
label token pools ~36 informative votes against ~160 uninformative ones. Even a
perfectly-learned matching circuit has to overcome that ratio.

`ink_pool=True` draws the observed positions from where the SUPPORT drawings
have ink (topped up at random if there are too few). Support only: using the
queries' ink would let the pool itself carry information about the very
drawings being classified.

The pixel 1-NN baseline is recomputed on the same pool, so it remains a floor
for this run rather than for exp1's easier-to-beat uniform one; expect it to
move, and compare against the number this run reports, not exp3's.

Chain: exp1 (neither) -> exp3 (label field) -> exp5 (also ink pool).

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp5
"""
...
SPEC = Spec(n_way=5, k_shot=1, n_query=1, n_ctx=196, img_size=28, n_bins=8, v_refs=64,
            label_field=True, ink_pool=True)
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"label-as-field + ink pool: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"n_bins={SPEC.n_bins} (exp3 + ink-biased positions)",
        "time_s": elapsed,
        "n_params": n_params(p),
