"""
Omniglot AR — exp10: coarse images + the plateau-crossing recipe, real task.

Where the evidence stands: exact matching (identity queries) crosses a phase
transition and solves completely; approximate matching — different drawings of
the same character — does not, at 2-way with 25000 steps (exp8, 0.488).

Those are different problems. Exact matching needs an equality test on values;
approximate matching needs a similarity metric that survives handwriting
variation. The hypothesis here is that resolution controls the distance between
them: at 10x10, two drawings of a character differ in far fewer pixels than at
28x28, so approximate matching moves toward the exact matching that is known to
be learnable. The 1-NN floor moves the same way and says so — 0.606 at 10x10
fully observed versus 0.431 at 28x28 with 196 of 784 pixels.

exp6 already ran 10x10 and sat at chance, but under the OLD recipe (batch 16,
lr 3e-4) that we now know cannot cross a plateau. This is exp6's task with
exp8's optimiser settings; neither alone was enough.

All 100 positions are observed, so `ink_pool` is off — there is no pool to bias
when everything is in it.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 0 exp10
"""
...
# ── hyperparameters ──────────────────────────────────────────────────────────
SPEC = Spec(n_way=2, k_shot=1, n_query=1, n_ctx=100, img_size=10, n_bins=8, v_refs=64,
            label_field=True, ink_pool=False)
D_MODEL, N_LAYERS, HEAD_DIM = 192, 3, 32
MICRO_BATCH, ACCUM = 32, 2          # effective batch = 64 episodes
LR, SEED = 1e-3, 0
NUM_STEPS, EVAL_EVERY = 25000, 500
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"coarse + plateau recipe: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"{SPEC.n_way}-way {SPEC.img_size}x{SPEC.img_size} full, batch 64, lr 1e-3",
        "time_s": elapsed,
        "n_params": n_params(p),
