"""
Omniglot AR — exp7: POSITIVE CONTROL. Each query drawing IS its support drawing.

Everything so far is a negative result, and a chain of negative results is only
worth reading if the apparatus is known to work. This run makes the task
trivially solvable and checks that the model solves it.

`identity_query=True` makes each query drawing the very same image as its
class's support drawing. Matching is then exact: at every observed position the
query's value equals the support's value, so the vote the exp3 circuit computes
is unambiguous. Nothing else changes from exp3.

Two possible outcomes, and both are informative:

  * **Solves it (accuracy -> 1.0).** The circuit described in the analysis IS
    learnable by this architecture at this budget, and the failure on the real
    task is about the difficulty of the match, not about a broken pipeline, a
    mis-specified target, or an unreachable gradient.
  * **Fails.** Then the apparatus is broken somewhere and every negative result
    in this project is uninterpretable until it is found. This outcome would
    invalidate the analysis section, not confirm it.

Note this is NOT a generalisation test — an identity query leaks its own answer
by construction, and unseen-character accuracy here is not a claim about
anything. The number to read is whether it can be fit at all.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp7
"""
...
# ── hyperparameters ──────────────────────────────────────────────────────────
SPEC = Spec(n_way=5, k_shot=1, n_query=1, n_ctx=196, img_size=28, n_bins=8, v_refs=64,
            label_field=True, identity_query=True)
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"POSITIVE CONTROL (identity query): {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"query image == support image (exp3 + identity)",
        "time_s": elapsed,
        "n_params": n_params(p),
