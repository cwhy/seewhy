"""
Omniglot AR — exp2: the difficulty floor. 2-way 1-shot, half the image observed.

A diagnostic, not a claim. exp1 sat at chance; this run exists to separate two
very different explanations for that:

  * **difficulty** — the circuit is learnable but 5 classes over 196 sparse
    pixels is too hard a first target, or needs a longer budget; or
  * **structural** — the token-level formulation cannot learn cross-drawing
    content matching at all, in which case making the task easier changes
    nothing.

So everything that can be made easier is: 2 classes instead of 5 (chance 0.500),
and 392 of 784 pixels observed instead of 196, which roughly doubles the ink
each drawing exposes. The architecture, optimiser, and budget are untouched.

If exp2 also lands on chance, the finding is about the formulation and exp3-5
should be re-planned around that. If exp2 clears its floor, exp1 was a
difficulty ceiling and the sweep in proposal.md is the right next step.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp2
"""
...
# ── hyperparameters ──────────────────────────────────────────────────────────
SPEC = Spec(n_way=2, k_shot=1, n_query=1, n_ctx=392, img_size=28, n_bins=8, v_refs=64)
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"difficulty floor: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"{SPEC.n_ctx}/{SPEC.img_size**2} px observed",
        "time_s": elapsed,
        "n_params": n_params(p),
