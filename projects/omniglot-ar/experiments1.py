"""
Omniglot AR — exp1: 5-way 1-shot in-context classification, token-level.

Hypothesis (proposal.md): the token-level attention model that sat at chance on
anonymised MNIST will clear chance on Omniglot, because the class-disjoint
background/evaluation split makes memorisation impossible by construction rather
than by an anonymisation trick, and 20 drawings per character remove the
pressure to memorise a prototype in the first place.

What changed from universal-ar exp39: the architecture is deliberately
IDENTICAL (same depth, width, head size, embedding scheme). Only the data and
the episode protocol change, so a difference in outcome is attributable to the
substrate.

Read it against three numbers, all reported here:
  * chance                  = 1/N_WAY = 0.200
  * pixel 1-NN on the same observed pixels — the "did it learn anything" floor
  * accuracy on BACKGROUND characters (seen in training) — the gap against
    evaluation characters is the memorisation gap

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp1
"""
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"{SPEC.n_way}-way {SPEC.k_shot}-shot in-context on Omniglot, "
                "train=background / test=evaluation (disjoint characters)",
        "time_s": elapsed,
        "n_params": n_params(p),
