"""
Experiment runner for Omniglot AR.

Runs experiments sequentially (default) or in parallel across GPUs.
Logs each experiment to projects/omniglot-ar/logs/{name}.log and prints a
...
Usage:
    ...
    uv run python projects/omniglot-ar/scripts/run_experiments.py --viz scripts/gen_viz_exp1.py exp1
"""
...
def print_results_table(names: list[str]):
    ...
    # The four numbers that decide whether a run means anything: accuracy on
    # UNSEEN characters, against the two floors it has to clear (chance, pixel
    # 1-NN), and accuracy on SEEN characters — seen minus unseen is the
    # memorisation gap. See concepts.md.
    header = (f"{'exp':<7} {'time':>6} {'unseen':>7} {'1-NN':>6} {'chance':>7} "
              f"{'seen':>6}  name")
    print("\n" + header)
    print("-" * 96)
    for r in rows:
        ...
        print(f"{r.get('experiment', '?'):<7} {t_str:>6} {num('acc_ev'):>7} "
              f"{num('nn_ev'):>6} {num('chance'):>7} {num('acc_bg'):>6}  "
              f"{r.get('name', '')[:44]}")
...
def main():
    parser = argparse.ArgumentParser(description="Run Omniglot AR experiments")
