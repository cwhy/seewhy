"""
Omniglot AR — exp8: the plateau-crossing recipe, 2-way real task.

exp1-exp7 all sat at chance. A diagnostic (scripts/tmp/match.py) then showed the
missing capability — content-dependent matching — DOES emerge, after a plateau:

    identity/ink/196  step 1000  loss 0.6934  acc 0.492
                      step 2000  loss 0.6930  acc 0.461   <- flat at ln 2
                      step 3000  loss 0.1033  acc 0.961   <- breaks through
                      step 5000  loss 0.0007  acc 1.000

What differed from exp7 was not the step budget (exp7 ran 12000) but the
resources for crossing the plateau: effective batch 64 rather than 16, lr 1e-3
rather than 3e-4, and an ink-biased pool. Crossing a plateau is a
signal-to-noise problem, and batch and learning rate are what set it.

This applies that recipe to the REAL task — query drawings are different
drawings of the same character, not copies — at 2-way, with 25000 steps to
leave room for a later transition than the identity task needed.

Read against the 1-NN floor this run reports (~0.83 under the ink pool), not
against exp1-7's.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 0 exp8
"""
...
# ── hyperparameters ──────────────────────────────────────────────────────────
SPEC = Spec(n_way=2, k_shot=1, n_query=1, n_ctx=196, img_size=28, n_bins=8, v_refs=64,
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"plateau-crossing recipe: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"{SPEC.n_way}-way, batch 64, lr 1e-3, 25k steps",
        "time_s": elapsed,
        "n_params": n_params(p),
