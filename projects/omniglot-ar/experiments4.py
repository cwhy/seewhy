"""
Omniglot AR — exp4: exp3 plus binarised pixel values.

exp3 tests the label field; this adds the second suspected obstacle on top.
Ink covers ~19% of a drawing, so with 8 intensity bins most tokens carry bin 0
and any position-wise agreement signal is dominated by two drawings agreeing
about blank background. Two bins (`n_bins=2`) makes "agrees on ink" and "agrees
on background" the only two outcomes, which is the sharpest form of the match
the circuit in exp3 has to compute.

Read as exp1 -> exp3 (add label field) -> exp4 (also binarise): each step adds
exactly one change, so whichever one moves the number is identified.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 0 exp4
"""
...
# ── hyperparameters ──────────────────────────────────────────────────────────
SPEC = Spec(n_way=5, k_shot=1, n_query=1, n_ctx=196, img_size=28, n_bins=2, v_refs=64,
            label_field=True)
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"label-as-field + binarised: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"n_bins={SPEC.n_bins} (exp3 + binarised values)",
        "time_s": elapsed,
        "n_params": n_params(p),
