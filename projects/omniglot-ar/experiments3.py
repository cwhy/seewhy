"""
Omniglot AR — exp3: the fix. exp1 with the class carried on every support token.

exp1 and exp2 sat at chance because the label lived ONLY on the label token at
`pos_label`. That layout forces a three-hop circuit — gather my drawing's pixels
by `ref`, match them position-wise against other drawings, recover WHICH `ref`
matched, then read that ref's label token — and hop three cannot work: softmax
attention averages over the ~n_draw tokens sharing a position, destroying the
identity of the drawing that agreed in exactly the step that needs it.

Here `label_field=True` puts the class on every token of a support drawing. A
query pixel attends to support pixels at its own position, weights them by value
agreement, and reads a LABEL embedding directly — the vote survives averaging
because what is averaged is the label, not the drawing identity. The query's
label token then pools those votes over its own `ref`. Two hops, and the
impossible one is gone.

This is the token-bag analogue of induction heads in sequence models, where `y`
sits adjacent to its `x`. Everything else — depth, width, head size, optimiser,
budget, episode shape — is IDENTICAL to exp1, so exp1 is the control and the
label field is the only variable.

Usage:
    uv run python projects/omniglot-ar/scripts/run_experiments.py --bg --gpu 1 exp3
"""
...
if __name__ == "__main__":
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"label-as-field: {SPEC.n_way}-way {SPEC.k_shot}-shot, "
                f"n_bins={SPEC.n_bins} (exp1 + label field)",
        "time_s": elapsed,
        "n_params": n_params(p),
