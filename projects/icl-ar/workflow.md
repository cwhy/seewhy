...
3. **Throwaway scripts go in `scripts/tmp/`** — never `python -c "..."` inline.
   ...
   `uv run python projects/icl-ar/scripts/tmp/<name>.py`.
...
uv run python projects/icl-ar/scripts/run_experiments.py --bg exp1

# With auto-viz after completion
uv run python projects/icl-ar/scripts/run_experiments.py --bg --viz scripts/gen_viz_exp1.py exp1

# Sequential blocking (safe — no XLA contention)
uv run python projects/icl-ar/scripts/run_experiments.py exp1 exp2

# Parallel across GPUs
uv run python projects/icl-ar/scripts/run_experiments.py --parallel exp1 exp2

# Pin to a specific GPU
uv run python projects/icl-ar/scripts/run_experiments.py --gpu 1 exp1
```

Logs always go to `projects/icl-ar/logs/{exp_name}.log`.
...
tail -20 projects/icl-ar/logs/exp1.log

# Poll until result appears in results.jsonl
uv run python projects/icl-ar/scripts/poll_result.py exp1
uv run python projects/icl-ar/scripts/poll_result.py --all
```
...
```python
with open("projects/icl-ar/params_exp1.pkl", "rb") as f:
...
```bash
uv run python projects/icl-ar/scripts/gen_viz_exp1.py
```
...
url = save_matplotlib_figure("icl-ar_my_plot", fig, dpi=150)
plt.close(fig)
# In the report body:
...
url = save_report_file("icl-ar_report_<name>", Path("projects/icl-ar/report-<name>.md"))
print(url)
```
...
Run all with `uv run python projects/icl-ar/scripts/tmp/<name>.py [args]`.
