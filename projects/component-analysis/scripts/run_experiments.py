"""
Experiment runner for component-analysis.

Runs experiments sequentially (default) or in parallel across GPUs.
Logs each experiment to projects/component-analysis/logs/{name}.log and
prints a results table when done.

Usage:
    # Run sequentially (blocks until done)
    uv run python projects/component-analysis/scripts/run_experiments.py exp1 exp2
    ...
    uv run python projects/component-analysis/scripts/run_experiments.py --gpu 0 exp1
...
def print_results_table(names: list[str]):
    ...
    header = f"{'exp':<8} {'time':>7}  summary"
    print("\n" + header)
    print("-" * 60)
    for r in rows:
        t = r.get("time_s", float("nan"))
        t_str = f"{t:.0f}s" if t == t else "—"
        # Build a short summary from whatever keys are present
        summary_parts = []
        if "global_evr_by_k" in r:
            evr = r["global_evr_by_k"]
            k10 = evr.get("10", "?")
            summary_parts.append(f"EVR@10={k10:.3f}" if isinstance(k10, float) else f"EVR@10={k10}")
        if "mean_pairwise_corr" in r:
            corrs = r["mean_pairwise_corr"]
            if corrs:
                summary_parts.append(f"corr[0]={corrs[0]:.3f}")
        summary_parts.append(r.get("name", ""))
        print(f"{r.get('experiment', '?'):<8} {t_str:>7}  {'  '.join(summary_parts)}")
...
def main():
    parser = argparse.ArgumentParser(description="Run component-analysis experiments")
    ...
    parser.add_argument("--bg", action="store_true",
                        help="Detach to background; print PID and log path, then exit")
