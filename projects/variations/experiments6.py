"""
exp6 — Variations: dedicated recon_emb + 4×8 categorical embedding + per-rank recall.

Changes from exp5:
  1. Dedicated reconstruction embedding — recon_emb: (COND_DIM,) learnable param;
     cat=(0,0) slot no longer reserved, all categories free for variation.
  2. 4×8 categorical structure — N_CAT=4, VOCAB_SIZE=8, E_DIM=4, COND_DIM=16;
     same 128 embedding params (4×8×4 = 128), 8^4 = 4096 total slots.
  3. Per-rank recall — _recall_batch_jit returns (mean, per_rank curve);
     eval logged as eval_recall_by_rank in history.

Usage:
    uv run python projects/variations/experiments6.py
"""
