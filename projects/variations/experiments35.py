"""
exp35 — Warm-start from exp33, 200 more epochs (LR cycle 2).

exp33 (LATENT=2048, N_VAR=128, B=64, 200ep): recon_mse=0.000983, recall_mse=0.007269.
Recall trajectory flattened at ep199 (−0.00010 per 50ep) — LR near minimum.

Goal: test whether exp33 is training-limited or capacity-limited.
Load exp33 params, reset optimizer state, restart cosine LR schedule.
If recall improves → training-limited (more steps help).
If stagnates → capacity-limited (need arch changes like N_VAR=256 in exp34).

Changes from exp33:
- Load params from params_exp33.pkl instead of fresh random init
- Fresh optimizer state (reset momentum)
- Fresh cosine LR schedule (3e-4 restart over 200ep)

Architecture identical to exp33 (LATENT=2048, N_VAR=128, B=64, CHUNK_SIZE=390).
Epoch time: ~148s (identical to exp33), total ~8.2h.
Expected recall_mse: 0.0065–0.0072 if training-limited, ~0.0072 if capacity-limited.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments35.py
"""
...
ALPHA          = 1.0
BATCH_SIZE     = 64    # identical to exp33
N_VAR_SAMPLES  = 128   # identical to exp33
MAX_EPOCHS     = 200
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16
CHUNK_SIZE     = 390   # chunked scan: 781//390=2 Python dispatches/epoch
...
def make_chunk_fn(optimizer):
    ...
    # 781 batches/epoch → 2 Python dispatches (drops last 1 batch).
...
def train(params, X_tr, X_te, knn_idx):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        n    = n_full_chunks * CHUNK_SIZE * BATCH_SIZE  # 2*390*64 = 49920
        ...
        for c in range(n_full_chunks):  # 2 Python dispatches
...
if __name__ == "__main__":
    ...
    # Warm-start: load exp33 params, reset optimizer state
    params_path = Path(__file__).parent / "params_exp33.pkl"
    logging.info(f"Loading warm-start params from {params_path}")
    with open(params_path, "rb") as f:
        params = {k: jnp.array(v) for k, v in pickle.load(f).items()}
    logging.info(
        f"Warm-start params loaded: {n_params(params):,}  "
        ...
        f"  dataset=cifar10  MAX_EPOCHS={MAX_EPOCHS}  warm_start=exp33"
    )
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=2048 wide-deconvnet N_VAR=128 K_NN=16 CIFAR-10 200ep warm-start-exp33",
