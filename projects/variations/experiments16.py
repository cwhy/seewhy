"""
exp16 — CNN encoder + deconvnet decoder on CIFAR-10, B=128, N_VAR=16.

Replaces exp15's MLP encoder with a CNN encoder:
  (B, 3072) → reshape (B, 32, 32, 3) NHWC
            → Conv(3→64,  3×3, stride=2, SAME) + GELU → (B, 16, 16, 64)
            → Conv(64→128, 3×3, stride=2, SAME) + GELU → (B, 8, 8, 128)
            → Conv(128→256, 3×3, stride=2, SAME) + GELU → (B, 4, 4, 256)
            → flatten → (B, 4096)
            → Linear(4096→512) + GELU
            → Linear(512→128) → (B, LATENT_DIM)

Decoder identical to exp15: 3-stage deconvnet (4×4→8×8→16×16→32×32, 3 RGB channels).
kNN cache shared with exp15: knn_cifar_k64.npy.

Usage:
    uv run python projects/variations/experiments16.py
"""
...
# Decoder architecture (same as exp15)
DC_STEM        = 512
DC_C1          = 128
DC_C2          = 64
DC_C3          = 32
...
def encode(params, x_flat):
    ...
    h = jax.nn.gelu(_conv_stride2(h, params["enc_c1"], params["enc_b1"]))  # (B,16,16,64)
    h = jax.nn.gelu(_conv_stride2(h, params["enc_c2"], params["enc_b2"]))  # (B,8,8,128)
    h = jax.nn.gelu(_conv_stride2(h, params["enc_c3"], params["enc_b3"]))  # (B,4,4,256)
    h = h.reshape(B, -1)                                                # (B, 4096)
    h = jax.nn.gelu(h @ params["enc_W1"] + params["enc_b4"])           # (B, 512)
    return h @ params["enc_W2"] + params["enc_b5"]                      # (B, 128)

# ── Decoder (deconvnet, same as exp15) ────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"CNN encoder + deconvnet decoder CIFAR-10  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} N_VAR={N_VAR_SAMPLES} K_NN={K_NN} B={BATCH_SIZE}",
