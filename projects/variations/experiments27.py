"""
exp27 — LATENT_DIM=1024: test encoder bottleneck hypothesis.

exp23 (LATENT=512) achieved recon_mse=0.00219. Each LATENT doubling (64→128→512)
halved MSE. LATENT=1024 (3× compression) may push below 0.0020.

Changes from exp23:
- LATENT_DIM: 512 → 1024
- ENC_FC1:    512 → 1024  (avoid compress-then-expand: 4096→512→1024)
- IN_DIM auto: 1024 + 16 = 1040

Everything else identical: ALPHA=1.0, N_VAR=64, K_NN=16, B=128, 100 epochs.

Usage:
    uv run python projects/variations/experiments27.py
"""
...
LATENT_DIM     = 1024   # was 512 — primary change
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
...
ENC_C2         = 128
ENC_C3         = 256
ENC_FC1        = 1024  # was 512 — match LATENT_DIM to avoid compress-then-expand
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=1024 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10",
