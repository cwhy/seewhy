"""
exp24 — LATENT_DIM=512 + intra-class kNN.

Pixel-space kNN can mix classes (e.g. a green "frog" is pixel-similar to
a green "car"). Restricting kNN to within-class images gives semantically
consistent variation targets, which should improve recall quality.

Precomputes knn_cifar_intraclass_k64.npy the first time it runs.

Base: exp23 (LATENT=512, wide-deconvnet, N_VAR=64, K_NN=16, ALPHA=1.0).

Usage:
    uv run python projects/variations/experiments24.py
"""
...
def compute_knn_intraclass(X, y, k=64, chunk=1000):
    """Compute kNN within each CIFAR-10 class; returns global training indices."""
    N = len(X)
    knn_idx = np.zeros((N, k), dtype=np.int32)
    for cls in range(N_CLASSES):
        class_global_idx = np.where(y == cls)[0]   # (5000,)
        X_cls = X[class_global_idx]                  # (5000, IMG_FLAT)
        N_cls = len(X_cls)
        sq = (X_cls ** 2).sum(1)
        XT = X_cls.T
        local_knn = np.zeros((N_cls, k), dtype=np.int32)
        for start in range(0, N_cls, chunk):
            end = min(start + chunk, N_cls)
            dists = np.array(sq[start:end, None] + sq[None, :] - 2.0 * (X_cls[start:end] @ XT))
            for j in range(end - start):
                dists[j, start + j] = np.inf
            local_knn[start:end] = np.argpartition(dists, k, axis=1)[:, :k].astype(np.int32)
            if (start / chunk) % 5 == 0:
                logging.info(f"  intraclass kNN class={cls}  {end}/{N_cls}")
        knn_idx[class_global_idx] = class_global_idx[local_knn]
    return knn_idx
...
if __name__ == "__main__":
    ...
    Y_train = np.array(data.y)   # (50000,) class labels

    KNN_IC_CACHE = Path(__file__).parent / "knn_cifar_intraclass_k64.npy"
    if KNN_IC_CACHE.exists():
        knn_idx_full = np.load(KNN_IC_CACHE)
        logging.info(f"Intra-class kNN loaded from cache: {KNN_IC_CACHE}")
    else:
        logging.info("Computing intra-class kNN (first time, ~2 min)…")
        knn_idx_full = compute_knn_intraclass(X_train, Y_train, k=64)
        np.save(KNN_IC_CACHE, knn_idx_full)
        logging.info(f"Intra-class kNN saved to {KNN_IC_CACHE}")
    knn_idx = knn_idx_full[:, :K_NN]
    logging.info(f"kNN ready (intra-class, sliced to k={K_NN}): {knn_idx.shape}")
    ...
    logging.info(
        ...
        f"  coverage_ratio={N_VAR_SAMPLES/K_NN:.1f}x  knn=intra-class"
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=512 intra-class-kNN N_VAR=64 K_NN=16 CIFAR-10",
        ...
        "batch_size": BATCH_SIZE, "encoder": "cnn", "decoder": "wide-deconvnet",
        "knn_type": "intra-class", "dataset": "cifar10", "img_flat": IMG_FLAT,
    })
