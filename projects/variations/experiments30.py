"""
exp30 — LATENT=1024 + 200 epochs: combine exp27 and exp29 improvements.

exp27 (LATENT=1024, 100ep): recon_mse=0.00173 (−21% vs exp23)
exp29 (LATENT=512, 200ep):  recon_mse=0.00165 (−25% vs exp23)
Both are independent improvements — combined should push below 0.0015.

Changes from exp23:
- LATENT_DIM: 512 → 1024
- ENC_FC1:    512 → 1024  (avoid compress-then-expand)
- MAX_EPOCHS: 100 → 200

IN_DIM auto: 1024 + 16 = 1040.
Cosine schedule auto-scales: num_steps = 200 * num_batches = 78,000.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments30.py
"""
...
LATENT_DIM     = 1024   # was 512 (exp27 change)
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
...
BATCH_SIZE     = 128
N_VAR_SAMPLES  = 64
MAX_EPOCHS     = 200   # was 100 (exp29 change)
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16
...
ENC_C2         = 128
ENC_C3         = 256
ENC_FC1        = 1024  # was 512 — match LATENT_DIM
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=1024 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10 200ep",
