"""
exp23 — LATENT_DIM=512: break the reconstruction plateau.

All 8 prior experiments (exp15–exp22) have recon MSE stuck at 0.0046–0.0049
regardless of decoder width, architecture, or IMLE params. Root cause:
LATENT_DIM=128 compresses 3072-d CIFAR images 24×, losing texture detail
before the decoder ever sees it.

Fix: LATENT_DIM=512 (6× compression). All other hyperparams from exp20
(N_VAR=64, K_NN=16, wide-deconvnet, discrete style, ALPHA=1.0).

Usage:
    uv run python projects/variations/experiments23.py
"""
...
LATENT_DIM     = 512    # was 128 — primary change
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
...
# CNN encoder channels (same as exp16/17/20)
ENC_C1         = 64
ENC_C2         = 128
ENC_C3         = 256
ENC_FC1        = 512

# Wide decoder channels (same as exp17/20)
DC_STEM        = 1024
DC_C1          = 256
DC_C2          = 128
DC_C3          = 64
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=512 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10",
