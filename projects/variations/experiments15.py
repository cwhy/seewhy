"""
exp15 — deconvnet decoder on CIFAR-10, B=128, N_VAR=16.

Adapts exp14 (MNIST deconvnet) to CIFAR-10:
  - 32×32 RGB images (3072-d flat vectors)
  - Deeper encoder: 3072 → 1024 → 512 → 128 (LATENT_DIM=128)
  - Wider decoder: stem=512, C1=128, C2=64, C3=32 (3 ConvTranspose stages)
  - 3 upsampling stages: 4×4 → 8×8 → 16×16 → 32×32
  - 3 output channels (RGB) → 3072

Usage:
    uv run python projects/variations/experiments15.py
"""
...
DC_STEM        = 512    # MLP hidden before reshape
DC_C1          = 128   # channels after reshape  (4×4 spatial)
DC_C2          = 64    # channels after first ConvTranspose (8×8 spatial)
DC_C3          = 32    # channels after second ConvTranspose (16×16 spatial)
# → ConvTranspose → (B, 3, 32, 32)
...
def init_params(key_gen):
    ...
    return {
        ...
        # Encoder: 3072 → 1024 → 512 → LATENT_DIM
        "enc_W1": n((IMG_FLAT, 1024),        scale=IMG_FLAT**-0.5),
        "enc_b1": jnp.zeros(1024),
        "enc_W2": n((1024, 512),             scale=1024**-0.5),
        "enc_b2": jnp.zeros(512),
        "enc_W3": n((512, LATENT_DIM),       scale=512**-0.5),
        "enc_b3": jnp.zeros(LATENT_DIM),
        # Decoder MLP stem: IN_DIM → DC_STEM → DC_C1*4*4
        ...
        # ct1: (DC_C1, 4, 4) → (DC_C2, 8, 8)     kernel 4×4
        "dc_ct1": n((DC_C1, DC_C2, 4, 4), scale=(DC_C1 * 4 * 4)**-0.5),
        # ct2: (DC_C2, 8, 8) → (DC_C3, 16, 16)   kernel 4×4
        "dc_ct2": n((DC_C2, DC_C3, 4, 4), scale=(DC_C2 * 4 * 4)**-0.5),
        # ct3: (DC_C3, 16, 16) → (3, 32, 32)      kernel 4×4
...
def encode(params, x):
    """x (B, 3072) → (B, 128)"""
    h = jax.nn.gelu(x @ params["enc_W1"] + params["enc_b1"])   # (B, 1024)
    h = jax.nn.gelu(h @ params["enc_W2"] + params["enc_b2"])   # (B, 512)
    return h @ params["enc_W3"] + params["enc_b3"]              # (B, 128)
...
def decode(params, zc):
    ...
    MLP stem projects to (B, DC_C1, 4, 4), then three stride-2 ConvTranspose
    layers upsample: 4×4 → 8×8 → 16×16 → 32×32 (3 RGB channels).
    """
    B = zc.shape[0]
    # MLP stem
    h = jax.nn.gelu(zc @ params["dc_W1"] + params["dc_b1"])   # (B, DC_STEM)
    h = jax.nn.gelu(h  @ params["dc_W2"] + params["dc_b2"])   # (B, DC_C1*16)
    h = h.reshape(B, DC_C1, 4, 4)                              # (B, DC_C1, 4, 4)
    # ConvTranspose 4 → 8
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"  decoder=deconvnet(C1={DC_C1},C2={DC_C2},C3={DC_C3},stem={DC_STEM})"
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"deconvnet decoder CIFAR-10  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} N_VAR={N_VAR_SAMPLES} K_NN={K_NN} B={BATCH_SIZE} C1={DC_C1} C2={DC_C2} C3={DC_C3} stem={DC_STEM}",
