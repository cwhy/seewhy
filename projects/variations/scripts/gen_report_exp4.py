"""
Generate and upload a report for variations exp4.

Produces:
  - Learning curves: total loss, L_recon, L_var
  - Reconstruction grid (test originals vs c=0^4 outputs)
  - 10 separate variation mosaics (one per digit, 16×16 = 256 sampled Gaussian vectors)
    with a kNN neighbor strip above each mosaic
  - Local report: projects/variations/report_exp4.md  (relative paths, works offline)
  - HTML report uploaded to R2

Usage:
    uv run python projects/variations/scripts/gen_report_exp4.py
"""
...
from experiments4 import encode, decode, LATENT_DIM, N_GAUSS
...
ax.set_xlabel("epoch"); ax.set_ylabel("MSE"); ax.set_title("Eval reconstruction MSE (c=0⁴)")
...
zeros_g  = jnp.zeros((len(indices), N_GAUSS))
recon    = np.array(decode(params, jnp.concatenate([z, zeros_g], axis=-1)))
...
fig.suptitle(f"variations {EXP} — reconstructions (c=0⁴)", fontsize=11)
...
# Sample 256 fixed Gaussian vectors with seed=0 for a reproducible 16×16 mosaic

GRID_SIDE   = 16
N_MOSAIC    = GRID_SIDE * GRID_SIDE   # 256
rng         = np.random.RandomState(0)
c_fixed     = rng.randn(N_MOSAIC, N_GAUSS).astype(np.float32)
all_c       = jnp.array(c_fixed)      # (256, N_GAUSS) — fixed Gaussian samples

digit_locs, digit_urls = {}, {}
for digit in range(10):
    # Use training image so knn_idx applies directly
    train_idx = int(np.where(y_train == digit)[0][0])
    x_source  = X_train[train_idx]                         # (784,) float32
    knn_imgs  = X_train[knn_idx[train_idx][:10]]           # (10, 784)

    # Variation mosaic: 256 sampled Gaussian conditioning vectors
    x_d = jnp.array(x_source)[None]
    z_d = jnp.repeat(encode(params, x_d / 255.0), N_MOSAIC, axis=0)
    zc  = jnp.concatenate([z_d, all_c], axis=-1)
    ...
    # Reconstruction for source image at canonical conditioning (c=0^4)
    z_src    = encode(params, x_d / 255.0)
    zeros_g  = jnp.zeros((1, N_GAUSS))
    recon_src = np.array(decode(params, jnp.concatenate([z_src, zeros_g], axis=-1)))[0]
    ...
    fig.suptitle(f"digit {digit} — kNN (row 0) | closest decoded (row 1) | 256 Gaussian samples (row 2)",
                 fontsize=9, y=0.99)
...
md_local = f"""# variations — exp4 report
...
**Final reconstruction MSE (c=0⁴ on test set)**: `{fin_mse}`
