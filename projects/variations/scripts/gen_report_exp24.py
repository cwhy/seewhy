"""
Generate and upload a report for variations exp24 (CIFAR-10, LATENT=512 + intra-class kNN).

Usage:
    uv run python projects/variations/scripts/gen_report_exp24.py
"""
...
# Load intra-class kNN for display (fall back to global kNN for report plots)
knn_ic_path = PROJECT / "knn_cifar_intraclass_k64.npy"
knn_idx = np.load(knn_ic_path) if knn_ic_path.exists() else np.load(PROJECT / "knn_cifar_k64.npy")
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, LATENT={LATENT_DIM} intra-class kNN)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (LATENT={LATENT_DIM}, intra-class kNN)", fontsize=11)
...
for cls in range(10):
    ...
    for k in range(10):
        ...
        axk.axis("off")
        if k == 0: axk.set_title("intra-class kNN →", fontsize=6, pad=2, loc="left")
    ...
    fig.suptitle(
        f"class {cls} ({CIFAR_CLASSES[cls]}) — intra-class kNN | closest decoded | mosaic",
        fontsize=9, y=0.99
    )
...
md_local = f"""# variations — exp24 report (CIFAR-10, LATENT={LATENT_DIM} + intra-class kNN)

**Algorithm**: CNN encoder + wide deconvnet. LATENT={LATENT_DIM}, intra-class pixel-space kNN.
