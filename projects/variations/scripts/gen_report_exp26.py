"""
Generate and upload a report for variations exp26 (CIFAR-10, DeltaNet decoder).

Usage:
    uv run python projects/variations/scripts/gen_report_exp26.py
"""
...
from experiments26 import (
    encode, decode_ar, cats_to_cond,
    LATENT_DIM, N_CAT, VOCAB_SIZE, COND_DIM, K_NN,
    EVAL_COND_CATS, DM, DK, N_LAYERS, T_SEQ, IMG_FLAT,
    ENC_C1, ENC_C2, ENC_C3, ENC_FC1, N_VAR_SAMPLES, ALPHA,
)
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, DeltaNet, LATENT={LATENT_DIM})", fontsize=13)
plt.tight_layout()
...
recon = np.array(decode_ar(params, jnp.concatenate([z, recon_cond], axis=-1)))
...
fig.suptitle(f"variations {EXP} — reconstructions (DeltaNet, LATENT={LATENT_DIM}, CIFAR-10)", fontsize=11)
...
for cls in range(10):
    ...
    variations = np.array(decode_ar(params, jnp.concatenate([z_d_rep, cur_mosaic_conds], axis=-1)))
    ...
    eval_vars  = np.array(decode_ar(params, jnp.concatenate([z_d_rep256, cur_eval_conds], axis=-1)))
    ...
    recon_src = np.array(decode_ar(params, jnp.concatenate([z_d, recon_cond_src], axis=-1)))[0]
...
md_local = f"""# variations — exp26 report (CIFAR-10, DeltaNet Autoregressive Decoder)
...
as a product of conditionals. LATENT_DIM={LATENT_DIM} (same as exp23).
...
| Decoder | DeltaNet: DM={DM}, DK={DK}, N_LAYERS={N_LAYERS}, T_SEQ={T_SEQ} (4×4 patches) |
| Encoder | CNN: Conv({ENC_C1}/{ENC_C2}/{ENC_C3}, stride-2) + FC({ENC_FC1}) → {LATENT_DIM} |
