"""
Generate and upload a report for variations exp18 (CIFAR-10, ResNet encoder + 4-stage decoder).

Usage:
    uv run python projects/variations/scripts/gen_report_exp18.py
"""
...
from experiments18 import (
    encode, decode, cats_to_cond,
    LATENT_DIM, N_CAT, VOCAB_SIZE, COND_DIM, K_NN,
    EVAL_COND_CATS, DC_C0, DC_C1, DC_C2, DC_C3, DC_STEM, IMG_FLAT,
    ENC_C1, ENC_C2, ENC_C3, ENC_FC1,
)
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, ResNet enc + 4-stage dec)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (recon_emb, CIFAR-10 ResNet enc)", fontsize=11)
...
md_local = f"""# variations — exp18 report (CIFAR-10, ResNet encoder + 4-stage decoder)
...
- Decoder: 3-stage (4×4 start) → 4-stage (2×2 start), adds DC_C0={DC_C0} stage
...
| Encoder | ResNet: DownBlock({ENC_C1}) + ResBlock → DownBlock({ENC_C2}) + ResBlock → DownBlock({ENC_C3}) + ResBlock → FC({ENC_FC1}→{LATENT_DIM}) |
| N_CAT | {N_CAT} |
| VOCAB_SIZE | {VOCAB_SIZE} |
| COND_DIM | {COND_DIM} |
| DC_STEM / DC_C0 / DC_C1 / DC_C2 / DC_C3 | {DC_STEM} / {DC_C0} / {DC_C1} / {DC_C2} / {DC_C3} |
| K_NN | {K_NN} |
| BATCH_SIZE | 128 |
| N_VAR_SAMPLES | 16 |
| MAX_EPOCHS | 100 |
