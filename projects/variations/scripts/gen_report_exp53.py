"""
Generate and upload a full visualisation report for exp53
(additive z+eps conditioning: z_var = z + eps ~ N(0, I_2048), 300ep).

Usage:
    CUDA_VISIBLE_DEVICES=1 uv run python projects/variations/scripts/gen_report_exp53.py
"""
...
ax.set_xlabel("epoch"); ax.set_ylabel("MSE"); ax.set_title("Training loss (z + eps additive)")
ax.set_yscale("log")
...
ax.set_xlabel("neighbour rank"); ax.set_ylabel("recall MSE")
ax.set_title("Per-rank recall MSE\n(blue = early  →  red = final)")
ax.legend(fontsize=8); ax.grid(alpha=0.3)

fig.suptitle(f"Learning dynamics — {EXP} (z_var = z + eps, eps ~ N(0, I_2048), 300ep)", fontsize=13)
plt.tight_layout()
...
recon      = np.array(decode(params, z))   # eps=0 → z_var = z
...
fig.suptitle(f"{EXP} — eps=0 reconstructions on test set", fontsize=11)
...
GRID_SIDE = 16   # 16×16 = 256 using all EVAL_EPS vectors
...
for cls in range(10):
    ...
    knn_imgs   = np.array(X_tr_jax[knn_idx_g[0, 1:11]])

    x_d        = jnp.array(x_source)[None]
    z_d        = encode(params, x_d / 255.0)
    z_d_rep    = jnp.repeat(z_d, 256, axis=0)
    variations = np.array(decode(params, z_d_rep + EVAL_EPS))
    ...
    recon_src = np.array(decode(params, z_d))[0]   # eps=0
    ...
    fig.suptitle(
        f"{EXP} — class {cls} ({cls_name})  |  16×16 mosaic of 256 fixed N(0,I_2048) eps variations\n"
