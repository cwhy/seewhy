"""
Generate and upload a report for variations exp16 (CIFAR-10, CNN encoder).

Usage:
    uv run python projects/variations/scripts/gen_report_exp16.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, CNN encoder)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (recon_emb, CIFAR-10 CNN encoder)", fontsize=11)
...
md_local = f"""# variations — exp16 report (CIFAR-10, CNN encoder)
...
**Final reconstruction MSE**: `{fin_mse}` (vs exp15 MLP encoder: 0.00489)
