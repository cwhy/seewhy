"""
Generate and upload a report for variations exp8.

Produces:
  - Learning curves + per-rank recall (x=iter, y=MSE, legend=key ranks)
  - Reconstruction grid (test originals vs recon_emb outputs)
  - 10 variation mosaics (4×8 quadrant layout)
  - Local report: projects/variations/report_exp8.md
  - HTML report uploaded to R2

Usage:
    uv run python projects/variations/scripts/gen_report_exp8.py
"""
...
# Key ranks to highlight in the recall plot (1-indexed)
RECALL_RANKS = [1, 8, 16, 32, 48, 64]
...
num_batches = len(X_train) // 256   # approximate, for iter axis

# ── Plot 1: Learning curves + per-rank recall ──────────────────────────────────
...
ax = axes[0]
ax.plot(epochs, history["loss"], label="L_var", lw=1.5)
ax.set_xlabel("epoch"); ax.set_ylabel("MSE"); ax.set_title("Training loss (L_var only)")
...
if history.get("eval_recall_mse"):
    ...
    ax.plot(rec_ep, rec_mse, "s--", lw=1.5, label="recall MSE (mean)")
ax.set_xlabel("epoch"); ax.set_ylabel("MSE"); ax.set_title("Eval MSE")
ax.grid(alpha=0.3); ax.legend()

if has_rank:
    # Per-rank recall: x=iter, y=MSE, one line per key rank
    rank_data = history["eval_recall_by_rank"]   # [(epoch, [K_NN floats]), ...]
    eval_epochs = [ep for ep, _ in rank_data]
    eval_iters  = [ep * num_batches for ep in eval_epochs]
    curves      = np.array([curve for _, curve in rank_data])   # (n_evals, K_NN)

    ax = axes[2]
    colors = plt.cm.plasma(np.linspace(0.1, 0.9, len(RECALL_RANKS)))
    for i, rank in enumerate(RECALL_RANKS):
        ax.plot(eval_iters, curves[:, rank - 1], color=colors[i],
                lw=1.8, marker="o", markersize=3, label=f"rank {rank}")
    ax.set_xlabel("iter"); ax.set_ylabel("recall MSE")
    ax.set_title("Per-rank recall MSE over training")
...
# ── Plot 3: Variation mosaics (4×8 quadrant layout) ───────────────────────────
...
for digit in range(10):
    ...
    # Closest decoded among EVAL_COND_CATS to each kNN
    ...
    eval_vars  = np.array(decode(params, zc_eval))
    ...
    fig.suptitle(
        f"digit {digit} — kNN (row 0) | closest decoded (row 1) | 4×8 mosaic (row 2)",
        fontsize=9, y=0.99
    )
...
md_local = f"""# variations — exp8 report
