"""
Generate and upload a report for variations exp25 (CIFAR-10, LATENT=512 + ALPHA=3.0).

Usage:
    uv run python projects/variations/scripts/gen_report_exp25.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, LATENT={LATENT_DIM} ALPHA={ALPHA})", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (LATENT={LATENT_DIM}, ALPHA={ALPHA})", fontsize=11)
...
md_local = f"""# variations — exp25 report (CIFAR-10, LATENT={LATENT_DIM} + ALPHA={ALPHA})

**Algorithm**: CNN encoder + wide deconvnet. LATENT={LATENT_DIM}, stronger variation loss ALPHA={ALPHA}.

Hypothesis: with 512-d latent, increasing variation loss weight from 1.0 to {ALPHA} forces
