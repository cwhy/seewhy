"""
Generate and upload a report for variations exp20 (CIFAR-10, N_VAR=64, K_NN=16).

Usage:
    uv run python projects/variations/scripts/gen_report_exp20.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, N_VAR=64, K_NN=16)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (recon_emb, CIFAR-10)", fontsize=11)
...
md_local = f"""# variations — exp20 report (CIFAR-10, N_VAR=64 K_NN=16)

**Algorithm**: CNN encoder + wide deconvnet decoder. IMLE coverage fix: N_VAR={N_VAR_SAMPLES}, K_NN={K_NN}.

Coverage ratio: {N_VAR_SAMPLES}/{K_NN} = {N_VAR_SAMPLES/K_NN:.1f}× (vs 0.25× in exp17).
kNN: pixel-space L2, sliced from k64 cache.
