"""
Generate and upload a full visualisation report for exp50
(exp35 replica from scratch: LATENT=2048, COND_DIM=16, VOCAB=8, 400ep).

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/scripts/gen_report_exp50.py
"""
...
fig.suptitle(f"Learning dynamics — {EXP} (exp35 replica from scratch, COND_DIM=16, VOCAB=8, 400ep)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"{EXP} — zero-cat reconstructions on test set", fontsize=11)
