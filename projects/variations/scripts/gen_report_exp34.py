"""
Generate and upload a report for variations exp34 (CIFAR-10, LATENT=2048, N_VAR=256, B=32).

Usage:
    uv run python projects/variations/scripts/gen_report_exp34.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, LATENT={LATENT_DIM}, N_VAR={N_VAR_SAMPLES})", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (LATENT={LATENT_DIM}, N_VAR={N_VAR_SAMPLES}, CIFAR-10)", fontsize=11)
...
md_local = f"""# variations — exp34 report (CIFAR-10, LATENT={LATENT_DIM}, N_VAR={N_VAR_SAMPLES})

**Algorithm**: CNN encoder + wide deconvnet decoder. Primary change: N_VAR={N_VAR_SAMPLES} (was 128 in exp33), B=32 (was 64).

Goal: match training coverage to eval coverage. Eval uses 256 fixed conditions with K_NN=16 → 16× coverage ratio.
exp33 trained at 8× coverage; exp34 trains at {N_VAR_SAMPLES/K_NN:.0f}× to close this gap.
...
| N_VAR_SAMPLES | {N_VAR_SAMPLES} (was 128 in exp33) |
| BATCH_SIZE | 32 (was 64 in exp33, keep B×N_VAR=8,192) |
| K_NN | {K_NN} |
| ALPHA | {ALPHA} |
| Coverage ratio | {N_VAR_SAMPLES/K_NN:.1f}× (was 8× in exp33) |
