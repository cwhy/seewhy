"""
Generate and upload a report for variations exp23 (CIFAR-10, LATENT=512).

Usage:
    uv run python projects/variations/scripts/gen_report_exp23.py
"""
...
md_local = f"""# variations — exp23 report (CIFAR-10, LATENT={LATENT_DIM})

**Algorithm**: CNN encoder + wide deconvnet decoder. Primary change: LATENT_DIM={LATENT_DIM} (was 128).

Tests the hypothesis that the reconstruction plateau at ~0.0046 is caused by 24× latent compression.
Expanding to {LATENT_DIM}-d (6× compression) should break the plateau.

## Hyperparameters

| param | value |
|-------|-------|
| LATENT_DIM | {LATENT_DIM} (was 128 in exp15–exp22) |
