"""
Generate and upload a report for variations exp22 (CIFAR-10, N_VAR=64 K_NN=16 + Gaussian style).

Usage:
    uv run python projects/variations/scripts/gen_report_exp22.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, N_VAR=64 K_NN=16 Gaussian)", fontsize=13)
plt.tight_layout()
...
md_local = f"""# variations — exp22 report (CIFAR-10, N_VAR=64 K_NN=16 + Gaussian style)

**Algorithm**: CNN encoder + upsample+conv decoder. Combined exp20 IMLE config with exp21 Gaussian style.

N_VAR={N_VAR_SAMPLES}, K_NN={K_NN}, STYLE_DIM={STYLE_DIM}, coverage={N_VAR_SAMPLES/K_NN:.1f}×.
...
| STYLE_DIM | {STYLE_DIM} (Gaussian, std={STYLE_STD}) |
| N_VAR_SAMPLES | {N_VAR_SAMPLES} |
| K_NN | {K_NN} |
| Coverage ratio | {N_VAR_SAMPLES/K_NN:.1f}× |
| DC_STEM / DC_C1 / DC_C2 / DC_C3 | {DC_STEM} / {DC_C1} / {DC_C2} / {DC_C3} |
| Decoder | upsample+conv (bilinear) |
| n_params | {n_p:,} |
| time | {t_s}s |
