...
Produces:
  ...
  - Local report: projects/variations/report_exp15.md
  - HTML report uploaded to R2

Usage:
    uv run python projects/variations/scripts/gen_report_exp15.py
"""
...
def flat_to_rgb(x_flat):
    """(H*W*C,) or (N, H*W*C) → (..., 32, 32, 3) float in [0,1] for imshow."""
    if x_flat.ndim == 1:
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10)", fontsize=13)
plt.tight_layout()
...
# ── Plot 3: One combined figure per CIFAR class ────────────────────────────────
...
for cls in range(10):
    ...
    variations = np.array(decode(params, zc))   # (256, 3072)

    # Build 16×16 mosaic image (RGB: H, W, 3)
    ...
    eval_vars  = np.array(decode(params, zc_eval))   # (256, 3072)
    ...
    fig.suptitle(
        f"class {cls} ({CIFAR_CLASSES[cls]}) — kNN (row 0) | closest decoded (row 1) | 4×8 mosaic 2×2 quadrants (row 2)",
        fontsize=9, y=0.99
    )
...
md_local = f"""# variations — exp15 report (CIFAR-10)
