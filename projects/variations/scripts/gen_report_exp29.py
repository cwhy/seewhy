"""
Generate and upload a report for variations exp29 (CIFAR-10, 200 epochs).

Usage:
    uv run python projects/variations/scripts/gen_report_exp29.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, LATENT={LATENT_DIM}, {MAX_EPOCHS} epochs)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (LATENT={LATENT_DIM}, {MAX_EPOCHS} epochs, CIFAR-10)", fontsize=11)
...
md_local = f"""# variations — exp29 report (CIFAR-10, {MAX_EPOCHS} epochs)

**Algorithm**: CNN encoder + wide deconvnet decoder. Primary change: MAX_EPOCHS={MAX_EPOCHS} (was 100 in exp23).
...
| Epochs | {MAX_EPOCHS} (was 100 in exp23) |
