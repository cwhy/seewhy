"""
Generate and upload a report for variations exp21 (CIFAR-10, continuous Gaussian style).

Usage:
    uv run python projects/variations/scripts/gen_report_exp21.py
"""
...
from experiments21 import (
    encode, decode,
    LATENT_DIM, STYLE_DIM, STYLE_STD, K_NN,
    EVAL_STYLES, DC_C1, DC_C2, DC_C3, DC_STEM, IMG_FLAT,
    ENC_C1, ENC_C2, ENC_C3, ENC_FC1, N_VAR_SAMPLES,
)
...
ax.set_xlabel("epoch"); ax.set_ylabel("MSE"); ax.set_title("Eval MSE (recon + recall)")
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, Gaussian style)", fontsize=13)
plt.tight_layout()
...
B_s        = len(indices)
recon_style = jnp.zeros((B_s, STYLE_DIM))
recon = np.array(decode(params, jnp.concatenate([z, recon_style], axis=-1)))
...
fig.suptitle(f"variations {EXP} — reconstructions (style=0, CIFAR-10)", fontsize=11)
plt.tight_layout(pad=0.3)
recon_loc, recon_url = save_fig_local_and_r2(f"variations_{EXP}_recon", fig, dpi=150)
plt.close(fig)
print(f"recon  → {recon_url}")

# ── Plot 3: Per-class variation mosaics (16×16 random Gaussian styles) ─────────

GRID_SIDE = 16  # 16×16 = 256 = len(EVAL_STYLES)
...
for cls in range(10):
    ...
    eval_vars  = np.array(decode(params, jnp.concatenate([z_d_rep256, EVAL_STYLES], axis=-1)))

    mosaic = np.zeros((GRID_SIDE * 32, GRID_SIDE * 32, 3))
    for idx in range(GRID_SIDE ** 2):
        r, c = divmod(idx, GRID_SIDE)
        mosaic[r*32:(r+1)*32, c*32:(c+1)*32] = flat_to_rgb(eval_vars[idx])
    ...
    recon_src = np.array(decode(params, jnp.concatenate([z_d, jnp.zeros((1, STYLE_DIM))], axis=-1)))[0]
    ...
    fig.suptitle(
        f"class {cls} ({CIFAR_CLASSES[cls]}) — kNN | closest decoded | 256 random styles",
        fontsize=9, y=0.99
    )
...
md_local = f"""# variations — exp21 report (CIFAR-10, continuous Gaussian style)

**Algorithm**: CNN encoder + upsample+conv decoder. Style: continuous Gaussian z_style ~ N(0, {STYLE_STD}²·I).

Key change from exp20: replaced categorical discrete style with continuous Gaussian (STYLE_DIM={STYLE_DIM}).
Reconstruction uses style=0; variation uses N_VAR={N_VAR_SAMPLES} random normal samples.
...
| STYLE_DIM | {STYLE_DIM} (continuous Gaussian, std={STYLE_STD}) |
| N_VAR_SAMPLES | {N_VAR_SAMPLES} |
| K_NN | {K_NN} |
| DC_STEM / DC_C1 / DC_C2 / DC_C3 | {DC_STEM} / {DC_C1} / {DC_C2} / {DC_C3} |
| Decoder | upsample+conv (bilinear, no checkerboard) |
| n_params | {n_p:,} |
| time | {t_s}s |

## Results

**Final reconstruction MSE**: `{fin_mse}`
...
## Variation grids (256 random Gaussian styles)
