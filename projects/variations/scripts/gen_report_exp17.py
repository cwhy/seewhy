"""
Generate and upload a report for variations exp17 (CIFAR-10, wide decoder).

Usage:
    uv run python projects/variations/scripts/gen_report_exp17.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, wide decoder)", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (recon_emb, CIFAR-10 wide decoder)", fontsize=11)
...
md_local = f"""# variations — exp17 report (CIFAR-10, wide decoder)
...
DC_STEM={DC_STEM}, DC_C1={DC_C1}, DC_C2={DC_C2}, DC_C3={DC_C3}.
...
**Final reconstruction MSE**: `{fin_mse}` (vs exp16 CNN encoder: 0.002545)
