"""
Generate and upload a report for variations exp32 (CIFAR-10, LATENT=2048).

Usage:
    uv run python projects/variations/scripts/gen_report_exp32.py
"""
...
fig.suptitle(f"variations {EXP} — learning curves (CIFAR-10, LATENT={LATENT_DIM})", fontsize=13)
plt.tight_layout()
...
fig.suptitle(f"variations {EXP} — reconstructions (LATENT={LATENT_DIM}, CIFAR-10)", fontsize=11)
...
md_local = f"""# variations — exp32 report (CIFAR-10, LATENT={LATENT_DIM})

**Algorithm**: CNN encoder + wide deconvnet decoder. Primary change: LATENT_DIM={LATENT_DIM} (was 1024 in exp30), ENC_FC1={ENC_FC1}.
...
| param | value |
|-------|-------|
| LATENT_DIM | {LATENT_DIM} (was 1024 in exp30) |
