"""
exp50 — Replicate exp35 from scratch (no warm start, no recon_emb).

exp35 reached recall=0.00666 but was a warm start from exp33 (200 epochs),
making it effectively 400 total epochs of training. This replicates those
exact hyperparameters from random init using the faster per-step JIT loop
(on-the-fly kNN, no lax.scan) over 400 epochs.

Differences from exp35:
- Random init instead of warm-start from exp33
- Per-step JIT (no lax.scan) — same algorithm, 3.4× faster per epoch
- On-the-fly kNN GEMM — same neighbours as precomputed, no file dependency
- No recon_emb

Everything else matches exp35:
- LATENT=2048, N_VAR=128, K_NN=16, COND_DIM=16 (E_DIM=4, N_CAT=4)
- VOCAB=8, ALPHA=1.0, ADAM_LR=3e-4, cosine schedule, BATCH=64
- Same wide-deconvnet decoder, same CNN encoder

Expected: match exp35's best recall=0.00666 in ~400×43s ≈ 4.8h (vs exp35's 16h).

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments50.py
"""
...
# ── Hyperparameters — matches exp35 exactly ───────────────────────────────────

LATENT_DIM    = 2048
N_CAT         = 4
VOCAB_SIZE    = 8
E_DIM         = 4              # same as exp35 (COND_DIM=16)
COND_DIM      = N_CAT * E_DIM  # 16
IN_DIM        = LATENT_DIM + COND_DIM  # 2064
BATCH_SIZE    = 64
N_VAR_SAMPLES = 128
MAX_EPOCHS    = 400            # exp33(200) + exp35(200) total
ADAM_LR       = 3e-4
ALPHA         = 1.0
SEED          = 42
...
# CNN encoder (same as exp35)
ENC_C1        = 64
ENC_C2        = 128
ENC_C3        = 256
ENC_FC1       = 2048

# Wide decoder (same as exp35)
DC_STEM       = 1024
DC_C1         = 256
DC_C2         = 128
DC_C3         = 64
...
def loss_fn(params, x_batch, cats, X_train_norm):
    ...
    # Phase 1 — chunked argmin over N_VAR (stop_gradient)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"exp35-from-scratch: LATENT=2048 N_VAR=128 K={K_NN} COND=16 VOCAB=8 B=64 400ep no-warm-start",
