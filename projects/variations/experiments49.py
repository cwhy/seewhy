"""
exp49 — exp45 + VOCAB_SIZE 8→32, MAX_EPOCHS=300.

Hypothesis: larger vocabulary gives 32^4 = ~1M conditioning codes vs 8^4 = 4096,
letting the model express far more distinct variations. Near-zero compute cost
(only the embedding table grows: 4×32×16 = 2048 params vs 4×8×16 = 512).

Changes from exp45:
- VOCAB_SIZE = 32  (was 8)
- MAX_EPOCHS = 300 (was 150)
- No early stopping (run full schedule)

Architecture otherwise identical: LATENT=2048, N_VAR=128, K_NN=16, COND_DIM=64.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments49.py
"""
...
LATENT_DIM    = 2048
N_CAT         = 4
VOCAB_SIZE    = 32   # 32^4 = ~1M codes vs 8^4 = 4096
E_DIM         = 16
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Local IMLE on-the-fly kNN CIFAR-10 LATENT=2048 N_VAR=128 K={K_NN} VOCAB=32 B=64 300ep",
