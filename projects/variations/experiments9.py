"""
exp9 — Variations: exp6 + per-image cat sampling (fixes shared-slot collapse).

Change from exp6:
  Cats are sampled per-image: (B, N_VAR, N_CAT) instead of shared (N_VAR, N_CAT).
  Each image gets its own pair of candidate conditioning vectors, so gradient
  signal is spread across more distinct slots per step, preventing one slot from
  dominating all images in the batch.

Usage:
    uv run python projects/variations/experiments9.py
"""
...
def loss_fn(params, x_batch, knn_idx_batch, key, X_train):
    ...
    # Variation path: per-image cat sampling — each image gets its own N_VAR candidates
    cats_per_img = jax.random.randint(key, (B, N_VAR_SAMPLES, N_CAT), 0, VOCAB_SIZE)  # (B, N_VAR, N_CAT)
    cats_flat    = cats_per_img.reshape(B * N_VAR_SAMPLES, N_CAT)
    cond_flat    = params["recon_emb"] + cats_to_cond(params, cats_flat)               # (B*N_VAR, COND_DIM)
    cond_var     = cond_flat.reshape(B, N_VAR_SAMPLES, COND_DIM)

    z_rep = jnp.repeat(z[:, None, :], N_VAR_SAMPLES, axis=1).reshape(B * N_VAR_SAMPLES, LATENT_DIM)
    c_rep = cond_var.reshape(B * N_VAR_SAMPLES, COND_DIM)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"inv-IMLE recon_emb+cat-emb  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} E_DIM={E_DIM} N_VAR={N_VAR_SAMPLES} K_NN={K_NN}",
