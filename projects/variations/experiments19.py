"""
exp19 — Feature-space kNN on CIFAR-10. Wide decoder (same as exp17) + dynamic kNN.

Key change from exp17: pixel-space kNN replaced with encoder-embedding kNN.
  - Initial kNN: loaded from cached pixel-space file (bootstrap)
  - Every 10 epochs: recompute kNN using current encoder embeddings (128-d, cosine)
  - Uses FAISS IndexFlatIP on L2-normalised embeddings

Hypothesis: pixel-space kNN is semantically noisy (lighting/background shifts).
Encoder-embedding kNN should find more semantically similar neighbors → better
variation loss signal → sharper, more coherent variations.

Requires: faiss-cpu (or faiss-gpu) — install with: pip install faiss-cpu

Usage:
    uv run python projects/variations/experiments19.py
"""
...
try:
    ...
    logging.warning("faiss not available — falling back to chunked numpy kNN for dynamic update")
...
SEED           = 42
K_NN           = 64
KNN_UPDATE_EVERY = 10  # recompute encoder-space kNN every N epochs

# CNN encoder channels (same as exp17)
ENC_C1         = 64    # 32×32 → 16×16
ENC_C2         = 128   # 16×16 → 8×8
ENC_C3         = 256   # 8×8   → 4×4
ENC_FC1        = 512   # flatten 4096 → 512

# Wide decoder channels (same as exp17)
DC_STEM        = 1024
DC_C1          = 256
DC_C2          = 128
DC_C3          = 64
# → 3-stage ConvTranspose → (B, 3, 32, 32)
...
# ── kNN pre-computation (pixel space, for bootstrap) ──────────────────────────

def compute_knn_pixel(X, k=K_NN, chunk=2000):
    X  = np.array(X)
    N  = len(X)
    knn_idx = np.zeros((N, k), dtype=np.int32)
    sq = (X ** 2).sum(1)
    XT = X.T
    logging.info(f"Computing pixel-space {k}-NN for {N} images (chunk={chunk})…")
...
# ── Dynamic kNN (encoder embedding space) ─────────────────────────────────────

def encode_all_batched(params, X_tr, batch_size=512):
    """Encode all training images in batches → (N, LATENT_DIM) float32 array."""
    embeddings = []
    for i in range(0, len(X_tr), batch_size):
        x = jnp.array(X_tr[i:i + batch_size] / 255.0)
        z = encode(params, x)
        embeddings.append(np.array(z))
    return np.concatenate(embeddings, axis=0)

def recompute_knn_dynamic(params, X_tr):
    """Recompute kNN using current encoder embeddings (cosine similarity).
    Returns (N, K_NN) int32 array of nearest neighbour indices.
    """
    t0 = time.perf_counter()
    E = encode_all_batched(params, X_tr)
    norms = np.linalg.norm(E, axis=1, keepdims=True) + 1e-8
    E_norm = (E / norms).astype(np.float32)

    if FAISS_AVAILABLE:
        index = faiss.IndexFlatIP(E_norm.shape[1])
        index.add(E_norm)
        _, I = index.search(E_norm, K_NN + 1)
        knn_idx = I[:, 1:].astype(np.int32)  # exclude self (rank 0)
    else:
        # Chunked numpy fallback (slower but correct)
        N = len(E_norm)
        knn_idx = np.zeros((N, K_NN), dtype=np.int32)
        chunk = 1000
        for start in range(0, N, chunk):
            end = min(start + chunk, N)
            sims = E_norm[start:end] @ E_norm.T   # (chunk, N)
            for j in range(end - start):
                sims[j, start + j] = -np.inf  # exclude self
            knn_idx[start:end] = np.argpartition(-sims, K_NN, axis=1)[:, :K_NN].astype(np.int32)
            if (start // chunk) % 5 == 0:
                logging.info(f"  dynamic kNN {end}/{N}")

    elapsed = time.perf_counter() - t0
    logging.info(f"Dynamic kNN recomputed in {elapsed:.1f}s  (faiss={FAISS_AVAILABLE})")
    return knn_idx
...
def init_params(key_gen):
    ...
    return {
        ...
        # CNN Encoder weights: HWIO format (kH, kW, C_in, C_out)
        ...
        # Fully connected head: 4×4×256 → 512 → LATENT_DIM
...
def _conv_stride2(x, W, b):
    """Stride-2 conv, SAME padding, NHWC/HWIO."""
...
# ── Decoder (wide deconvnet, same as exp17) ───────────────────────────────────
...
# ── Training loop (with dynamic kNN update) ───────────────────────────────────
...
def train(params, X_tr, X_te, knn_idx):
    ...
    for epoch in range(MAX_EPOCHS):
        # Dynamic kNN update every KNN_UPDATE_EVERY epochs (after epoch 0 bootstrap)
        if epoch > 0 and epoch % KNN_UPDATE_EVERY == 0:
            logging.info(f"Epoch {epoch}: recomputing kNN from encoder embeddings…")
            knn_idx = recompute_knn_dynamic(params, X_tr)
...
if __name__ == "__main__":
    ...
    logging.info(f"FAISS available: {FAISS_AVAILABLE}")
    ...
    # Bootstrap: use pixel-space kNN as initial kNN
    KNN_CACHE = Path(__file__).parent / f"knn_cifar_k{K_NN}.npy"
    if KNN_CACHE.exists():
        knn_idx = np.load(KNN_CACHE)
        logging.info(f"Pixel-space kNN loaded from cache (bootstrap): {KNN_CACHE}")
    else:
        knn_idx = compute_knn_pixel(X_train, k=K_NN)
        np.save(KNN_CACHE, knn_idx)
        logging.info(f"Pixel-space kNN computed and saved to {KNN_CACHE}")
    logging.info(f"kNN ready (initial): {knn_idx.shape}")
    ...
    logging.info(
        ...
        f"  N_VAR={N_VAR_SAMPLES}  K_NN={K_NN}  KNN_UPDATE_EVERY={KNN_UPDATE_EVERY}"
        f"  B*N_VAR={BATCH_SIZE*N_VAR_SAMPLES} decode/step"
        f"  encoder=CNN(C1={ENC_C1},C2={ENC_C2},C3={ENC_C3},FC={ENC_FC1}→{LATENT_DIM})"
        f"  decoder=wide-deconvnet(C1={DC_C1},C2={DC_C2},C3={DC_C3},stem={DC_STEM})"
        f"  knn=dynamic-encoder-cosine  dataset=cifar10  img_flat={IMG_FLAT}"
    )
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"Feature-space kNN CIFAR-10  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} N_VAR={N_VAR_SAMPLES} K_NN={K_NN} B={BATCH_SIZE}",
        ...
        "knn_type":        "dynamic-encoder-cosine",
        "dataset":         "cifar10",
        "img_flat":        IMG_FLAT,
    })
