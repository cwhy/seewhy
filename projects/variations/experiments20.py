"""
exp20 — IMLE coverage fix: N_VAR=64, K_NN=16 on CIFAR-10.

Root cause of blur: with N_VAR=16 style samples covering only 0.4% of the
4096-mode discrete space, most kNN neighbors never get a well-matched candidate.
The model averages unmatched neighbors → blur.

Fix: N_VAR=64 (4× more candidates), K_NN=16 (fewer, closer neighbors).
Coverage ratio: 64 candidates / 16 neighbors = 4 candidates per neighbor on average.
kNN indices: sliced from existing knn_cifar_k64.npy (first 16 cols = nearest subset).

Architecture: same as exp17 (wide CNN encoder + wide deconvnet decoder).

Usage:
    uv run python projects/variations/experiments20.py
"""
...
ALPHA          = 1.0
BATCH_SIZE     = 128
N_VAR_SAMPLES  = 64    # was 16 — 4× more candidates for IMLE matching
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16    # was 64 — fewer, closer neighbors → easier to cover

# CNN encoder channels (same as exp16/17)
ENC_C1         = 64
ENC_C2         = 128
ENC_C3         = 256
ENC_FC1        = 512
...
def loss_fn(params, x_batch, knn_idx_batch, key, X_train):
    ...
    # N_VAR=64 candidates give much better IMLE coverage
    ...
    knn_imgs   = (X_train / 255.0)[knn_idx_batch]          # (B, K_NN=16, IMG_FLAT)
...
if __name__ == "__main__":
    ...
    # Reuse k64 cache, slice to k16 (first K_NN cols are "some of the 64 nearest")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"IMLE coverage fix: N_VAR=64 K_NN=16 CIFAR-10",
