"""
exp53 — Additive Gaussian perturbation on z: z_var = z + eps, eps ~ N(0, I_2048).

Instead of concatenating a conditioning vector, noise is added directly to the latent.
The decoder input is always LATENT_DIM (2048) — same architecture as a plain autoencoder.
Variation space is grounded in the 2048-dim latent geometry.

Key differences from exp51:
- eps ~ N(0, I_2048) added to z, not concatenated
- IN_DIM = LATENT_DIM = 2048 (no extra decoder width)
- Recon: eps=0 → z_var = z exactly
- Fewer parameters than exp51 (smaller decoder stem input)

Changes from exp45 baseline:
- No cat_emb, no cats_to_cond
- decode input: z + eps (not concat([z, cond]))
- Phase 1 saves winning eps vectors (B, K_NN, 2048)
- Phase 2 re-decodes with z + best_eps

Usage:
    CUDA_VISIBLE_DEVICES=1 uv run python projects/variations/experiments53.py
"""
...
LATENT_DIM    = 2048
IN_DIM        = LATENT_DIM        # decoder takes z + eps directly, same dim
BATCH_SIZE    = 64
N_VAR_SAMPLES = 128
MAX_EPOCHS    = 300
ADAM_LR       = 3e-4
...
# CNN encoder (same as exp45/exp49)
ENC_C1        = 64
ENC_C2        = 128
ENC_C3        = 256
ENC_FC1       = 2048

# Wide decoder (same as exp45/exp49)
DC_STEM       = 1024
DC_C1         = 256
DC_C2         = 128
DC_C3         = 64
...
# Fixed eval eps: 256 noise vectors in latent space, sampled once at init
EVAL_EPS = jax.random.normal(jax.random.PRNGKey(0), (256, LATENT_DIM))
...
def init_params(key_gen):
    ...
    return {
        # No cat_emb — conditioning is additive noise on z
...
def decode(params, z_var):
    """z_var = z + eps, shape (B, LATENT_DIM)."""
    B = z_var.shape[0]
    h = jax.nn.gelu(z_var @ params["dc_W1"] + params["dc_b1"])
...
def loss_fn(params, x_batch, key, X_train_norm):
    """
    Two-phase chunked IMLE with additive latent noise: z_var = z + eps.

    Phase 1: sample eps_c ~ N(0, I_2048) per chunk, decode z+eps under stop_gradient,
             find argmin over K neighbours, save winning eps vectors.
    Phase 2: re-decode z + best_eps WITH gradient.
    Recon: decode z + 0 = z exactly (no noise).
    """
    B    = x_batch.shape[0]
    x_nm = x_batch / 255.0
    z    = encode(params, x_nm)                                          # (B, LATENT_DIM)
    ...
    best_eps  = jnp.zeros((B, K_NN, LATENT_DIM))

    for c0 in range(0, N_VAR_SAMPLES, CHUNK):
        chunk_key = jax.random.fold_in(key, c0)
        eps_c = jax.random.normal(chunk_key, (CHUNK, LATENT_DIM))       # (CHUNK, LATENT_DIM)
        # z_var = z + eps for each eps in chunk
        z_c   = jnp.repeat(z[:, None, :], CHUNK, axis=1).reshape(B * CHUNK, LATENT_DIM)
        eps_c_rep = jnp.broadcast_to(eps_c[None], (B, CHUNK, LATENT_DIM)).reshape(B * CHUNK, LATENT_DIM)
        z_var_c = z_c + eps_c_rep
        Y_c   = jax.lax.stop_gradient(decode(params, z_var_c)).reshape(B, CHUNK, IMG_FLAT)
        sq_Y_c  = (Y_c ** 2).sum(-1)
        dists_c = sq_knn[:, :, None] + sq_Y_c[:, None, :] - 2.0 * jnp.einsum("bkd,bvd->bkv", knn, Y_c)
        min_d_c = dists_c.min(axis=-1)
        min_i_c = jnp.argmin(dists_c, axis=-1)
        upd      = min_d_c < best_dist
        best_dist = jnp.where(upd, min_d_c, best_dist)
        # save winning eps vectors
        win_eps_c = eps_c[min_i_c]                                       # (B, K_NN, LATENT_DIM)
        best_eps  = jnp.where(upd[:, :, None], win_eps_c, best_eps)
    best_eps = jax.lax.stop_gradient(best_eps)                          # (B, K_NN, LATENT_DIM)

    # Phase 2 — re-decode z + best_eps WITH gradient
    win_z    = jnp.repeat(z, K_NN, axis=0)                              # (B*K_NN, LATENT_DIM)
    win_eps  = best_eps.reshape(B * K_NN, LATENT_DIM)
    matched  = decode(params, win_z + win_eps)
    L_imle   = ((matched - knn.reshape(B * K_NN, IMG_FLAT)) ** 2).mean()

    # Reconstruction — eps=0 → z_var = z
...
def _eval_recon_batch(params, x_batch):
    ...
    return ((decode(params, z) - x_nm) ** 2).mean()
...
def _recall_batch_jit(params, x_norm, X_train_norm, all_eps):
    ...
    e_rep = jnp.broadcast_to(all_eps[None], (B, N_SLOTS, LATENT_DIM)).reshape(B * N_SLOTS, LATENT_DIM)
    Y_all = decode(params, z_rep + e_rep).reshape(B, N_SLOTS, IMG_FLAT)
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"LATENT={LATENT_DIM}  eps_dim={LATENT_DIM} (additive)  IN_DIM={IN_DIM}"
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Additive z-noise IMLE CIFAR-10 LATENT=2048 eps_dim=2048 N_VAR=128 K={K_NN} B=64 300ep",
        ...
        "imle":       "local-onthefly-kNN-inverse+recon-additive-z-noise",
        "dataset":    "cifar10",
        "img_flat":   IMG_FLAT,
        "cond_dim":   LATENT_DIM,
        "dc_stem":    DC_STEM,
