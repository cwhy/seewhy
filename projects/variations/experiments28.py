"""
exp28 — N_VAR_SAMPLES=128: test IMLE coverage hypothesis.

exp23 (N_VAR=64) has L_var plateauing early (~0.00948). Only 64 style candidates
are sampled per batch; more candidates = better matching of kNN neighbors.

Changes from exp23:
- N_VAR_SAMPLES: 64 → 128  (2× IMLE coverage)

IMLE decode_ar batch: 128×128=16,384 (vs 8,192) — ~25 min overhead vs exp23.
All other hyperparams identical.

Usage:
    uv run python projects/variations/experiments28.py
"""
...
ALPHA          = 1.0
BATCH_SIZE     = 64    # halved from 128 to keep B*N_VAR=8192 (same as exp23's 128*64)
N_VAR_SAMPLES  = 128   # was 64 — primary change: 2× IMLE coverage
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16

# CNN encoder channels (same as exp23)
ENC_C1         = 64
ENC_C2         = 128
ENC_C3         = 256
ENC_FC1        = 512

# Wide decoder channels (same as exp23)
DC_STEM        = 1024
DC_C1          = 256
DC_C2          = 128
DC_C3          = 64
...
def make_step_fn(optimizer):
    ...
    # N_VAR=128 with B=64 gives 8,192 decode passes/step (same as exp23 128*64).
    # lax.scan over 781 steps would need 2× more checkpoint storage than exp23.
    @jax.jit
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=512 wide-deconvnet N_VAR=128 K_NN=16 CIFAR-10",
