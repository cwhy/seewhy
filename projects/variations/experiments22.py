"""
exp22 — Combined: N_VAR=64 continuous Gaussian style + upsample+conv decoder.

Combines exp20 (N_VAR=64 coverage fix) and exp21 (continuous style + no-checkerboard).
This is expected to be the strongest combination.

  - Continuous Gaussian style: STYLE_DIM=16, std=1.0
  - N_VAR=64 candidates, K_NN=16 (coverage ratio 4:1)
  - Wide upsample+conv decoder (no ConvTranspose checkerboard)
  - CNN encoder same as exp17

Usage:
    uv run python projects/variations/experiments22.py
"""
...
ALPHA          = 1.0
BATCH_SIZE     = 128
N_VAR_SAMPLES  = 64   # 4× more candidates
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16   # fewer, closer neighbors → coverage ratio = 64/16 = 4×
...
def init_params(key_gen):
    ...
    return {
        ...
        "dc_cv1": n((3, 3, DC_C1, DC_C2), scale=(3*3*DC_C1)**-0.5),
        "dc_cb1": jnp.zeros(DC_C2),
        "dc_cv2": n((3, 3, DC_C2, DC_C3), scale=(3*3*DC_C2)**-0.5),
        "dc_cb2": jnp.zeros(DC_C3),
        "dc_cv3": n((3, 3, DC_C3, 3),     scale=(3*3*DC_C3)**-0.5),
        "dc_cb3": jnp.zeros(3),
    }
...
def decode(params, zc):
    ...
    h = h.reshape(B, 4, 4, DC_C1)
    h = _upsample2x(h)
    h = jax.nn.gelu(_conv_same(h, params["dc_cv1"], params["dc_cb1"]))
    h = _upsample2x(h)
    h = jax.nn.gelu(_conv_same(h, params["dc_cv2"], params["dc_cb2"]))
    h = _upsample2x(h)
    h = jax.nn.sigmoid(_conv_same(h, params["dc_cv3"], params["dc_cb3"]))
    return h.transpose(0, 3, 1, 2).reshape(B, IMG_FLAT)
...
if __name__ == "__main__":
    ...
    if not KNN64_CACHE.exists():
        raise FileNotFoundError(f"Need {KNN64_CACHE}")
    knn_idx = np.load(KNN64_CACHE)[:, :K_NN]
    logging.info(f"kNN ready (sliced k64→k{K_NN}): {knn_idx.shape}")

    key_gen = infinite_safe_keys_from_key(jax.random.PRNGKey(SEED))
    params  = init_params(key_gen)
    logging.info(
        f"Parameters: {n_params(params):,}  "
        f"STYLE=Gaussian(dim={STYLE_DIM},std={STYLE_STD})"
        f"  N_VAR={N_VAR_SAMPLES}  K_NN={K_NN}  coverage={N_VAR_SAMPLES/K_NN:.1f}x"
        f"  decoder=upsample+conv  dataset=cifar10"
    )
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": "N_VAR=64 continuous Gaussian + upsample+conv decoder CIFAR-10",
        ...
        "style_type": "continuous-gaussian", "dataset": "cifar10", "img_flat": IMG_FLAT,
    })
