"""
exp25 — LATENT_DIM=512 + ALPHA=3.0: stronger variation pressure.

Hypothesis: with a 512-d latent (exp23), the encoder has enough capacity.
Increasing ALPHA from 1.0 to 3.0 forces the decoder to cover more variation
modes, potentially producing sharper and more diverse outputs.

Base: exp23 (LATENT=512, wide-deconvnet, N_VAR=64, K_NN=16, pixel-space kNN).
Only change: ALPHA=3.0.

Usage:
    uv run python projects/variations/experiments25.py
"""
...
ALPHA          = 3.0    # was 1.0 — stronger variation pressure
BATCH_SIZE     = 128
N_VAR_SAMPLES  = 64
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"LATENT={LATENT_DIM}  N_VAR={N_VAR_SAMPLES}  K_NN={K_NN}  ALPHA={ALPHA}  B={BATCH_SIZE}"
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=512 ALPHA=3.0 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10",
