"""
exp33 — LATENT=2048 + N_VAR=128 + chunked scan, 200 epochs.

exp31 (LATENT=1024, N_VAR=128, 200ep): recon_mse=0.001184, recall_mse=0.007426
Both metrics still declining at ep199.

Changes from exp31:
- LATENT_DIM: 1024 → 2048
- ENC_FC1:    1024 → 2048
- CHUNK_SIZE = 390: chunked lax.scan replaces Python step loop

IN_DIM auto: 2048 + 16 = 2064.
B=64 (unchanged, OOM constraint with N_VAR=128).
num_batches = 50000//64 = 781; n_full_chunks = 781//390 = 2 dispatches/epoch.
Expected ~2× speedup over exp31 (150s/epoch → ~80-90s/epoch).

Memory safety: CHUNK_SIZE=390 is safe because exp23 already runs lax.scan over
exactly 390 steps with B=128, N_VAR=64 (same B×N_VAR=8,192 per-step decode passes).

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments33.py
"""
...
LATENT_DIM     = 2048   # was 1024 (exp31)
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
...
ALPHA          = 1.0
BATCH_SIZE     = 64    # keep B*N_VAR=8,192 to avoid scan OOM
N_VAR_SAMPLES  = 128   # exp28/31 change
MAX_EPOCHS     = 200
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16
...
def make_chunk_fn(optimizer):
    ...
    # Same per-step memory as exp23 scan (B×N_VAR=8,192 decode passes).
    @jax.jit
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=2048 wide-deconvnet N_VAR=128 K_NN=16 CIFAR-10 200ep chunked-scan",
