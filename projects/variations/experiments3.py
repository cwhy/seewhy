"""
exp3 — Variations: inverted IMLE (fast) matching (kNN → closest sample).

Change from exp1:
  Instead of: pick one random b → decode → find closest kNN neighbor
  Now:        sample N_VAR_SAMPLES bit vectors → decode all → for each kNN neighbor
              find the closest decoded output → push that output toward the neighbor

This ensures every kNN neighbor is "served" by some slot each step,
driving genuine diversity across bit patterns instead of collapse.

Speed fix vs exp2: BATCH_SIZE=256 (234 batches/epoch vs 937) keeps epoch time ~5-10s.
B*N_VAR=2048 decode calls/step — memory ~4GB, safe on RTX 4090.

Usage:
    uv run python projects/variations/experiments3.py
"""
...
D              = D_R * D_C * D_V  # 144
ALPHA          = 1.0
BATCH_SIZE     = 256               # B*N_VAR_SAMPLES=1024 decode calls per step (matches exp1 memory)
N_VAR_SAMPLES  = 8               # bit patterns sampled per step for matching
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 64
...
def decode(params, zb):
    ...
    B     = zb.shape[0]
    h     = jax.nn.gelu(zb @ params["dec_W_mlp1"] + params["dec_b_mlp1"])
...
def loss_fn(params, x_batch, knn_idx_batch, key, X_train):
    ...
    # Variation path: sample N_VAR_SAMPLES bit vectors, decode all for each image
    b_samples = jax.random.bernoulli(key, 0.5, (N_VAR_SAMPLES, N_BITS)).astype(jnp.float32)

    z_rep = jnp.repeat(z[:, None, :], N_VAR_SAMPLES, axis=1).reshape(B * N_VAR_SAMPLES, LATENT_DIM)
    b_rep = jnp.broadcast_to(b_samples[None], (B, N_VAR_SAMPLES, N_BITS)).reshape(B * N_VAR_SAMPLES, N_BITS)
    Y_all = decode(params, jnp.concatenate([z_rep, b_rep], axis=-1))      # (B*N_VAR, 784)
    ...
    # Dot-product trick: ||a-b||² = ||a||² + ||b||² - 2a·bᵀ  — avoids (B,K,N,784) broadcast
...
if __name__ == "__main__":
    ...
    logging.info(
        ...
        f"LATENT={LATENT_DIM}  BITS={N_BITS}  N_VAR={N_VAR_SAMPLES}  K_NN={K_NN}"
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"inverted-IMLE  LATENT={LATENT_DIM} BITS={N_BITS} N_VAR={N_VAR_SAMPLES} K_NN={K_NN}",
