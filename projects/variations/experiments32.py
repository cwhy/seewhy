"""
exp32 — LATENT=2048, 200 epochs: scale up from exp30.

exp30 (LATENT=1024, 200ep): recon_mse=0.00124
LATENT doubling law held at every step: 512→1024 at 200ep gave −25%.
LATENT=2048 should give another ~−25%.

Changes from exp30:
- LATENT_DIM: 1024 → 2048
- ENC_FC1:    1024 → 2048  (match LATENT_DIM)

IN_DIM auto: 2048 + 16 = 2064.
B=128, N_VAR=64, standard lax.scan (same as exp30 — no OOM risk).
Cosine schedule auto-scales: num_steps = 200 * num_batches = 78,000.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments32.py
"""
...
LATENT_DIM     = 2048   # was 1024 (exp30)
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=2048 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10 200ep",
