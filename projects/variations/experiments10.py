"""
exp10 — exp6 + N_VAR_SAMPLES=32, K_NN=16.

Fixes the N_VAR=2 sparsity issue. Samples 32 conditioning vectors per step
(vs 2 in exp6), covering much more of the slot space each batch.
K_NN=16: sliced from existing 64-NN index.
BATCH_SIZE=64 to keep B×N_VAR=2048 decode/step (avoids OOM).

Usage:
    uv run python projects/variations/experiments10.py
"""
...
if __name__ == "__main__":
    ...
    # Reuse the 64-NN cache, slice to K_NN=16
    KNN_CACHE = Path(__file__).parent / "knn_mnist_k64.npy"
    if KNN_CACHE.exists():
        knn_idx = np.load(KNN_CACHE)[:, :K_NN]
        logging.info(f"kNN loaded from cache (sliced to k={K_NN}): {KNN_CACHE}")
    else:
        knn_idx = compute_knn(X_train, k=K_NN)
        np.save(Path(__file__).parent / f"knn_mnist_k{K_NN}.npy", knn_idx)
