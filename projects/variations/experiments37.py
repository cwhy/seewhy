"""
exp37 — Global IMLE on CIFAR-10 (No Recon, No kNN, JAX GPU NN search).

Key differences from exp35 (local IMLE):
- No kNN precomputation: global NN search over full 50k training set
- No recon_emb param: all conditioning codes uniform
- No L_recon: pure IMLE loss only
- NN search done entirely in JAX on GPU — no FAISS, no CPU transfer bottleneck:
    forward_and_find_nn: decode 4096 variations + GEMM (4096×50k×3072) ≈ 58ms/step
    On RTX 4090 (165 TFLOPS): 4096×50k×3072×2 FLOPs ≈ 8ms for the NN search
- Same cats for NN search and gradient: correct global IMLE (code→target mapping is consistent)
- Python for loop per epoch (2 jit calls per step)

Architecture identical to exp35 (LATENT=2048, wide-deconvnet, CNN encoder).
Estimated epoch time: ~120-160s, total ~7-9h for 200ep.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments37.py
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Global IMLE CIFAR-10 LATENT=2048 N_VAR=64 B=64 200ep exact-GPU-NN",
