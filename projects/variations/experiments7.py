"""
exp7 — Variations: forward IMLE (image→sample matching) instead of inverted IMLE.

Changes from exp6:
  Forward IMLE: for each image x, sample N_VAR conditioning vectors, decode them,
  find the closest decoded output to x, push that output toward x.
  No kNN needed in training — diversity emerges from the selection process across batches.
  kNN is still used for eval_recall_mse only.

  All other architecture choices from exp6 unchanged:
  - Dedicated recon_emb
  - 4×8 categorical structure (N_CAT=4, VOCAB_SIZE=8, COND_DIM=16)
  - Per-rank recall eval

Usage:
    uv run python projects/variations/experiments7.py
"""
...
LATENT_DIM     = 64
N_CAT          = 4
VOCAB_SIZE     = 8               # 8^4 = 4096 total slots
E_DIM          = 4
...
ALPHA          = 1.0
BATCH_SIZE     = 256
N_VAR_SAMPLES  = 2               # conditioning slots sampled per image per step
MAX_EPOCHS     = 100
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 64
...
def loss_fn(params, x_batch, key):
    """
    Forward IMLE: for each image, find closest decoded sample, push it toward image.
    ...
    # Reconstruction path: recon_emb + cat_emb[zeros]
    ...
    # Forward IMLE variation path: recon_emb + cat_emb[random cats]
    ...
    # For each image, find closest decoded sample (stop-grad: selection only)
    sq_x   = (x_norm ** 2).sum(-1)                                        # (B,)
    sq_Y   = (Y_all ** 2).sum(-1)                                         # (B, N_VAR)
    dot    = jnp.einsum("bd,bvd->bv", x_norm, Y_all)                     # (B, N_VAR)
    dists  = sq_x[:, None] + sq_Y - 2.0 * dot                            # (B, N_VAR)
    best   = jnp.argmin(dists, axis=-1)                                   # (B,)
    matched = Y_all[jnp.arange(B), best]                                  # (B, 784)

    L_var = ((matched - jax.lax.stop_gradient(x_norm)) ** 2).mean()
...
def train(params, X_tr, X_te, knn_idx):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        params, opt_state, loss, l_recon, l_var = epoch_fn(
            params, opt_state, Xs_batched, key
        )
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"forward-IMLE recon_emb+cat-emb  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} E_DIM={E_DIM} N_VAR={N_VAR_SAMPLES} K_NN={K_NN}",
