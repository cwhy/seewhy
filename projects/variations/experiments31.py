"""
exp31 — LATENT=1024 + N_VAR=128 + 200 epochs: all three improvements combined.

exp27 (LATENT=1024):  recon_mse=0.00173 (−21%)
exp28 (N_VAR=128):    recon_mse=0.00178 (−19%), recall_mse=0.00826 (−7%)
exp29 (200 epochs):   recon_mse=0.00165 (−25%)

Changes from exp23:
- LATENT_DIM: 512 → 1024
- ENC_FC1:    512 → 1024
- N_VAR_SAMPLES: 64 → 128
- MAX_EPOCHS: 100 → 200
- BATCH_SIZE: 128 → 64  (keep B*N_VAR=8,192 to avoid scan OOM)

Uses Python step loop (not lax.scan) — same OOM fix as exp28.
Requires CUDA_VISIBLE_DEVICES=0 (single GPU).
~9h estimated (200 epochs × ~150s/epoch).

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments31.py
"""
...
LATENT_DIM     = 1024   # exp27 change
N_CAT          = 4
VOCAB_SIZE     = 8
E_DIM          = 4
COND_DIM       = N_CAT * E_DIM   # 16
IN_DIM         = LATENT_DIM + COND_DIM  # 1040
ALPHA          = 1.0
BATCH_SIZE     = 64    # halved: keep B*N_VAR=8,192 to avoid scan OOM
N_VAR_SAMPLES  = 128   # exp28 change
MAX_EPOCHS     = 200   # exp29 change
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16
...
def make_step_fn(optimizer):
    # Python-loop version: JIT each step independently to avoid scan OOM.
    # LATENT=1024 + N_VAR=128 with B=64 → 8,192 decode passes/step.
    # Requires CUDA_VISIBLE_DEVICES=0 (single GPU).
    @jax.jit
    def step_fn(params, opt_state, x_batch, knn_idx_batch, key, X_train):
...
def train(params, X_tr, X_te, knn_idx):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        Xs_batched  = X_tr[perm[:n]].reshape(num_batches, BATCH_SIZE, IMG_FLAT)
        KNN_batched = knn_idx[perm[:n]].reshape(num_batches, BATCH_SIZE, K_NN)

        losses_ep, l_recon_ep, l_var_ep = [], [], []
        for b in range(num_batches):
            step_key = jax.random.fold_in(key, b)
            params, opt_state, loss_b, lr_b, lv_b = step_fn(
                params, opt_state,
                jnp.array(Xs_batched[b]), jnp.array(KNN_batched[b]),
                step_key, X_train_jax
            )
            losses_ep.append(float(loss_b)); l_recon_ep.append(float(lr_b)); l_var_ep.append(float(lv_b))

        loss    = float(np.mean(losses_ep))
        l_recon = float(np.mean(l_recon_ep))
        l_var   = float(np.mean(l_var_ep))
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=1024 wide-deconvnet N_VAR=128 K_NN=16 CIFAR-10 200ep",
