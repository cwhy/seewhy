...
def loss_fn(params, x_batch, cats, X_train_norm):
    ...
    knn = X_train_norm[knn_idx[:, 1:]]                                        # (B, K_NN, IMG_FLAT)
    ...
    # 3. Inverse IMLE: for each neighbour find best variation (assignment treated as constant)
    sq_knn   = (knn ** 2).sum(-1)                                             # (B, K_NN)
    sq_Y     = (Y_all ** 2).sum(-1)                                           # (B, N_VAR)
    dot_kv   = jnp.einsum("bkd,bvd->bkv", knn, Y_all)                        # (B, K_NN, N_VAR)
    dists_kv = sq_knn[:, :, None] + sq_Y[:, None, :] - 2.0 * dot_kv         # (B, K_NN, N_VAR)
    best_var = jax.lax.stop_gradient(jnp.argmin(dists_kv, axis=-1))          # (B, K_NN)
    matched  = Y_all[jnp.arange(B)[:, None], best_var]                       # (B, K_NN, IMG_FLAT)
    L_imle   = ((matched - knn) ** 2).mean()
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Local IMLE on-the-fly kNN CIFAR-10 LATENT=2048 N_VAR=64 K={K_NN} B=64 200ep single-pass+recon",
