"""
exp29 — MAX_EPOCHS=200: training not yet converged at ep99.

exp23 training curves were still declining at ep99 (−1.6e-5/checkpoint).
L_recon flat at 0.00213 training but eval still declining. 200 epochs with
a correctly-scaled cosine schedule should continue improving.

Changes from exp23:
- MAX_EPOCHS: 100 → 200

Cosine schedule auto-scales: num_steps = 200 * num_batches = 78,000.
LR decays 3e-4 → 1.5e-5 over the full 200 epochs. ~4h.

Usage:
    uv run python projects/variations/experiments29.py
"""
...
BATCH_SIZE     = 128
N_VAR_SAMPLES  = 64
MAX_EPOCHS     = 200   # was 100 — primary change
ADAM_LR        = 3e-4
SEED           = 42
K_NN           = 16
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"LATENT=512 wide-deconvnet N_VAR=64 K_NN=16 CIFAR-10 200ep",
