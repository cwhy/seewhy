"""
exp52 — exp49 + VOCAB_SIZE 32→64, MAX_EPOCHS=400.

Hypothesis: VOCAB=64 gives 64^4 = ~16M conditioning codes vs 32^4 = ~1M.
exp49 showed VOCAB=32 converges better than VOCAB=8 given enough epochs (300ep → 0.00658).
VOCAB=64 continues this scaling. Needs more epochs because each code is even sparser
per training step (128/16M vs 128/1M coverage per step).

Changes from exp49:
- VOCAB_SIZE = 64  (was 32)
- MAX_EPOCHS = 400 (was 300)

Architecture otherwise identical: LATENT=2048, N_CAT=4, E_DIM=16, COND_DIM=64,
N_VAR=128, K_NN=16, wide deconvnet decoder, CNN encoder.

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments52.py
"""
...
LATENT_DIM    = 2048
N_CAT         = 4
VOCAB_SIZE    = 64   # 64^4 = ~16M codes vs 32^4 = ~1M
E_DIM         = 16
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Local IMLE on-the-fly kNN CIFAR-10 LATENT=2048 N_VAR=128 K={K_NN} VOCAB=64 B=64 400ep",
