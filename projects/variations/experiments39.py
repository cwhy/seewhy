"""
exp39 — Global IMLE + exclusive Voronoi assignment + reconstruction loss.

Adds reconstruction loss to exp38 to anchor the encoder to input-specific structure.
Without recon loss (exp38) the encoder is unanchored → recall 3.5× worse than exp35.

Loss: ALPHA * L_recon + L_imle
  L_recon = MSE(decode(z, cats_zero), x)   — anchors encoder to reconstruct x
  L_imle  = MSE(Y_var, exclusive_NN(Y_var)) — covers training distribution globally

Key differences from exp38:
- ALPHA * L_recon added to loss (cats=zeros is the reconstruction slot)
- history tracks L_recon and L_imle separately

Key differences from exp35 (local IMLE):
- No kNN precomputation: global NN via on-the-fly GPU GEMM
- No recon_emb: reconstruction uses cats=zeros directly
...
Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/variations/experiments39.py
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":       EXP_NAME,
        "name":             f"Global IMLE CIFAR-10 LATENT=2048 N_VAR=64 B=64 200ep exclusive-Voronoi-NN+recon",
        ...
        "imle":       "global-jax-gpu-exclusive-voronoi+recon",
        "dataset":    "cifar10",
        "img_flat":   IMG_FLAT,
    })
