"""
exp13 — exp12 + B=128, N_VAR=16 (step-count optimization).

Same B×N=2048 decode budget per step as exp12, but 468 scan steps per epoch
instead of 937. Per-step cost is nearly unchanged; halving step count gives
~2x epoch speedup. N_VAR reduced from 32→16 to keep budget fixed.

Results may differ slightly from exp12 due to fewer variation candidates..

Same hyperparameters as exp11 (N_VAR=32, K_NN=64, B=64).
Replaces the per-pixel vmap in decode() with 3 batched matmuls,
eliminating 784 sequential-ish operations in favour of pure XLA-fused ops.

Results should be directly comparable to exp11 (identical model capacity).

Usage:
    uv run python projects/variations/experiments13.py
"""
...
LATENT_DIM     = 64
N_CAT          = 4               # number of categorical variables
VOCAB_SIZE     = 8               # categories per variable  (8^4 = 4096 slots total)
...
# Fixed eval conditioning set: 256 random samples from 4096 total (seed=0)
_rng_eval = np.random.RandomState(0)
EVAL_COND_CATS = jnp.array(
    _rng_eval.randint(0, VOCAB_SIZE, (256, N_CAT)), dtype=jnp.int32
)   # (256, N_CAT) — fixed at module load, used in recall eval
...
def init_params(key_gen):
    ...
    return {
        ...
        # Dedicated reconstruction conditioning vector
...
def decode(params, zc):
    ...
    Replaces the per-pixel vmap from exp11 with 3 batched matmuls:
      1. Outer products for all 784 pixels at once      → (784, D_RC)
      2. Project all positions into gram space           → (784, D)
      3. Apply gram to all pixel queries simultaneously  → (B, 784, D)
    No vmap, no per-pixel index ops — pure matmuls XLA fuses on GPU.
    """
    B     = zc.shape[0]
    h     = jax.nn.gelu(zc @ params["dec_W_mlp1"] + params["dec_b_mlp1"])   # (B, MLP_HIDDEN)
    ...
    # Step 1: outer products for all 784 pixels at once
    row_embs = params["dec_row_emb"][ROWS]      # (784, D_R)
    col_embs = params["dec_col_emb"][COLS]      # (784, D_C)
    pos_all  = jnp.einsum("pi,pj->pij", row_embs, col_embs).reshape(784, D_RC)  # (784, D_RC)

    # Step 2: project all pixel positions into gram space
    # dec_W_pos is (D, D_RC); q_all = pos_all @ W_pos.T → (784, D)
    q_all = pos_all @ params["dec_W_pos"].T                                  # (784, D)

    # Step 3: apply gram to all pixel queries at once
    # gram: (B, D, D); q_all.T: (D, 784) → f_all: (B, D, 784) → (B, 784, D)
    f_all = (gram @ q_all.T).transpose(0, 2, 1)                             # (B, 784, D)

    # Step 4: val net — batched over B and 784 simultaneously
    # dec_W_val1: (2D, D); dec_W_val2: (1, 2D)
    h1  = jax.nn.gelu(f_all @ params["dec_W_val1"].T + params["dec_b_val1"]) # (B, 784, 2D)
    out = (h1 @ params["dec_W_val2"].T).squeeze(-1)                          # (B, 784)
...
def loss_fn(params, x_batch, knn_idx_batch, key, X_train):
    ...
    # Reconstruction path: recon_emb + cat_emb[zeros] (fixed recon slot)
    cats_recon = jnp.zeros((1, N_CAT), dtype=jnp.int32)
    recon_cond = params["recon_emb"] + cats_to_cond(params, cats_recon)     # (1, COND_DIM)
    recon_cond = jnp.broadcast_to(recon_cond, (B, COND_DIM))
    x_recon    = decode(params, jnp.concatenate([z, recon_cond], axis=-1))
    L_recon    = ((x_recon - x_norm) ** 2).mean()

    # Variation path: recon_emb + cat_emb[random cats]
...
EVAL_RECALL_B = 8    # images per recall mini-batch (8 × 256 slots = 2048 decode calls)

@jax.jit
def _recall_batch_jit(params, x_norm, knn_norm, all_conds):
    """x_norm: (B,784), knn_norm: (B,K_NN,784), all_conds: (N_SLOTS,COND_DIM)
    Returns: (mean_recall, per_rank) where per_rank is (K_NN,)"""
    B       = x_norm.shape[0]
    ...
    matched = Y_all[jnp.arange(B)[:, None], best]   # (B, K_NN, 784)

    # Sort kNN by distance to source image (rank 0 = closest neighbor)
    sq_x       = (x_norm ** 2).sum(-1)                                  # (B,)
    dot_xk     = jnp.einsum("bd,bkd->bk", x_norm, knn_norm)            # (B, K_NN)
    dist_x_knn = sq_x[:, None] + sq_knn - 2.0 * dot_xk                 # (B, K_NN)
    rank_order = jnp.argsort(dist_x_knn, axis=-1)                      # (B, K_NN)

    sorted_matched = matched[jnp.arange(B)[:, None], rank_order]       # (B, K_NN, 784)
    sorted_knn     = knn_norm[jnp.arange(B)[:, None], rank_order]      # (B, K_NN, 784)
...
def eval_recall_mse(params, X_tr, knn_idx, n_images=512):
    """Recall MSE with per-rank breakdown. Returns (mean_mse, per_rank_list)."""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment":      EXP_NAME,
        "name":            f"inv-IMLE recon_emb+cat-emb vectorized-decode  LATENT={LATENT_DIM} N_CAT={N_CAT} VOCAB={VOCAB_SIZE} E_DIM={E_DIM} N_VAR={N_VAR_SAMPLES} K_NN={K_NN}",
