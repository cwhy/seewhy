...
EXP_NAME            = "exp11d_fmnist"
USE_ROSA_FEAT       = False           # exp11d: lesion at scale + n_pop=128
...
ES_SIGMA            = 0.001
ES_N_POP            = 128          # exp11d: matches exp10d
ES_RANK             = 4              # exp10c: higher-rank perturbations
ADAM_LR_INIT        = 1e-3
ADAM_LR_END         = 1e-4
...
def classify_loss(p, x_batch, y_batch, predicted_ids):
    ...
    if not USE_ROSA_FEAT:
        rosa_pred_emb = jnp.zeros_like(rosa_pred_emb)
...
def main():
    ...
    row = {
        "experiment": EXP_NAME,
        "name": f"eggroll-NO-rosa-60K-rank4-128pop-{DATASET}",
        "dataset": DATASET,
        "test_acc": final_test,
