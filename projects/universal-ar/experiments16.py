"""
Universal AR — exp16: 0-vs-1 with FULL DIAGNOSTICS — positive control for exp15.

Same instrumentation as exp15 but on the EASY pair that already reached 1.000
(exp12). Shows what a WORKING label loss curve looks like, so exp15 (4v9) can be
read against it.

exp13 (4 vs 9, anonymised) sat exactly at chance 0.500. But two things were
invisible: (a) no TRAIN-query accuracy, so we could not tell "cannot fit" from
"cannot generalise"; (b) the logged loss is pixel+label combined, and labels are
only ~1/17 of scored tokens (16 masked pixels + 1 label per sample) — so a totally
flat label loss is nearly invisible in it. exp13 loss fell 0.47→0.42 while label
accuracy never moved: that was pixels, not labels.

This re-runs the identical config with: separate pixel/label losses, label accuracy
on the training batch, and label accuracy for TRAIN-drawn queries as well as test.
...
TASK_DIGITS = (0, 1)                   # EASY pair — positive control for the diagnostics
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 with full diagnostics: split pixel/label loss + train-query accuracy",
