"""
Universal AR — exp29: give the failing task a real share of the gradient.

exp28 settled what the problem is NOT. With deterministic labels the identical model
reaches 0.875 on 4-vs-9, so the encoder represents the shape fine; only the in-context
binding fails (exp26, anonymised: 0.516).

Binding is an induction-head-shaped circuit — spot that support sample s carries label
token t, notice the query resembles s, emit t. Circuits like that are known to be hard
to form. And we had been starving it: the loss was a single mean over every scored
token, so per episode

    256 retrieval-pixel + 256 generalise-pixel + 10 retrieval-label + 6 generalise-label

made the task under study ~1.1% of the gradient while pixel completion took ~97%.

This does not add a fudge factor; it fixes the accounting. Take a mean PER TASK TYPE
and weight those, so the label share is a deliberate knob instead of a side effect of
how many tokens each task emits. W_LAB_GEN=1.0 puts generalise-labels at ~25% of the
gradient, roughly a 20x increase.

Everything else is identical to exp26 (KDA, shuffled tokens, anonymised 4v9, obs 0.25,
6000 steps), so it is a controlled pair: exp26 = 0.516.
"""
...
W_LAB_GEN = 1.0                        # weight on the generalise-label term (1.0 = equal to each other task)
...
def loss_fn(p, pos, val, ref, target, isq, is_lab, is_retr):
    ...
    # BALANCED objective. The old loss was one mean over every scored token, so with
    # 512 pixel queries and 6 generalise-label queries per episode the task under study
    # was ~1.1% of the gradient and pixel completion was ~97%. Take a mean PER TASK TYPE
    # and weight them, so "how much label signal" is a knob rather than an accident of
    # how many tokens each task happens to emit.
    loss = mean("pix_gen") + mean("pix_retr") + mean("lab_retr") + W_LAB_GEN * lab_loss
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 anonymised, task-balanced loss (W_LAB_GEN={W_LAB_GEN})",
