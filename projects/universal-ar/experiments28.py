"""
Universal AR — exp28: can the ENCODER discriminate 4 from 9 at all?

Seven architectures now sit at chance on anonymised 4-vs-9 while solving 0-vs-1
perfectly. Every one of them was asked to do two things at once: build a
representation of a half-observed digit, AND bind it to a label read from context.
Nothing so far separates those two abilities.

This flips ONE flag: labels become deterministic. Class 4 always maps to the same
label token, class 9 to the other. The model can now predict a query's label from its
OWN pixels and ignore the support entirely — so no binding, no comparison, no
in-context anything stands in the way. It measures exactly one thing:

    can this encoder build a class-discriminative representation of a
    half-observed 4 or 9?

  high  -> the representation is fine; the failure is specifically in-context binding
  low   -> the encoder cannot separate 4/9 even under direct supervision, and no
           in-context mechanism could ever have worked

Everything else is identical to exp26 (KDA memory, shuffled tokens, obs 0.25, same
episodes, same 6000 steps), so it is a controlled pair: exp26 = 0.516 anonymised.

Prior hint, from a different config: exp10 reached 0.828 on 10-way with fixed labels
at obs 0.5, which suggests the representation IS capable — but that was attention, a
different observation fraction and a different task, so this measures it directly.
"""
...
ANON_LABELS = False                    # DETERMINISTIC labels — encoder capability test
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 DETERMINISTIC labels — encoder capability test (controlled pair with exp26)",
