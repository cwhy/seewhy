"""
Universal AR — exp19: 4-vs-9 with K-SHOT support (the task finally has headroom).

Why the previous 4v9 runs were unwinnable. A nearest-neighbour baseline on the
binned pixels shows how much signal actually exists:

    pair    1-shot  5-shot  20-shot  50-shot        (chance 0.50)
    0 vs 1   0.872   0.961    0.987    0.989
    4 vs 9   0.567   0.635    0.787    0.829

At 1-shot, 4-vs-9 is barely above chance EVEN WITH THE FULL IMAGE (0.584). exp15
/17/18 all sat at ln(2) because there was almost nothing to extract — not because
alignment or depth was the bottleneck (both were tested and neither moved it).

A design flaw compounded it: training episodes drew 10 random support samples from
a 2-class pool (~5 per class) while evaluation was 1-shot balanced — i.e. testing
on a harder setting than training.

This fixes both. Support is K-SHOT and matched between train and eval:
  * OBS_FRAC 0.25 (196 px/sample) to afford many support samples in context
  * N_SHOT = 20 labelled examples PER CLASS at eval; ~20/class in training too
  * naive-NN ceiling for this config ≈ 0.711 (measured), so there is real headroom
  * micro-batch 2 x accum 4 keeps the effective batch at 8

  learns → the earlier failures were a signal/task-design problem, now fixed
  fails  → the model cannot exploit signal that demonstrably exists (naive NN can)
"""
...
HEAD_DIM = 32
D, N_LAYERS = 256, 4
OBS_FRAC = 0.25                         # fewer pixels per sample, to afford many support samples
N_CTX = int(round(OBS_FRAC * POS_PIX))  # 392
N_QP = 16
N_SUP, N_QRY = 40, 4                    # training: ~20 support per class (matches eval k-shot)
Q_EVAL = 4
N_SHOT = 20                            # eval: 20 labelled support examples PER CLASS
MICRO_BATCH, ACCUM = 2, 4              # effective batch = 8 (smaller micro-batch: longer context)
...
def build_eval_bal(Xtr, ytr, cls_idx, Xq, yq, rng):
    S, n_pool = N_TASK * N_SHOT + Q_EVAL, min(2 * N_CTX, POS_PIX); arrs = _alloc(S)
    for b in range(MICRO_BATCH):
        pool = rng.permutation(POS_PIX)[:n_pool]; refs = rng.permutation(V_REFS)[:S]; t = 0
        perm = _perm(rng)                        # fresh label semantics for THIS episode
        reps = [(Xtr[cls_idx[c][rng.integers(len(cls_idx[c]))]], c, True)
                for c in range(N_TASK) for _ in range(N_SHOT)]   # N_SHOT per class
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 K-SHOT support (N_SHOT=20/class, obs 0.25); naive-NN ceiling 0.711",
