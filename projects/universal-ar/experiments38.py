"""
Universal AR — exp38: the REAL variant-A test (4v9) — self-recall of pixels AND labels.

A correction to exp35/36. Those trained on label retrieval alone, which is solvable by
matching the ref tag and never requires looking at a pixel — and the model duly learned
nothing about images (pixel retrieval 0.000, pixel loss ~16). Chance generalisation there
was determined by the task definition, not by the architecture, so it tested nothing.

The founding hypothesis in concepts.md ("variant A") was self-recall of the OBSERVED
ENTRIES — pixels included. Pixel self-recall does force the model to encode image
content, so this is the version where cross-sample completion could plausibly emerge.

    loss = pixel-retrieval + label-retrieval        (both generalisation terms get zero gradient)

The archived exp3/exp5 ran this with FIXED labels and saw label generalisation emerge
weakly, then degrade with more context (0.40 -> 0.15). It has never been run under
ANONYMISED labels, where memorised class prototypes are worth exactly chance.

Read it against: 4v9 trained WITH the generalise loss reaches ~0.52 (chance).
lab_gen_2way restricts the argmax to the label slots (chance 0.5); the open-vocabulary
number is reported too, since an untrained head can answer a label query with a pixel bin.
"""
...
def loss_fn(p, pos, val, ref, target, isq, is_lab, is_retr):
    ...
    # SELF-RECALL ONLY (the real "variant A"): recall your own observed entries, pixels
    # AND labels. Unlike label-retrieval-only, pixel self-recall genuinely forces the
    # model to encode image content — so this is the version where emergence is even
    # possible. Neither generalisation term receives any gradient.
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"RETRIEVAL-ONLY objective ({TASK_DIGITS}); generalisation measured but never optimised",
