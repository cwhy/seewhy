"""
Universal AR — exp17: 4-vs-9 with SHARED positions (alignment test).

exp15 showed the 4v9 label loss pinned at exactly ln(2)=0.6931 with train accuracy
also at chance — the model cannot FIT the matching task, while pixel completion in
the same run trains fine. Hypothesis: query and support each observe a DIFFERENT
random half of the 784 positions (sharing only ~196), so comparing two samples
requires per-position alignment across ~196 positions for each support sample —
too much for 4 layers, leaving only gross ink statistics (which separate 0/1 but
not 4/9).

This removes the alignment problem: every sample in an episode observes the SAME
pixel positions, so matching becomes a direct position-by-position comparison.
Everything else identical to exp15 (4v9, anonymised labels, full diagnostics).

  learns  → alignment across differing position subsets was the bottleneck
  fails   → the limit is fine-grained visual matching itself, not alignment
"""
...
def build_train(Xs, ys, rng):
    ...
    for b in range(MICRO_BATCH):
        ...
        sel_shared = rng.permutation(n_pool)      # SHARED positions: every sample sees the same pixels
        ...
        for k, j in enumerate(si):
            t = _emit(arrs, b, t, Xs[j], int(perm[int(ys[j])]), int(refs[k]), True, True, pool, sel_shared)
        for k, j in enumerate(qi):
            cls = int(ys[j])
            t = _emit(arrs, b, t, Xs[j], int(perm[cls]), int(refs[N_SUP + k]), False,
                      cls in sup_classes, pool, sel_shared)
...
def build_eval_bal(Xtr, ytr, cls_idx, Xq, yq, rng):
    ...
    for b in range(MICRO_BATCH):
        ...
        for k, (img, cls, given) in enumerate(reps + qry):
            t = _emit(arrs, b, t, img, int(perm[cls]), int(refs[k]), given, True, pool, sel_shared)
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 SHARED positions (alignment test)",
