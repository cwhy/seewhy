...
def _perm(rng):
    """Per-episode class→label-token permutation (identity if anonymisation is off)."""
    return rng.permutation(N_CLASSES) if ANON_LABELS else np.arange(N_CLASSES)
...
if __name__ == "__main__":
    ...
    logging.info(f"ANON_LABELS={ANON_LABELS} — per-episode class→label-token permutation; "
                 f"chance 0.10; fixed-label control (exp10) = 0.828")
    ...
    row = {"experiment": EXP_NAME, "name": "ANONYMISED labels (per-episode permutation) — true in-context-learning test",
