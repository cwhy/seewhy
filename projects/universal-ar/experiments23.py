"""
Universal AR — exp22: MLP COMBINER token embedding (conjunctive), 4 vs 9.

Why. exp20/21 split the failure cleanly:

    task                        4v9      0v1
    RETRIEVAL  label / pixel  1.000/1.000  1.000/1.000
    GENERALISE label          0.492        0.992

Addressing and copying are perfect; the ONLY broken step is deciding which support
sample a query resembles — and it breaks exactly when gross statistics are
insufficient (4v9) and works when they suffice (0v1).

The mechanism. To compare two samples the model must aggregate each into a summary,
and attention aggregates by SUMMING token embeddings. With additive embeddings

    token = pos_emb[p] + val_emb[v] + ref_emb[r]
    sum   = Σ pos_emb + Σ val_emb + N·ref_emb

positions and values are summed SEPARATELY, so a summary encodes "which positions
were observed" and "which values occurred" but never "which value at which
position". 0-vs-1 differ in the value marginal (a 1 has far less ink); 4-vs-9 differ
only in the conjunction — which is destroyed. Retrieval is unaffected because it is
a direct (pos, ref) address match that needs no summary. This single mechanism
predicts every observation, including why shared positions (exp17), more depth
(exp18) and k-shot support (exp19) all failed to move it.

The fix. Make the token embedding CONJUNCTIVE with adequate capacity:

    token = MLP( concat[ pos_emb[p], val_emb[v], ref_emb[r] ] )

A linear map on the concatenation is algebraically identical to the additive
embedding, so the GELU nonlinearity is doing the real work. This also revisits the
earlier cross-product (TPR) idea, which was right in spirit but failed on a
field-cardinality bottleneck (d_field=8 cannot separate 785 positions); the MLP
keeps full dimensionality per field.

Prediction: 4v9 generalisation improves while retrieval stays at 1.000.
Base = exp20 (4v9, anonymised labels, retrieval-only data, full diagnostics).
"""
...
TASK_DIGITS = (0, 1)                   # EASY pair — control: must STAY perfect with the combiner
...
def init(key, Dm, L):
    g = jax.random.split(key, 3 + L * 6 + 7); i = iter(g)
    ...
    p = {"pos_emb": jax.random.normal(next(i), (N_POS, Dm)) * 0.02,
         ...
         "ref_emb": jax.random.normal(next(i), (V_REFS, Dm)) * 0.02,
         # MLP combiner: concat(pos,val,ref) -> GELU MLP -> d_model. The nonlinearity is the
         # whole point: a LINEAR map on the concatenation is algebraically identical to the
         # additive embedding, so only a nonlinearity can form position x value conjunctions.
         "cmb_W1": lin(next(i), (3 * Dm, 2 * Dm)), "cmb_b1": jnp.zeros(2 * Dm),
         "cmb_W2": lin(next(i), (2 * Dm, Dm)), "cmb_b2": jnp.zeros(Dm), "layers": []}
    for _ in range(L):
...
def forward(p, pos, val, ref):
    pe = onehot_mm(pos, p["pos_emb"], N_POS)
    ve = onehot_mm(val, p["val_emb"], N_VAL)
    re_ = onehot_mm(ref, p["ref_emb"], V_REFS)
    h = jnp.concatenate([pe, ve, re_], axis=-1)                       # (B,N,3D)
    x = jax.nn.gelu(h @ p["cmb_W1"] + p["cmb_b1"]) @ p["cmb_W2"] + p["cmb_b2"]
    for Lp in p["layers"]:
        x = _layer(Lp, x)
...
if __name__ == "__main__":
    ...
    row = {"experiment": EXP_NAME, "name": f"4v9 MLP-combiner (conjunctive) token embedding + retrieval data",
