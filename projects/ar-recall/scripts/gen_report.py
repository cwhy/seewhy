...
so this is idempotent and a partial regeneration only touches what moved.
    ...
    uv run --no-sync python projects/ar-recall/scripts/gen_report.py
    uv run --no-sync python projects/ar-recall/scripts/gen_report.py --into report
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Regenerate ar-recall figures from results.jsonl")
