...
def init_params(key_gen, input_dim: int, embedding_dim: int, bottleneck_dim: int) -> Dict[str, Array]:
    """
    Initialize parameters for an MLP-based reconstruction model.

    - sample_hidden_matrix: first linear layer, maps flattened image -> hidden (d_x, bottleneck_dim)
    - sample_embedding_matrix: second linear layer, maps quantized hidden -> sample embedding (bottleneck_dim, d_emb)
    - position_embeddings: per-position embedding (d_x, d_emb), using 1D pixel index
    - mlp_w1, mlp_b1: first MLP layer over concatenated (sample_emb, pos_emb)
    - mlp_w2, mlp_b2: second MLP layer producing per-position logits
    """
    scale_input = compute_weight_scale(input_dim)
    scale_bottleneck = compute_weight_scale(bottleneck_dim)
    scale_mlp_in = compute_weight_scale(2 * embedding_dim)
    scale_mlp_hidden = compute_weight_scale(bottleneck_dim)

    key_hidden = next(key_gen).get()
    key_sample = next(key_gen).get()
    key_pos = next(key_gen).get()
    key_mlp1 = next(key_gen).get()
    key_mlp2 = next(key_gen).get()
    ...
    # MLP that acts on concatenated [sample_emb, pos_emb] per position
    mlp_w1 = (
        jax.random.normal(key_mlp1, (2 * embedding_dim, bottleneck_dim)) * scale_mlp_in
    )
    mlp_b1 = jnp.zeros((bottleneck_dim,))
    mlp_w2 = (
        jax.random.normal(key_mlp2, (bottleneck_dim, 1)) * scale_mlp_hidden
    )
    mlp_b2 = jnp.zeros((1,))
...
def forward(params: Dict[str, Array], X: Array) -> Array:
    ...
    # shape: (batch, d_x) @ (d_x, bottleneck_dim) -> (batch, bottleneck_dim)
    ...
    # shape: (batch, bottleneck_dim) @ (bottleneck_dim, d_emb) -> (batch, d_emb)
    sample_emb = jnp.dot(hidden_q, params["sample_embedding_matrix"])

    # Position embeddings: learned per 1D pixel position, shape (d_x, d_emb)
    pos_emb = params["position_embeddings"]

    batch_size = X.shape[0]
    n_positions = pos_emb.shape[0]
    d_emb = sample_emb.shape[1]

    # Broadcast sample embedding over positions: (batch, 1, d_emb) -> (batch, d_x, d_emb)
    sample_broadcast = jnp.broadcast_to(
        sample_emb[:, None, :], (batch_size, n_positions, d_emb)
    )
    # Broadcast position embeddings over batch: (1, d_x, d_emb) -> (batch, d_x, d_emb)
    pos_broadcast = jnp.broadcast_to(
        pos_emb[None, :, :], (batch_size, n_positions, d_emb)
    )

    # Concatenate along the embedding dimension: (batch, d_x, 2 * d_emb)
    pair = jnp.concatenate([sample_broadcast, pos_broadcast], axis=-1)

    # Flatten positions into batch dimension for MLP: (batch * d_x, 2 * d_emb)
    pair_flat = pair.reshape(batch_size * n_positions, 2 * d_emb)

    # Two-layer MLP to produce per-position logits
    h1 = jnp.dot(pair_flat, params["mlp_w1"]) + params["mlp_b1"]
    h1 = jax.nn.relu(h1)
    logits_flat = jnp.dot(h1, params["mlp_w2"]) + params["mlp_b2"]

    # Reshape back to (batch, d_x)
    logits = logits_flat.reshape(batch_size, n_positions)
...
if __name__ == "__main__":
    ...
    X_train = normalize_pixels(fashion_dataset.X)
