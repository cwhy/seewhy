"""
exp_mlp2_ema_1024 — deeper MLP encoder + EMA + augmentation, LATENT_DIM=1024.

3-layer MLP encoder (2 hidden layers, wider):
  x_norm (784,) → gelu(W1) → h1 (1024,) → gelu(W2) → h2 (1024,) → W3 → embedding (LATENT_DIM,)

Everything else identical to exp_mlp_ema (4 actions, FiLM predictor, EMA τ=0.996).

Usage:
    uv run python projects/ssl/exp_mlp2_ema_1024.py
"""
...
LATENT_DIM      = 1024
MLP_ENC_H1      = 1024   # encoder hidden layer 1
MLP_ENC_H2      = 1024   # encoder hidden layer 2
...
if __name__ == "__main__":
    ...
    logging.info(
        f"LATENT_DIM={LATENT_DIM}  MLP_ENC_H1={MLP_ENC_H1}  MLP_ENC_H2={MLP_ENC_H2}"
        f"  EMA_TAU={EMA_TAU}  N_ACTIONS={N_ACTIONS}  encoder=mlp2"
    )
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-mlp2-ema aug LATENT={LATENT_DIM} h1={MLP_ENC_H1} h2={MLP_ENC_H2}",
