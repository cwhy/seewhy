"""
exp2 — LeWM JEPA on MNIST: random masking instead of fixed bottom-half mask.

vs exp1: MASK changes from fixed bottom-half to per-batch random 50% pixel mask.
  A fresh random mask is sampled each batch, so the encoder never sees a
  consistent spatial pattern — forcing it to build a more holistic embedding.
...
Usage:
    uv run python projects/ssl/experiments2.py
"""
...
MASK_RATIO = 0.5    # fraction of pixels zeroed per batch
...
def total_loss(params, x, key):
    ...
    k_mask, k_sigreg = jax.random.split(key)
    # Per-batch random mask: same mask applied to all images in the batch
    mask = jax.random.bernoulli(k_mask, MASK_RATIO, shape=(784,))   # (784,) True = zeroed
    x_masked = x * (1.0 - mask.astype(jnp.float32))[None, :]
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-gram mask=random50% LATENT={LATENT_DIM} LAMBDA={LAMBDA}",
