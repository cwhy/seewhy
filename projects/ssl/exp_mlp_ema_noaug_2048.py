"""
exp_mlp_ema_noaug_2048 — MLP encoder + EMA + NO augmentation, LATENT_DIM=2048.

Replaces the Hebbian gram + Hadamard readout with a simple 2-layer MLP encoder:
  x_norm (784,) → gelu(W1) → h (MLP_ENC_HIDDEN,) → W2 → embedding (LATENT_DIM,)

Everything else identical to exp_ema (4 actions, FiLM predictor, EMA τ=0.996).
Comparison baseline for the gram encoder.

Usage:
    uv run python projects/ssl/exp_mlp_ema_noaug_2048.py
"""
...
EXP_NAME = "exp_mlp_ema_noaug_2048"
...
LATENT_DIM      = 2048
MLP_ENC_HIDDEN  = 512   # encoder hidden size
...
if __name__ == "__main__":
    ...
    logging.info(
        f"LATENT_DIM={LATENT_DIM}  MLP_ENC_HIDDEN={MLP_ENC_HIDDEN}"
        f"  EMA_TAU={EMA_TAU}  N_ACTIONS={N_ACTIONS}  encoder=mlp"
    )
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-mlp-ema aug LATENT={LATENT_DIM} enc_hidden={MLP_ENC_HIDDEN}",
