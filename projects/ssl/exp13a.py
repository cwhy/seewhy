...
vs exp11a (96.4%): scale LATENT_DIM 1024→2048. W_enc_A/W_enc_B project DEC_RANK²=1024→2048.
...
LATENT_DIM   = 2048
DEC_RANK     = 32    # DEC_RANK² = 1024 → projected to LATENT_DIM=2048
...
def init_params(key_gen):
    ...
    return {
        ...
        "W_enc"       : n((D, DEC_RANK),               scale=D**-0.5),         # (144, 32)
        "W_enc_A"     : n((DEC_RANK**2, LATENT_DIM),   scale=(DEC_RANK**2)**-0.5),  # (1024, 1024)
        "W_enc_B"     : n((DEC_RANK**2, LATENT_DIM),   scale=(DEC_RANK**2)**-0.5),  # (1024, 1024)
        # ── Predictor ─────────────────────────────────────────────────────────
...
def encode(params, x):
    """
    Hebbian gram + Hadamard readout.  x: (B, 784) float32 raw pixels → (B, LATENT_DIM).

    Builds the same Hebbian gram H = FᵀF as exp10a/exp11b, then replaces the
    MLP readout with two parallel linear projections + hadamard product:
      a = H @ W_enc_A   (B, LATENT_DIM)
      b = H @ W_enc_B   (B, LATENT_DIM)
      h = a ⊙ b         (B, LATENT_DIM)
    DEC_RANK²=1024=LATENT_DIM so it aligns exactly.
    """
    ...
    # Hebbian matrix: H = FᵀF
    F = x_norm[:, :, None] * f[None, :, :]                                    # (B, 784, DEC_RANK)
    H = jnp.einsum("bpi,bpj->bij", F, F).reshape(x.shape[0], DEC_RANK ** 2)  # (B, 1024)

    # Hadamard readout: two projections, element-wise product
    a = H @ params["W_enc_A"]   # (B, LATENT_DIM)
    b = H @ params["W_enc_B"]   # (B, LATENT_DIM)
    return a * b                 # (B, LATENT_DIM)
...
if __name__ == "__main__":
    ...
    logging.info(
        f"LATENT_DIM={LATENT_DIM}  DEC_RANK={DEC_RANK}  encoder=hebbian-gram+hadamard-readout"
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian+hadamard noise_std={NOISE_STD} LATENT={LATENT_DIM} DEC_RANK={DEC_RANK}",
