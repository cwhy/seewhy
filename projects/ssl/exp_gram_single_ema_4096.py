"""
exp_gram_single_ema_4096 — Gram encoder + single-projection Hadamard + EMA, LATENT=4096.

Ablation of exp_ema: removes W_enc_A and W_enc_B (two projections) and replaces
with a single W_proj that is Hadamard-multiplied with the raw flattened gram H.

Original Hadamard readout:
  a = H_flat @ W_enc_A  →  (B, LATENT)
  b = H_flat @ W_enc_B  →  (B, LATENT)
  output = a ⊙ b

Single-projection Hadamard:
  z = H_flat @ W_proj   →  (B, LATENT)
  output = z ⊙ H_flat

Requires LATENT_DIM = DEC_RANK² (so shapes match for Hadamard).
Run at: LATENT=256 (DEC_RANK=16), LATENT=1024 (DEC_RANK=32), LATENT=4096 (DEC_RANK=64).

Everything else identical to exp_ema (positional encoding, EMA, FiLM predictor).

Usage:
    uv run python projects/ssl/exp_gram_single_ema_4096.py
"""
...
EXP_NAME = "exp_gram_single_ema_4096"
...
# LATENT_DIM must equal DEC_RANK²
LATENT_DIM   = 4096
DEC_RANK     = 64    # DEC_RANK² = 4096 = LATENT_DIM
...
def init_params(key_gen):
    ...
    return {
        # ── Encoder (Hebbian gram + single-projection Hadamard) ────────────────
        ...
        # Single projection matrix (replaces W_enc_A + W_enc_B)
...
# ── Encoder (Gram + single-projection Hadamard) ───────────────────────────────

def encode(params, x):
    """Gram + single W_proj Hadamard.  x: (B, 784) → (B, LATENT_DIM=DEC_RANK²)."""
    x_norm   = x / 255.0
    ...
    # Single projection Hadamard: z ⊙ H  (shapes match since LATENT = DEC_RANK²)
    z = H @ params["W_proj"]                                           # (B, LATENT_DIM)
    return z * H                                                        # (B, LATENT_DIM)
...
if __name__ == "__main__":
    assert LATENT_DIM == DEC_RANK ** 2, f"LATENT_DIM must equal DEC_RANK²: {LATENT_DIM} != {DEC_RANK**2}"
    ...
    logging.info(f"LATENT_DIM={LATENT_DIM}  DEC_RANK={DEC_RANK}  encoder=gram_single")
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-gram-single-ema aug LATENT={LATENT_DIM} DEC_RANK={DEC_RANK}",
