"""
exp6 — LeWM JEPA on MNIST: Hebbian gram encoder.

vs exp5c (92.9%): replace outer(Σ x·fA, Σ x·fB) with the true Hebbian matrix
  H = Fᵀ F where F[p] = x[p]/255 * f[p].
  Each pixel is a data point; H[i,j] accumulates co-activation of feature i and j
  across all 784 pixels — one shared projection W_enc instead of two (W_enc_A, W_enc_B).
...
Usage:
    uv run python projects/ssl/exp6.py
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian mask=random50% LATENT={LATENT_DIM} DEC_RANK={DEC_RANK} MLP_HIDDEN={MLP_HIDDEN}",
