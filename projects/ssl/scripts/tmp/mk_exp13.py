"""
Generate experiments 13a–13b.

exp11a baseline: Hebbian gram + Hadamard readout, DEC_RANK=32, LATENT_DIM=1024 → 96.4%

  exp13a: same, scale LATENT_DIM 1024→2048
  exp13b: same encoder, two actions (denoise σ=75 / mask 50%), FiLM predictor
"""
...
# ── exp13a: based on exp11a, LATENT_DIM 1024→2048 ─────────────────────────────
src = (SSL / "exp11a.py").read_text()
text = src
text = text.replace(
...
text = text.replace("    uv run python projects/ssl/exp11a.py", "    uv run python projects/ssl/exp13a.py")
text = text.replace('EXP_NAME = "exp11a"', 'EXP_NAME = "exp13a"')
...
(SSL / "exp13a.py").write_text(text)
print("wrote exp13a.py")

# ── exp13b: Hebbian gram + Hadamard + FiLM two-action (noise75 / mask50) ───────
exp13b = '''\
"""
...
Usage:
    uv run python projects/ssl/exp13b.py
"""
...
(SSL / "exp13b.py").write_text(exp13b)
print("wrote exp13b.py")
