"""
exp_conv2_ema_1024 — improved ConvNet encoder + EMA, LATENT_DIM=1024.

Fixes from exp_conv_ema:
  - Wider channels: 64 → 128 → 256 (was 32/64/128)
  - Added FC_HIDDEN=512 intermediate layer between GAP and output
    (avoids direct 256→LATENT projection bottleneck)

Architecture:
  (B,784) → reshape (B,28,28,1)
  → Conv(64,3×3)+GeLU → MaxPool(2×2)     [14×14×64]
  → Conv(128,3×3)+GeLU → MaxPool(2×2)    [7×7×128]
  → Conv(256,3×3)+GeLU → GlobalAvgPool   [256]
  → Linear(FC_HIDDEN)+GeLU → Linear(LATENT_DIM)

Usage:
    uv run python projects/ssl/exp_conv2_ema_1024.py
"""
...
LATENT_DIM      = 1024
CONV_CH         = [64, 128, 256]   # wider channels
FC_HIDDEN       = 512              # intermediate FC between GAP and output
MLP_PRED_HIDDEN = 512
...
def encode(params, x):
    ...
    h = h.mean(axis=(1, 2))                              # global avg pool → (B, 256)
...
if __name__ == "__main__":
    ...
    logging.info(f"LATENT_DIM={LATENT_DIM}  conv_ch={CONV_CH}  fc_hidden={FC_HIDDEN}  encoder=conv2")
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-conv2-ema aug LATENT={LATENT_DIM} ch={CONV_CH}",
