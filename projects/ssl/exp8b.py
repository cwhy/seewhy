"""
exp8b — LeWM JEPA on MNIST: fixed multi-noise (all levels per image per batch).

vs exp8 (95.0%, random per-image): instead of sampling one noise level per image,
  apply ALL 4 noise levels to every image in the batch. The batch is expanded 4×
  (B → B*N_ACTIONS), each copy noised at a different level with its action embedding.
  Every image contributes equally to every noise level — no sampling variance.
...
Context (prev)  = each image noised at all 4 levels simultaneously
Target  (after) = clean MNIST image (shared across all 4 noisy copies)

Architecture:
  encoder        (Hebbian gram): image (784,) → embedding (LATENT_DIM,)
  action_encoder (lookup table): noise_level_idx → action_emb (ACTION_DIM,)
  predictor      (MLP)         : concat(noisy_emb, action_emb) → clean_emb
...
Usage:
    uv run python projects/ssl/exp8b.py
"""
...
# Discrete noise levels — each is a named "action": denoise_gaussian_25, _50, _75, _100
NOISE_STDS   = jnp.array([25.0, 50.0, 75.0, 100.0])
ACTION_LABELS = ["denoise_gaussian_25", "denoise_gaussian_50",
                 "denoise_gaussian_75", "denoise_gaussian_100"]
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── Action encoder (noise-level embedding table) ───────────────────────
        "action_emb"  : n((N_ACTIONS, ACTION_DIM), scale=ACTION_DIM**-0.5),
        # ── Predictor (conditioned on action via concatenation) ────────────────
        "W_pred1"     : n((LATENT_DIM + ACTION_DIM, MLP_HIDDEN), scale=(LATENT_DIM + ACTION_DIM)**-0.5),
...
def predict(params, e, a):
    ...
    e: (B, LATENT_DIM), a: (B, ACTION_DIM) → predicted embedding: (B, LATENT_DIM).
    """
    h = jax.nn.gelu(jnp.concatenate([e, a], axis=-1) @ params["W_pred1"] + params["b_pred1"])
...
def total_loss(params, x, key):
    ...
    Expands each batch 4× by applying every noise level to every image:
      x_exp[b*N_ACTIONS + k] = x[b] + noise ~ N(0, NOISE_STDS[k]²)
    This ensures balanced, deterministic coverage of all noise levels.
    """
    k_noise, k_sigreg = jax.random.split(key)
    B = x.shape[0]

    # Expand: repeat each image N_ACTIONS times → (B*N_ACTIONS, 784)
    x_exp     = jnp.repeat(x, N_ACTIONS, axis=0)                          # (B*N_ACTIONS, 784)
    stds      = jnp.tile(NOISE_STDS, B)                                    # (B*N_ACTIONS,)
    noise     = jax.random.normal(k_noise, x_exp.shape) * stds[:, None]
    x_noisy   = jnp.clip(x_exp + noise, 0.0, 255.0)

    e_full    = encode(params, x)                                          # (B, LATENT_DIM)
    e_noisy   = encode(params, x_noisy)                                    # (B*N_ACTIONS, LATENT_DIM)
    act_idx   = jnp.tile(jnp.arange(N_ACTIONS), B)                        # (B*N_ACTIONS,)
    a_emb     = params["action_emb"][act_idx]                              # (B*N_ACTIONS, ACTION_DIM)
    e_pred    = predict(params, e_noisy, a_emb)                            # (B*N_ACTIONS, LATENT_DIM)

    e_full_exp = jnp.repeat(e_full, N_ACTIONS, axis=0)                    # (B*N_ACTIONS, LATENT_DIM)
    pred_loss  = ((e_pred - e_full_exp) ** 2).mean()
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian action-fixed-multinoise LATENT={LATENT_DIM} DEC_RANK={DEC_RANK} MLP_HIDDEN={MLP_HIDDEN} ACTION_DIM={ACTION_DIM}",
