"""
exp8 — LeWM JEPA on MNIST: multi-level noise with action-conditioned predictor.

vs exp7c (95.4%, σ=75): instead of a single fixed noise level, sample a noise level
  per image from NOISE_STDS = [25, 50, 75, 100] (discrete "actions").
  The predictor is conditioned on an action embedding encoding which noise level
  was applied — it learns to denoise_gaussian_25 / _50 / _75 / _100 simultaneously.
...
Usage:
    uv run python projects/ssl/exp8.py
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian action-noise LATENT={LATENT_DIM} DEC_RANK={DEC_RANK} MLP_HIDDEN={MLP_HIDDEN} ACTION_DIM={ACTION_DIM}",
