"""
exp_masked_distill_mlp — MLP-shallow student + frozen ConvNet-v2 DAE teacher.

Training: masked distillation.
  - Teacher: exp_dae_conv2 encoder (frozen, ConvNet-v2 DAE, 97.96% probe acc)
  - Student: MLP-shallow encoder + FiLM predictor (same as EMA-JEPA)
  - Loss: MSE(predictor(student(aug_x), action), stop_grad(teacher(clean_x)))
  - Augmentations: Gaussian σ=75, pixel mask 25/50/75% (same 4 as EMA)

Unlike EMA-JEPA there is no momentum target encoder — the teacher is fixed.
Simpler training loop: no target_params in carry, no EMA update.

Usage:
    uv run python projects/ssl/exp_masked_distill_mlp.py
"""
...
EXP_NAME = "exp_masked_distill_mlp"
...
if __name__ == "__main__":
    ...
    logging.info(
        f"LATENT_DIM={LATENT_DIM}  enc_hidden={MLP_ENC_HIDDEN}  pred_hidden={MLP_PRED_HIDDEN}"
        f"  N_ACTIONS={N_ACTIONS}  teacher=exp_dae_conv2"
    )
    ...
    append_result({
        ...
        "enc_hidden"      : MLP_ENC_HIDDEN,
        "pred_hidden"     : MLP_PRED_HIDDEN,
        "n_actions"       : N_ACTIONS,
        "noise_std"       : NOISE_STD,
