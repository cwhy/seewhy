"""
exp8c — LeWM JEPA on MNIST: FiLM + fixed multi-noise (exp8a + exp8b combined).

vs exp8a (95.1%, FiLM + random) and exp8b (95.0%, fixed + concat):
  combines both changes — FiLM-conditioned predictor with all 4 noise levels
  applied to every image per batch (B → B*N_ACTIONS).
...
Architecture:
  ...
  predictor      (FiLM MLP)    : FiLM(gelu(e @ W_pred1), action_emb) → clean_emb
...
Usage:
    uv run python projects/ssl/exp8c.py
"""
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian film+fixed-multinoise LATENT={LATENT_DIM} DEC_RANK={DEC_RANK} MLP_HIDDEN={MLP_HIDDEN} ACTION_DIM={ACTION_DIM}",
