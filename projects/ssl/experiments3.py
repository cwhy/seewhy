"""
exp3 — LeWM JEPA on MNIST: reduce mask ratio from 50% to 25%.

vs exp2: MASK_RATIO 0.5 → 0.25. More pixels visible per batch; easier prediction
  task but potentially richer context for the encoder.
...
Usage:
    uv run python projects/ssl/experiments3.py
"""
...
MASK_RATIO = 0.25   # fraction of pixels zeroed per batch
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-gram mask=random25% LATENT={LATENT_DIM} LAMBDA={LAMBDA}",
