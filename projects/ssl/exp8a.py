"""
exp8a — LeWM JEPA on MNIST: FiLM-conditioned predictor.

vs exp8 (95.0%, concat): replace concatenation with FiLM modulation.
  The action embedding produces per-channel scale and shift for the predictor's
  hidden layer: h = gelu(e @ W_pred1) * (1 + scale(a)) + shift(a).
  This lets the action steer the predictor without inflating its input width.
...
Context (prev)  = MNIST image with per-sample Gaussian noise (level sampled from NOISE_STDS)
Target  (after) = clean MNIST image
...
Usage:
    uv run python projects/ssl/exp8a.py
"""
...
def init_params(key_gen):
    ...
    return {
        ...
        "b_film_s"    : jnp.zeros(MLP_HIDDEN),        # scale: h *= (1 + film_s(a))
        "W_film_t"    : n((ACTION_DIM, MLP_HIDDEN),   scale=ACTION_DIM**-0.5),
        "b_film_t"    : jnp.zeros(MLP_HIDDEN),        # shift: h += film_t(a)
...
def predict(params, e, a):
    ...
    FiLM: scale and shift the hidden activations with action-derived affine params.
      h = gelu(e @ W_pred1 + b_pred1)
      h = h * (1 + a @ W_film_s + b_film_s) + (a @ W_film_t + b_film_t)
    """
    h     = jax.nn.gelu(e @ params["W_pred1"] + params["b_pred1"])      # (B, MLP_HIDDEN)
    scale = a @ params["W_film_s"] + params["b_film_s"]                  # (B, MLP_HIDDEN)
    shift = a @ params["W_film_t"] + params["b_film_t"]                  # (B, MLP_HIDDEN)
    h     = h * (1.0 + scale) + shift
...
def total_loss(params, x, key):
    ...
    k_idx, k_noise, k_sigreg = jax.random.split(key, 3)
    B = x.shape[0]

    # Per-image: sample a noise level index ~ Uniform{0,...,N_ACTIONS-1}
    idx      = jax.random.randint(k_idx, (B,), 0, N_ACTIONS)       # (B,)
    std      = NOISE_STDS[idx]                                       # (B,)
    noise    = jax.random.normal(k_noise, x.shape) * std[:, None]   # (B, 784)
    x_noisy  = jnp.clip(x + noise, 0.0, 255.0)

    e_full   = encode(params, x)                                     # (B, LATENT_DIM)
    e_noisy  = encode(params, x_noisy)                               # (B, LATENT_DIM)
    a_emb    = params["action_emb"][idx]                             # (B, ACTION_DIM)
    e_pred   = predict(params, e_noisy, a_emb)                       # (B, LATENT_DIM)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-hebbian action-film LATENT={LATENT_DIM} DEC_RANK={DEC_RANK} MLP_HIDDEN={MLP_HIDDEN} ACTION_DIM={ACTION_DIM}",
