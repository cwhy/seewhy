"""
exp_distill_chain_dae_arch — DAE-seeded same-architecture distillation chain.

For each architecture, starts from a DAE-trained teacher and trains successive
distillation hops (all same architecture).

  hop 0: DAE teacher              (eval only — no training)
  hop 1: DAE teacher → student1   (same arch)
  hop 2: student1 → student2      (same arch)
  ...
  hop 10: student9 → student10    (same arch)

Architectures: mlp2, vit_patch, gram_dual, gram_single, gram_glu

Results go to results_distill_chain_dae_arch.jsonl.
Skip-if-done on (arch, hop) so the script is safe to restart.

Usage:
    uv run python projects/ssl/exp_distill_chain_dae_arch.py
"""
...
INIT_FNS = {
    "mlp2": init_params_mlp2, "vit_patch": init_params_vit_patch,
    "gram_dual": init_params_gram_dual, "gram_single": init_params_gram_single,
    "gram_glu": init_params_gram_glu,
}

# ── Student encode functions (DEC_RANK=32 for Gram) ───────────────────────────
...
ENCODE_FNS = {
    "mlp2": encode_mlp2, "vit_patch": encode_vit_patch,
    "gram_dual": encode_gram_dual, "gram_single": encode_gram_single,
    "gram_glu": encode_gram_glu,
}

# ── DAE teacher encode functions (infer rank dynamically for Gram) ─────────────
# These are used for hop 0 eval and hop 1 teacher side.
# Gram-dual DAE was trained with DEC_RANK=64; others match student structure.
...
def teacher_encode_gram_dual(params, x):
    ...
    F        = (pos_rc @ params["W_pos_enc"] @ params["W_enc"])
    F        = x_norm[:,:,None] * F[None,:,:]
    dr       = params["W_enc"].shape[1]   # infer rank (DAE=64, student=32)
...
def make_epoch_fn(optimizer, encode_fn, teacher_params, use_patch_mask=False,
                  teacher_encode_fn=None):
    if teacher_encode_fn is None:
        teacher_encode_fn = encode_fn
...
if __name__ == "__main__":
    ...
    for arch in ARCHS:
        encode_fn         = ENCODE_FNS[arch]
        teacher_encode_fn = TEACHER_ENCODE_FNS[arch]

        # ── Load DAE teacher params ────────────────────────────────────────────
        pkl_path = SSL / TEACHER_PKLS[arch]
        logging.info(f"[{arch}] loading DAE teacher from {pkl_path.name}")
        dae_params = {k: jnp.array(v)
                      for k, v in pickle.load(open(pkl_path, "rb")).items()}

        # ── hop 0: evaluate DAE teacher ────────────────────────────────────────
        if (arch, 0) not in done:
            logging.info(f"[{arch}] hop 0: evaluating DAE teacher")
            probe = linear_probe_accuracy(teacher_encode_fn, dae_params,
                                          X_train, Y_train, X_test, Y_test)
            knn   = knn_1nn_accuracy(teacher_encode_fn, dae_params,
                                     X_train, Y_train, X_test, Y_test)
            ...
            append_result({"arch": arch, "hop": 0, "probe_acc": round(probe, 6),
                           ...
                           "epochs": 0, "latent_dim": LATENT_DIM, "teacher_type": "dae"})
            done.add((arch, 0))

        # ── hops 1-N_HOPS: chain distillation ─────────────────────────────────
        # hop 1: teacher is DAE (use teacher_encode_fn)
        # hop 2+: teacher is previous student (use encode_fn)
        current_teacher = dae_params
        current_teacher_encode = teacher_encode_fn

        for hop in range(1, N_HOPS + 1):
            hop_pkl_path = SSL / f"params_distill_chain_dae_{arch}_hop{hop}.pkl"

            if (arch, hop) in done:
                logging.info(f"  [{arch} hop{hop}] already done — loading params")
                current_teacher = {k: jnp.array(v)
                                   for k, v in pickle.load(open(hop_pkl_path, "rb")).items()}
                current_teacher_encode = encode_fn   # hop 2+ uses student encode
                continue

            t_type = "dae" if hop == 1 else f"student_hop{hop-1}"
            logging.info(f"[{arch}] hop {hop}: training with {t_type} teacher")
            new_params, probe, knn, elapsed = train_hop(
                arch, hop, current_teacher, X_train, Y_train, X_test, Y_test,
                teacher_encode_fn=current_teacher_encode)

            with open(hop_pkl_path, "wb") as f:
            ...
            current_teacher_encode = encode_fn   # subsequent hops use student encode
