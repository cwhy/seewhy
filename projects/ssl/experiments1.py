"""
exp1 — LeWM JEPA on MNIST: predict complete-image embedding from masked-image embedding.
...
Context (prev)  = top half of MNIST image (rows 0–13; bottom 14 rows zeroed)
...
SIGReg (Maes et al. 2026) — Epps-Pulley test statistic.
  Matches empirical characteristic function of embeddings to N(0, I):
    err(t, w) = (E_z[cos(t · w᷊z)] − e^{−t²/2})² + E_z[sin(t · w᷊z)]²
  Integrated with trapezoidal quadrature weighted by a Gaussian window.
  Prevents representational collapse without an EMA target encoder.

Eval: linear-probe accuracy on frozen embeddings of complete test images.

Usage:
    uv run python projects/ssl/experiments1.py
"""
...
MASK = ROWS >= 14   # (784,) bool: True = bottom-half pixel (zeroed in prev)
...
def init_params(key_gen):
    ...
    return {
        ...
        "W_enc_A"     : n((D, DEC_RANK),           scale=D**-0.5),      # (144, 16)
        "W_enc_B"     : n((D, DEC_RANK),           scale=D**-0.5),      # (144, 16)
...
def encode(params, x):
    """
    Gram-style encoder.  x: (B, 784) float32 raw pixels → embedding: (B, LATENT_DIM).

    Per-pixel positional features are pixel-value-weighted and aggregated into
    two gram factors whose outer product forms the gram matrix — mirroring the
    gram matrix used in the imle-gram decoder.
    """
    x_norm = x / 255.0   # (B, 784)

    # Per-pixel positional features: (784, D)
    ...
    # Two positional projection maps: (784, DEC_RANK) each
    pix_A = pos_feat @ params["W_enc_A"]   # (784, DEC_RANK)
    pix_B = pos_feat @ params["W_enc_B"]   # (784, DEC_RANK)

    # Pixel-value-weighted sums: (B, DEC_RANK)
    A_sum = jnp.einsum("bp,pr->br", x_norm, pix_A)   # (B, DEC_RANK)
    B_sum = jnp.einsum("bp,pr->br", x_norm, pix_B)   # (B, DEC_RANK)

    # Gram matrix (outer product) → flatten to (B, DEC_RANK²)
    gram = jnp.einsum("bi,bj->bij", A_sum, B_sum).reshape(x.shape[0], DEC_RANK ** 2)

    # MLP to embedding
    h = jax.nn.gelu(gram @ params["W_enc1"] + params["b_enc1"])   # (B, MLP_HIDDEN)
...
def total_loss(params, x, key):
    ...
    x_masked = x * (1.0 - MASK.astype(jnp.float32))[None, :]   # zero bottom half
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment"      : EXP_NAME,
        "name"            : f"jepa-gram mask=bottom-half LATENT={LATENT_DIM} LAMBDA={LAMBDA}",
