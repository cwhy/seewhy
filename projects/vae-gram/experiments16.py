"""
exp16 — Gram VAE: larger gram D=196 (D_R=D_C=7), LATENT_DIM=64 locked.

Changes from exp5 (88.7%):
  1. D_R, D_C: 6 → 7. D: 144 → 196. Richer per-pixel tokens, larger gram.
  2. LATENT_DIM=64, FREE_BITS=0.5 locked.

Hypothesis: a larger gram gives the encoder more to work with — it has more
information to select from when compressing into 32 nats.

Usage:
    uv run python projects/vae-gram/experiments16.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp16 — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp16_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp16 — D={D} (D_R={D_R},D_C={D_C},D_V={D_V}) latent={LATENT_DIM}", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp16_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp16.pkl", "wb") as f:
        pickle.dump({k: np.array(v) for k, v in params.items()}, f)
    save_learning_curves(history)
    save_reconstruction_grid(params, X_test, "final")
    final_acc = bin_accuracy(params, X_test)
    append_result({
        "experiment": "exp16", "name": f"gram-vae d={D} (d_r={D_R},d_c={D_C},d_v={D_V}) latent={LATENT_DIM}",
