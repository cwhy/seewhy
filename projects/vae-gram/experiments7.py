"""
exp7 — Gram VAE: z-conditioned position query.

Changes from exp5:
  1. Z-conditioned decoder position query:
       q = W_pos_dec @ pos_rc + W_z_pos @ z
     Previously q was purely position-based (fixed learned vectors). The z correction
     lets the decoder modulate how it reads from the gram per-sample: a "7" queries
     different gram components than a "4" at the same pixel position.
  2. Back to LATENT_DIM=64, FREE_BITS=0.5 (exp6's larger latent hurt — dims collapsed).
  3. All other settings same as exp5.

Usage:
    uv run python projects/vae-gram/experiments7.py
"""
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── decoder: z-conditioned query correction ───────────────────────
        # q = W_pos_dec @ pos_rc + W_z_pos @ z
        # Allows decoder to shift its query direction per-sample.
        "W_z_pos": n((LATENT_DIM, D), scale=LATENT_DIM ** -0.5),
...
def decode(params, z):
    ...
    # z-conditioned query shift: (B, D)
    z_shift = z @ params["W_z_pos"]

    def decode_pixel(r, c, g, zs):
        re     = params["dec_row_emb"][r]
        ce     = params["dec_col_emb"][c]
        pos_rc = (re[:, None] * ce[None, :]).reshape(D_RC)
        q      = params["W_pos_dec"] @ pos_rc + zs  # z-conditioned query
        f      = g @ q                              # (D,)
        h1     = jax.nn.gelu(params["W_val1"] @ f + params["b_val1"])  # (2D,)
        return params["W_val2"] @ h1                                    # (N_VAL_BINS,)

    def decode_image(g, zs):
        return jax.vmap(decode_pixel, in_axes=(0, 0, None, None))(ROWS, COLS, g, zs)

    return jax.vmap(decode_image)(gram, z_shift)
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp7 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp7_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp7 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp7_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp7 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp7_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle("exp7 — gram VAE (z-conditioned position query)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp7_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp7.pkl"
    ...
    row = {
        "experiment":      "exp7",
        "name":            f"gram-vae d={D} z-pos-query 2-layer-dec dec_rank={DEC_RANK}",
