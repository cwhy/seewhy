"""
exp18 — Gram VAE: all encoder improvements combined, LATENT_DIM=64 locked.

Changes from exp5 (88.7%):
  1. N_ENC_Q: 8 → 16  (exp15)
  2. D_R, D_C: 6 → 7, D: 144 → 196  (exp16)
  3. Encoder MLP: feat → GELU(W_enc1) → mu, lv, hidden=1024  (exp17)
  4. LATENT_DIM=64, FREE_BITS=0.5 locked throughout.

Hypothesis: individually each encoder improvement may be marginal within the 32-nat
bottleneck, but combined they give the encoder its best chance of selecting the most
useful information to compress into 64 dims.

Usage:
    uv run python projects/vae-gram/experiments18.py
"""
...
def init_params(key_gen):
    ...
    feat_dim = N_ENC_Q * D  # 16 * 196 = 3136
    ...
    return {
        ...
        "W_enc1": n((feat_dim, ENC_MLP_HIDDEN), scale=feat_dim ** -0.5),
        "b_enc1": jnp.zeros(ENC_MLP_HIDDEN),
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp18 — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp18_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp18 — all enc improvements D={D} N_ENC_Q={N_ENC_Q} enc-MLP latent={LATENT_DIM}", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp18_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp18.pkl", "wb") as f:
    ...
    append_result({
        "experiment": "exp18",
        "name": f"gram-vae all-enc D={D} N_ENC_Q={N_ENC_Q} enc-mlp h={ENC_MLP_HIDDEN} latent={LATENT_DIM}",
