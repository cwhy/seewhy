"""
exp17 — Gram VAE: encoder MLP, LATENT_DIM=64 locked.

Changes from exp5 (88.7%):
  1. Encoder MLP: gram features → GELU(W_enc1 @ feat) → mu, lv. Hidden=1024.
  2. LATENT_DIM=64, FREE_BITS=0.5 locked.

Exp6 tried this but broke it with FREE_BITS=0.3. Exp12 tried it with LATENT_DIM=256.
This cleanly tests encoder MLP with the locked exp5 latent config.

Usage:
    uv run python projects/vae-gram/experiments17.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp17 — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp17_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp17 — enc-MLP h={ENC_MLP_HIDDEN} latent={LATENT_DIM} free_bits={FREE_BITS}", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp17_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp17.pkl", "wb") as f:
        pickle.dump({k: np.array(v) for k, v in params.items()}, f)
    save_learning_curves(history)
    save_reconstruction_grid(params, X_test, "final")
    final_acc = bin_accuracy(params, X_test)
    append_result({
        "experiment": "exp17", "name": f"gram-vae enc-mlp h={ENC_MLP_HIDDEN} d={D} latent={LATENT_DIM}",
