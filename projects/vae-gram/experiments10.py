"""
exp10 — Gram VAE: larger latent (256) with correct free bits.

Changes from exp5:
  1. LATENT_DIM: 64 → 256. Much larger information capacity through the bottleneck.
     With FREE_BITS=0.5, total minimum KL = 256 × 0.5 = 128 nats/image (vs 32 before).
  2. FREE_BITS: kept at 0.5. Critical — exp6 dropped this to 0.3 which caused most
     dims to collapse. Keeping FREE_BITS=0.5 forces all 256 dims to encode signal.
  3. KL_WARMUP_EPOCHS: 30 (from 20). Larger latent needs longer warmup to stabilize.

Hypothesis: the 88.7% ceiling is an information-theoretic limit of the 64-dim VAE
compressing each MNIST image to ~32 nats. LATENT_DIM=256 at 0.5 nats/dim gives
128 nats of capacity, potentially unlocking more accurate reconstructions.

Usage:
    uv run python projects/vae-gram/experiments10.py
"""
...
N_ENC_Q          = 8
LATENT_DIM       = 256   # ↑ from 64
DEC_RANK         = 32
MLP_HIDDEN       = 512

ADAM_LR          = 3e-4
KL_WEIGHT        = 1.0
KL_WARMUP_EPOCHS = 30    # slightly longer warmup for larger latent
FREE_BITS        = 0.5   # KEEP at 0.5 — all 256 dims must encode signal
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp10 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp10_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp10 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp10_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp10 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp10_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp10 — gram VAE (latent={LATENT_DIM}, free_bits={FREE_BITS})", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp10_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp10.pkl"
    ...
    row = {
        "experiment":      "exp10",
        "name":            f"gram-vae d={D} latent={LATENT_DIM} free_bits={FREE_BITS} dec_rank={DEC_RANK}",
