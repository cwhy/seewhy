"""
exp11 — Gram VAE: even larger latent (512).

Changes from exp10 (89.0%):
  1. LATENT_DIM: 256 → 512. 256 nats minimum encoding capacity (vs 128 in exp10).
  2. FREE_BITS: 0.5 (unchanged — all 512 dims must encode signal).

In exp10, all 256 latent dims were at the free-bits floor (KL ≈ 0.5/dim), meaning
the encoder was informationally saturated. If 512 dims also saturate, it confirms
the gram encoder is the bottleneck, not the latent size.

Usage:
    uv run python projects/vae-gram/experiments11.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp11 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp11_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp11 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp11_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp11 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp11_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp11 — gram VAE (latent={LATENT_DIM}, free_bits={FREE_BITS})", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp11_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp11.pkl"
