"""
exp5 — Gram VAE: deeper decoder head + larger gram.

Changes from exp4:
  1. D_R=6, D_C=6, D_V=4 → D=144 (from 125). Larger gram space.
  2. Decoder head: 2-layer MLP after gram query — gram@q → Linear(D,2D) → GELU → Linear(2D,N_BINS).
     Previously a single linear W_val_head. The non-linear hidden layer can model richer
     position-conditional pixel distributions.
  3. DEC_RANK=32 (from 24). Richer pseudo-gram rank.

Usage:
    uv run python projects/vae-gram/experiments5.py
"""
...
LATENT_DIM       = 64
DEC_RANK         = 32
MLP_HIDDEN       = 512   # z → A, B MLP hidden
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── decoder: 2-layer value head (gram@q → hidden → logits) ────────
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp5 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp5_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp5 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp5_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp5 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp5_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle("exp5 — gram VAE (D=144, 2-layer decoder head)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp5_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp5.pkl"
    ...
    row = {
        "experiment":      "exp5",
        "name":            f"gram-vae d={D} n_enc_q={N_ENC_Q} 2-layer-dec dec_rank={DEC_RANK}",
