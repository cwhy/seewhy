"""
exp13 — Gram VAE: stronger decoder (DEC_RANK=64, MLP_HIDDEN=1024).

Changes from exp11 (89.2%):
  1. DEC_RANK: 32 → 64. Pseudo-gram A@B.T now rank-64, doubling decoder capacity.
  2. MLP_HIDDEN: 512 → 1024. Decoder z→h projection is wider.
  3. LATENT_DIM=512, FREE_BITS=0.5 unchanged from exp11.

Hypothesis: with 512 latent dims all saturated at 0.5 nats (256 nats total), the
decoder is now the bottleneck. The pseudo-gram rank=32 may be insufficient to
represent the full variety of MNIST images. DEC_RANK=64 doubles this capacity.
Also, MLP_HIDDEN=512 is a square projection for LATENT_DIM=512 — widening to 1024
gives the decoder MLP more expressiveness.

Usage:
    uv run python projects/vae-gram/experiments13.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp13 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp13_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp13 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp13_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp13 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp13_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp13 — gram VAE dec-rank={DEC_RANK} mlp-h={MLP_HIDDEN} (latent={LATENT_DIM})", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp13_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp13.pkl"
    ...
    row = {
        "experiment":      "exp13",
        "name":            f"gram-vae dec-rank={DEC_RANK} mlp-h={MLP_HIDDEN} latent={LATENT_DIM}",
