"""
exp8 — Gram VAE: longer training + larger batch.

Changes from exp5 (best so far at 88.7%):
  1. MAX_EPOCHS: 100 → 300. The exp5 curve was still visibly improving at ep99.
  2. BATCH_SIZE: 128 → 256. More stable gradient estimates per step.
  3. ADAM_LR: 3e-4 → 4e-4 (slightly higher, matched to larger batch).
  4. DEC_RANK: 32 → 48. Richer pseudo-gram now that training is longer.

Usage:
    uv run python projects/vae-gram/experiments8.py
"""
...
ADAM_LR          = 4e-4  # slightly higher for larger batch
KL_WEIGHT        = 1.0
KL_WARMUP_EPOCHS = 20
FREE_BITS        = 0.5
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp8 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp8_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp8 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp8_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp8 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp8_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle("exp8 — gram VAE (300 epochs, batch=256, dec_rank=48)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp8_learning_curves", fig, format="svg")
    plt.close(fig)
...
def train(params, X_tr, X_te, y_te):
    ...
    schedule    = optax.cosine_decay_schedule(ADAM_LR, num_steps, alpha=0.02)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp8.pkl"
    ...
    row = {
        "experiment":      "exp8",
        "name":            f"gram-vae d={D} 300ep batch={BATCH_SIZE} dec_rank={DEC_RANK}",
