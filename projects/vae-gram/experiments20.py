"""
exp20 — Gram VAE: aggressive minimisation (MLP_HIDDEN=128, DEC_RANK=8, N_ENC_Q=4).

Changes from exp5 (88.7%, 5.0M params, 262s):
  1. MLP_HIDDEN: 512 → 128
  2. DEC_RANK:   32  → 8
  3. N_ENC_Q:    8   → 4
  LATENT_DIM=64, FREE_BITS=0.5 locked.

W_A: 128×(8×144) = 147K (was 2.36M). Expected total: ~430K params.
How far can we shrink before accuracy drops below 88.7%?

Usage:
    uv run python projects/vae-gram/experiments20.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp20 — {tag}", fontsize=10); plt.tight_layout()
    url = save_matplotlib_figure(f"exp20_reconstructions_{tag}", fig, format="png", dpi=150)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp20 — MLP_H={MLP_HIDDEN} DEC_RANK={DEC_RANK} N_ENC_Q={N_ENC_Q} latent={LATENT_DIM}", fontsize=12)
    plt.tight_layout(); url = save_matplotlib_figure("exp20_learning_curves", fig, format="svg")
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp20.pkl", "wb") as f:
        pickle.dump({k: np.array(v) for k, v in params.items()}, f)
    save_learning_curves(history)
    save_reconstruction_grid(params, X_test, "final")
    final_acc = bin_accuracy(params, X_test)
    append_result({
        "experiment": "exp20", "name": f"gram-vae mlp_h={MLP_HIDDEN} dec_rank={DEC_RANK} n_enc_q={N_ENC_Q} latent={LATENT_DIM}",
