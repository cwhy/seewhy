"""
exp9 — Gram VAE: multi-head position queries.

Changes from exp5 (88.7%, best so far):
  1. N_DEC_HEADS=4 independent position query vectors per pixel (from 1).
     Each head has its own W_pos_dec_h, producing f_h = gram @ q_h.
     The 4 responses are concatenated → (4*D,) before the val head.
     Different heads can specialize in different spatial/value features.
  2. W_val1 expands to (2*D, N_DEC_HEADS*D) to handle the concatenated input.
  3. Everything else identical to exp5.

Hypothesis: single position query per pixel can only extract one linear combination
of the rank-DEC_RANK pseudo-gram. Multi-head gives 4 independent reads, richer
per-pixel decoder features.

Usage:
    uv run python projects/vae-gram/experiments9.py
"""
...
DEC_RANK         = 32
MLP_HIDDEN       = 512
N_DEC_HEADS      = 4    # multi-head position query
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── decoder: multi-head position embeddings ───────────────────────
        # Each head has its own (D, D_RC) query matrix.
        "dec_row_emb":  n((28, D_R), scale=D_R ** -0.5),
        "dec_col_emb":  n((28, D_C), scale=D_C ** -0.5),
        "W_pos_dec":    n((N_DEC_HEADS, D, D_RC), scale=D_RC ** -0.5),

        # ── decoder: 2-layer value head (input = N_DEC_HEADS * D) ─────────
        "W_val1": n((2 * D, N_DEC_HEADS * D), scale=(N_DEC_HEADS * D) ** -0.5),
...
def decode(params, z):
    ...
    # Precompute all query vectors: shape (N_DEC_HEADS, D, D_RC) → used as W_h @ pos
    # W_pos_dec[h]: (D, D_RC)
    ...
    def decode_pixel(r, c, g):
        ...
        # Multi-head queries: each head gets a D-dim query vector
        def head_response(W_h):
            q = W_h @ pos_rc    # (D,)
            return g @ q        # (D,)

        fs = jax.vmap(head_response)(params["W_pos_dec"])   # (N_DEC_HEADS, D)
        f_cat = fs.reshape(N_DEC_HEADS * D)                 # (N_DEC_HEADS * D,)

        # 2-layer val head on concatenated multi-head features
        h1 = jax.nn.gelu(params["W_val1"] @ f_cat + params["b_val1"])  # (2D,)
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp9 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp9_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp9 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp9_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp9 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp9_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp9 — gram VAE (N_DEC_HEADS={N_DEC_HEADS} position queries)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp9_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp9.pkl"
    ...
    row = {
        "experiment":      "exp9",
        "name":            f"gram-vae d={D} n_dec_heads={N_DEC_HEADS} dec_rank={DEC_RANK}",
