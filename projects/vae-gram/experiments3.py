"""
exp3 — Gram VAE: fix decoder bottleneck, improve accuracy.

Changes from exp2:
  1. Direct val head: replace `dec_val_emb @ mean_pool(f)` with `W_val_head @ f`.
     exp2 pooled the D=64 gram feature down to D_V=4 before scoring against 32 bins,
     discarding 93% of the information. The direct head uses the full D-dim feature.
  2. Larger LATENT_DIM: 32 → 64 (more information through the bottleneck).
  3. Larger DEC_RANK: 8 → 16 (more expressive pseudo-gram).
  4. Bin accuracy tracked every epoch — gives a real measure of reconstruction quality.
  5. Cosine LR decay for better convergence.
  6. SVG for learning curves, PNG for pixel grids.

Usage:
    uv run python projects/vae-gram/experiments3.py
"""
...
EVAL_EVERY = 5    # compute bin accuracy every N epochs
VIZ_EVERY  = 20
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── decoder: position embeddings (proper scale) ────────────────────
        "dec_row_emb": n((28, D_R), scale=D_R ** -0.5),
        "dec_col_emb": n((28, D_C), scale=D_C ** -0.5),
        "W_pos_dec":   n((D, D_RC), scale=D_RC ** -0.5),

        # ── decoder: direct value head (D → N_VAL_BINS) ───────────────────
        # Replaces exp2's dec_val_emb + mean_pool. Uses the full D-dim gram
        # feature directly, retaining all positional information.
        "W_val_head": n((N_VAL_BINS, D), scale=D ** -0.5),
    }
...
def decode(params, z):
    ...
    def decode_pixel(r, c, g):
        ...
        q      = params["W_pos_dec"] @ pos_rc   # (D,)
        f      = g @ q                          # (D,)
        # Direct head: full D-dim feature → N_VAL_BINS scores
        return params["W_val_head"] @ f         # (N_VAL_BINS,)
...
def bin_accuracy(params, X, batch_size=256):
    """Fraction of pixels where argmax bin == true bin."""
    correct = total = 0
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp3 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp3_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp3 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp3_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp3 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp3_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle("exp3 — gram VAE (direct val head)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp3_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp3.pkl"
    ...
    row = {
        "experiment":      "exp3",
        "name":            f"gram-vae-direct-head d={D} latent={LATENT_DIM} dec_rank={DEC_RANK}",
