"""
exp6 — Gram VAE: encoder MLP + larger latent.

Changes from exp5:
  1. Encoder MLP: gram-query features → Linear(ENC_MLP_HIDDEN) → GELU → mu/lv.
     Previously a direct linear W_mu @ feat. Non-linear encoder better exploits
     the rich gram features.
  2. LATENT_DIM: 64 → 128. More information through the bottleneck.
  3. FREE_BITS: 0.5 → 0.3. With a larger latent, slightly relax free bits to
     allow more dims to be near-zero (not all 128 need to be used equally).

Usage:
    uv run python projects/vae-gram/experiments6.py
"""
...
N_ENC_Q          = 8
ENC_MLP_HIDDEN   = 512   # encoder: gram features → hidden → mu/lv
LATENT_DIM       = 128   # ↑ from 64
DEC_RANK         = 32
DEC_MLP_HIDDEN   = 512   # z → A, B MLP hidden

ADAM_LR          = 3e-4
KL_WEIGHT        = 1.0
KL_WARMUP_EPOCHS = 20
FREE_BITS        = 0.3   # slightly lower: larger latent, some dims may be near-zero
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── encoder: gram → hidden → latent ──────────────────────────────
        "h_enc":        n((N_ENC_Q, D)),
        "ln_enc_gamma": jnp.ones(enc_feat),
        "ln_enc_beta":  jnp.zeros(enc_feat),
        "W_enc_h": n((enc_feat,       ENC_MLP_HIDDEN), scale=enc_feat      ** -0.5),
        "b_enc_h": jnp.zeros(ENC_MLP_HIDDEN),
        "W_mu":    n((ENC_MLP_HIDDEN, LATENT_DIM),     scale=ENC_MLP_HIDDEN ** -0.5),
        "b_mu":    jnp.zeros(LATENT_DIM),
        "W_lv":    n((ENC_MLP_HIDDEN, LATENT_DIM),     scale=ENC_MLP_HIDDEN ** -0.5),
        "b_lv":    jnp.zeros(LATENT_DIM),

        # ── decoder MLP: z → hidden → A, B ───────────────────────────────
        "W_mlp1": n((LATENT_DIM,    DEC_MLP_HIDDEN), scale=LATENT_DIM    ** -0.5),
        "b_mlp1": jnp.zeros(DEC_MLP_HIDDEN),
        "W_A":    n((DEC_MLP_HIDDEN, DEC_RANK * D),  scale=DEC_MLP_HIDDEN ** -0.5),
        "W_B":    n((DEC_MLP_HIDDEN, DEC_RANK * D),  scale=DEC_MLP_HIDDEN ** -0.5),
...
def encode(params, x):
    ...
    # Non-linear encoder MLP
    h       = jax.nn.gelu(feat @ params["W_enc_h"] + params["b_enc_h"])
    mu      = h @ params["W_mu"] + params["b_mu"]
    log_var = h @ params["W_lv"] + params["b_lv"]
    return mu, log_var
...
def decode(params, z):
    ...
    def decode_pixel(r, c, g):
        ...
        f      = g @ q                                              # (D,)
        h1     = jax.nn.gelu(params["W_val1"] @ f + params["b_val1"])  # (2D,)
        return params["W_val2"] @ h1                               # (N_VAL_BINS,)
...
def reconstruct_mode(params, x):
    mu, _     = encode(params, x)
    logits    = decode(params, mu)
    mode_bins = jnp.argmax(logits, axis=-1)
    return BIN_CENTERS[mode_bins]
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    for row, (imgs, label) in enumerate([
        ...
        for i in range(n):
            axes[row, i].imshow(np.array(imgs[i]).reshape(28, 28),
                                cmap="gray", vmin=0, vmax=255)
            axes[row, i].axis("off")
        axes[row, 0].set_title(label, fontsize=9, loc="left")
    fig.suptitle(f"exp6 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp6_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp6 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp6_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
    logging.info(f"  pixel entropy   → {url}")


def save_latent_pca(params, X, y, tag, n_samples=2000):
    mus   = _encode_batched(params, X[:n_samples])
    ...
    for cls in range(10):
        mask = y_sub == cls
        ax.scatter(z2d[mask, 0], z2d[mask, 1], s=6, alpha=0.5,
                   color=cmap(cls), label=str(cls))
    ax.legend(markerscale=2, fontsize=8)
    ax.set_xlabel("PC 1"); ax.set_ylabel("PC 2")
    ax.set_title(f"exp6 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp6_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle("exp6 — gram VAE (enc MLP, latent=128)", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp6_learning_curves", fig, format="svg")
    plt.close(fig)
...
def train(params, X_tr, X_te, y_te):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        logging.info(
            f"epoch {epoch:3d}  β={beta:.2f}  recon={e_recon:.4f}"
            f"  kl={e_kl:.4f}  total={e_total:.4f}{acc_str}  {time.perf_counter() - t0:.1f}s"
        )
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp6.pkl"
    ...
    row = {
        "experiment":      "exp6",
        "name":            f"gram-vae d={D} enc-mlp latent={LATENT_DIM} dec_rank={DEC_RANK}",
        ...
        "n_enc_q":          N_ENC_Q,
        "enc_mlp_hidden":   ENC_MLP_HIDDEN,
        "latent_dim":       LATENT_DIM,
        "dec_rank":         DEC_RANK,
        "dec_mlp_hidden":   DEC_MLP_HIDDEN,
    ...
    append_result(row)
    logging.info(f"Done. bin_acc={final_acc:.1%}  recon={history['recon'][-1]:.4f}"
                 f"  kl={history['kl'][-1]:.4f}  {elapsed:.1f}s")
