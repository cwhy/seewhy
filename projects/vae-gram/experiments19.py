"""
exp19 — Gram VAE: smaller decoder (MLP_HIDDEN=256, DEC_RANK=16), latent=64 locked.

Changes from exp5 (88.7%, 5.0M params, 262s):
  1. MLP_HIDDEN: 512 → 256
  2. DEC_RANK:   32  → 16
  LATENT_DIM=64, FREE_BITS=0.5 locked.

W_A and W_B dominate exp5's param count (each 512×4608=2.36M). Halving both dims:
  W_A: 256×(16×144) = 590K  (was 2.36M)
  W_B: same
Expected params: ~1.4M. Target: still reach 88.7%.

Usage:
    uv run python projects/vae-gram/experiments19.py
"""
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp19 — {tag}", fontsize=10); plt.tight_layout()
    url = save_matplotlib_figure(f"exp19_reconstructions_{tag}", fig, format="png", dpi=150)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp19 — MLP_H={MLP_HIDDEN} DEC_RANK={DEC_RANK} latent={LATENT_DIM}", fontsize=12)
    plt.tight_layout(); url = save_matplotlib_figure("exp19_learning_curves", fig, format="svg")
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp19.pkl", "wb") as f:
        pickle.dump({k: np.array(v) for k, v in params.items()}, f)
    save_learning_curves(history)
    save_reconstruction_grid(params, X_test, "final")
    final_acc = bin_accuracy(params, X_test)
    append_result({
        "experiment": "exp19", "name": f"gram-vae mlp_h={MLP_HIDDEN} dec_rank={DEC_RANK} latent={LATENT_DIM}",
