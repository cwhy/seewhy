"""
exp12 — Gram VAE: encoder MLP (non-linear gram → latent projection).

Changes from exp10 (89.0%):
  1. Encoder MLP: gram features → GELU(W_enc1 @ feat) → mu, lv.
     Adds non-linearity between the gram query aggregation and the latent.
     ENC_MLP_HIDDEN=1024 (vs direct linear in exp10).
  2. LATENT_DIM=256, FREE_BITS=0.5 unchanged (exp10's proven config).

This was partially tested in exp6 but exp6 also changed FREE_BITS=0.3→collapse.
Exp12 cleanly isolates the encoder MLP effect with all other params from exp10.

Architecture note:
  Before: feat (B, N_ENC_Q*D) → W_mu → mu (B, 256)
  After:  feat (B, N_ENC_Q*D) → GELU(W_enc1) → feat_h (B, 1024) → W_mu → mu (B, 256)

Usage:
    uv run python projects/vae-gram/experiments12.py
"""
...
N_ENC_Q          = 8
ENC_MLP_HIDDEN    = 1024  # ← new: non-linear encoder MLP hidden size
LATENT_DIM        = 256   # same as exp10
DEC_RANK          = 32
MLP_HIDDEN        = 512
...
KL_WEIGHT        = 1.0
KL_WARMUP_EPOCHS = 30
FREE_BITS        = 0.5   # MUST stay at 0.5
...
def init_params(key_gen):
    ...
    return {
        ...
        # ── encoder: gram → MLP → latent ─────────────────────────────────
        "h_enc":        n((N_ENC_Q, D)),
        "ln_enc_gamma": jnp.ones(feat_dim),
        "ln_enc_beta":  jnp.zeros(feat_dim),
        "W_enc1": n((feat_dim, ENC_MLP_HIDDEN),  scale=feat_dim ** -0.5),   # new
        "b_enc1": jnp.zeros(ENC_MLP_HIDDEN),                                # new
...
def encode(params, x):
    ...
    # encoder MLP (new non-linear layer)
    feat_h  = jax.nn.gelu(feat @ params["W_enc1"] + params["b_enc1"])

    mu      = feat_h @ params["W_mu"] + params["b_mu"]
    log_var = feat_h @ params["W_lv"] + params["b_lv"]
    return mu, log_var
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp12 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp12_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp12 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp12_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp12 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp12_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp12 — gram VAE enc-MLP (latent={LATENT_DIM}, free_bits={FREE_BITS})", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp12_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp12.pkl"
    ...
    row = {
        "experiment":      "exp12",
        "name":            f"gram-vae enc-mlp d={D} latent={LATENT_DIM} enc_mlp_hidden={ENC_MLP_HIDDEN}",
