"""
exp14 — Gram VAE: larger gram dimension D=196 (D_R=7, D_C=7, D_V=4).

Changes from exp11 (89.2%):
  1. D_R, D_C: 6 → 7 (D_V stays at 4). D: 144 → 196.
     Larger per-pixel token space → richer gram matrix (196×196 vs 144×144).
     Gram encoder feature dim: N_ENC_Q*D = 8*196 = 1568 (vs 1152).
  2. LATENT_DIM=512, FREE_BITS=0.5, DEC_RANK=32, MLP_HIDDEN=512 unchanged.

Hypothesis: D=144 is a relatively small token dimension. Going to D=196 (D_R=D_C=7)
gives each position a richer subspace and the gram a 36% larger representation space.
This should improve the encoder's ability to capture image structure.

Note: gram computation scales as O(784 * D²). With D=196: 784*196²=30.1M flops
vs D=144: 784*144²=16.2M. ~1.85x more expensive but should be GPU-manageable.

Usage:
    uv run python projects/vae-gram/experiments14.py
"""
...
D_R, D_C, D_V = 7, 7, 4   # ↑ from 6,6,4
D    = D_R * D_C * D_V     # 196
D_RC = D_R * D_C           # 49

N_ENC_Q          = 8
LATENT_DIM        = 512    # same as exp11
DEC_RANK          = 32
MLP_HIDDEN        = 512
...
def save_reconstruction_grid(params, X, tag, n=8):
    ...
    fig.suptitle(f"exp14 reconstructions — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp14_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_pixel_entropy_grid(params, X, tag, n=4):
    ...
    fig.suptitle(f"exp14 pixel uncertainty — {tag}", fontsize=10)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp14_entropy_{tag}", fig, format="png", dpi=150)
    plt.close(fig)
...
def save_latent_pca(params, X, y, tag, n_samples=2000):
    ...
    ax.set_title(f"exp14 latent PCA — {tag}"); ax.grid(True, alpha=0.2)
    plt.tight_layout()
    url = save_matplotlib_figure(f"exp14_latent_pca_{tag}", fig, format="svg")
    plt.close(fig)
...
def save_learning_curves(history):
    ...
    fig.suptitle(f"exp14 — gram VAE D={D} (D_R={D_R},D_C={D_C},D_V={D_V}) latent={LATENT_DIM}", fontsize=12)
    plt.tight_layout()
    url = save_matplotlib_figure("exp14_learning_curves", fig, format="svg")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    params_path = Path(__file__).parent / "params_exp14.pkl"
    ...
    row = {
        "experiment":      "exp14",
        "name":            f"gram-vae d={D} (d_r={D_R},d_c={D_C},d_v={D_V}) latent={LATENT_DIM}",
