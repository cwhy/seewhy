"""
exp21 — Gram VAE: exp19 architecture + faster training (BATCH_SIZE=512, LR=6e-4).

Changes from exp19 (~1.4M params, 88.7% target):
  1. BATCH_SIZE: 128 → 512 (4× larger batches, fewer steps/epoch)
  2. ADAM_LR:    3e-4 → 6e-4 (scale LR with batch size)
  Architecture identical to exp19: MLP_HIDDEN=256, DEC_RANK=16, LATENT_DIM=64.

Goal: reach 88.7% in fewer seconds, not just fewer params.
Larger batches = fewer gradient steps per epoch = faster wall-clock time if GPU
is not already saturated (likely the case at BATCH_SIZE=128 for this small model).

Usage:
    uv run python projects/vae-gram/experiments21.py
"""
...
DEC_RANK          = 16   # same as exp19
MLP_HIDDEN        = 256  # same as exp19

ADAM_LR          = 6e-4  # ↑ from 3e-4
KL_WEIGHT        = 1.0
KL_WARMUP_EPOCHS = 20
...
def save_reconstruction_grid(params, X, tag, n=8):
    x_orig = X[:n]; x_mean = reconstruct_mean(params, x_orig)
    fig, axes = plt.subplots(2, n, figsize=(n*1.5, 4))
    for i in range(n):
        axes[0,i].imshow(np.array(x_orig[i]).reshape(28,28), cmap="gray", vmin=0, vmax=255); axes[0,i].axis("off")
        axes[1,i].imshow(np.array(x_mean[i]).reshape(28,28), cmap="gray", vmin=0, vmax=255); axes[1,i].axis("off")
    axes[0,0].set_title("original", fontsize=9, loc="left"); axes[1,0].set_title("recon", fontsize=9, loc="left")
    fig.suptitle(f"exp21 — {tag}", fontsize=10); plt.tight_layout()
    url = save_matplotlib_figure(f"exp21_reconstructions_{tag}", fig, format="png", dpi=150)
    plt.close(fig); logging.info(f"  reconstructions → {url}")

def save_learning_curves(history):
    epochs = range(len(history["recon"]))
    fig, axes = plt.subplots(1, 3, figsize=(13, 4))
    axes[0].plot(epochs, history["recon"], label="recon"); axes[0].plot(epochs, history["total"], label="total", linestyle="--", alpha=0.7)
    axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("nats/pixel"); axes[0].set_title("Loss"); axes[0].legend(); axes[0].grid(True, alpha=0.3)
    axes[1].plot(epochs, history["kl"], color="orange"); axes[1].axhline(FREE_BITS, color="gray", linestyle=":", label=f"free_bits={FREE_BITS}")
    axes[1].set_xlabel("Epoch"); axes[1].set_ylabel("KL"); axes[1].set_title("KL"); axes[1].legend(); axes[1].grid(True, alpha=0.3)
    acc_e = [e for e,a in history["bin_acc"]]; acc_v = [a for e,a in history["bin_acc"]]
    axes[2].plot(acc_e, acc_v, color="green", marker="o", markersize=3)
    axes[2].set_xlabel("Epoch"); axes[2].set_ylabel("Bin accuracy"); axes[2].set_title("Bin accuracy"); axes[2].grid(True, alpha=0.3)
    axes[2].yaxis.set_major_formatter(plt.FuncFormatter(lambda y,_: f"{y:.1%}"))
    fig.suptitle(f"exp21 — exp19 arch + batch={BATCH_SIZE} lr={ADAM_LR}", fontsize=12)
    plt.tight_layout(); url = save_matplotlib_figure("exp21_learning_curves", fig, format="svg")
    plt.close(fig); logging.info(f"  learning curves → {url}")
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "params_exp21.pkl", "wb") as f:
        pickle.dump({k: np.array(v) for k, v in params.items()}, f)
    save_learning_curves(history)
    save_reconstruction_grid(params, X_test, "final")
    final_acc = bin_accuracy(params, X_test)
    append_result({
        "experiment": "exp21", "name": f"gram-vae exp19-arch batch={BATCH_SIZE} lr={ADAM_LR}",
