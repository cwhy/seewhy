"""
exp9 — architecture search: depth=2 bilinear vs mlp=512.

Findings from exp8: correct EGGROLL normalization (eps/rank) completely fails from
random init — perturbations too small to escape the ~11% cross-entropy plateau.
Our "incorrect" std=sigma*sqrt(rank) is actually crucial for warm-up escape.

exp8 verdict: rank=32/64 with correct normalization = 11.24% throughout all 25 epochs.
  The perturbation is 16-22× too small to generate useful fitness differences.

exp9 hypothesis: squeeze more accuracy out of 25 epochs by changing architecture:
  GPU0: depth=2 — two bilinear hops (M@M@H_0 vs M@H_0). Same param count.
    → richer higher-order pixel correlations, quartic vs quadratic in embeddings.
  GPU1: mlp=512 — double hidden layer (W1 is 512×64 instead of 256×64).
    → more discriminative capacity in the final classifier, 2× non-emb params.

Both use rank=8, sigma=0.001, n_pop=128 (the current best 25-ep config).

Reference: https://eshyperscale.github.io/
Usage:
    uv run python projects/es/experiments9.py
"""
...
EXPERIMENTS = [
    # GPU 0: depth=2 — two bilinear hops. Same param count as depth=1.
    # M@M@H_0: quartic in pixel embeddings (vs quadratic for depth=1).
    # Hypothesis: richer correlations → better features → higher accuracy.
    dict(label="f3(4,4,4) mlp=256 rank=8 n_pop=128 25ep depth=2",
         mlp=256, sigma=0.001, n_pop=128, rank=8, depth=2, gpu=0),
    # GPU 1: mlp=512 — double MLP hidden layer. Non-emb params: ~37k vs ~19.5k.
    # Hypothesis: larger classifier head better separates bilinear features.
    dict(label="f3(4,4,4) mlp=512 rank=8 n_pop=128 25ep",
         mlp=512, sigma=0.001, n_pop=128, rank=8, depth=1, gpu=1),
]
...
def run_single(gpu_id):
    ...
    for cfg in EXPERIMENTS:
        ...
        row = {
            ...
            "notes": f"arch search: depth={cfg.get('depth',1)} mlp={cfg['mlp']}",
        }
        append_result(row)
...
if __name__ == "__main__":
    if "--gpu" in sys.argv:
        ...
        procs  = []
        for gpu_id in [0, 1]:
            log_path = f"/tmp/exp9_gpu{gpu_id}.log"
        ...
        for line in open(JSONL):
            row = json.loads(line)
            if row.get("experiment") == "exp9" and row["name"] not in seen:
                seen.add(row["name"])
