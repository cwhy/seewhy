"""
exp23c — Single-sided ES: clean hyperparameter sweep (n_pop=256, K=1 only).

Grad-accum (K>1) is unreliable without full JIT on the outer step.
n_pop>256 OOMs. So we fix n_pop=256 and sweep sigma and rank_switch.

Baseline: n_pop=256, sw=12, sigma=0.001 → 95.40% (exp23-A, exp22-B)

Plateau escape with single-sided: ep5 (vs ep7 antithetic).
Rank switch at ep8 gives 4 extra epochs of rank=2 in the productive phase.

Sweep:
  A: sw=12  σ=0.001   (reference)
  B: sw=8   σ=0.001   (earlier rank switch)
  C: sw=8   σ=0.0007  (earlier switch + finer sigma)
  D: sw=8   σ=0.0005  (even finer sigma)
  E: sw=12  σ=0.0007  (finer sigma, standard switch)
  F: sw=6   σ=0.001   (very early switch, right after escape)

Usage:
    CUDA_VISIBLE_DEVICES=0 uv run python projects/es/experiments23c.py
"""
...
REPORT_PATH = Path(__file__).parent / "report23c.md"
...
def make_ss_step(fwd, sigma, tx, rank):
...
def train_ss(name, fwd, params, X_tr, y_tr, X_te, y_te, sigma=0.001, rank_switch=12):
    num_batches = len(X_tr) // BATCH_SIZE
    lr_sched    = optax.cosine_decay_schedule(ADAM_LR_INIT,
                                              MAX_EPOCHS * num_batches,
                                              alpha=ADAM_LR_END / ADAM_LR_INIT)
    tx        = optax.adam(lr_sched)
    step_hi   = make_ss_step(fwd, sigma, tx, rank=8)
    step_lo   = make_ss_step(fwd, sigma, tx, rank=2)
...
def make_plots(results):
    ...
    ax.set_title(f"exp23c — Single-sided ES n_pop={N_POP}: sigma & rank_switch sweep")
    ax.legend(loc="lower right", fontsize=8); ax.grid(alpha=0.3)
    fig.tight_layout()
    urls["accuracy"] = save_matplotlib_figure("exp23c_accuracy", fig)
    plt.close(fig)
...
def write_report(urls, rows):
    ...
    table = "\n".join(
        f"| {v} | {s} | {sw} | {tr*100:.2f}% | {te*100:.2f}% | {t:.0f}s |"
        for v, s, sw, tr, te, t in rows)
    lines = [
        "# exp23c — Single-sided ES Sweep Report",
        "",
        f"**Date:** {date_str}  **Epochs:** {MAX_EPOCHS}  **n_pop:** {N_POP}",
        "",
        f"Baseline: sw=12, σ=0.001 → **95.40%** (exp23-A)",
        "",
        "## Results",
        "",
...
SWEEP = [
    # (name,          sigma,  rank_switch)
    ("A sw=12 σ=.001", 0.001,  12),
    ("B sw=8  σ=.001", 0.001,   8),
    ("C sw=8  σ=.0007",0.0007,  8),
    ("D sw=8  σ=.0005",0.0005,  8),
    ("E sw=12 σ=.0007",0.0007, 12),
    ("F sw=6  σ=.001", 0.001,   6),
]
...
if __name__ == "__main__":
    ...
    for name, sigma, rank_sw in SWEEP:
        logging.info(f"\n{'='*55}\n{name}\n{'='*55}")
        ...
        all_results.append((name, sigma, rank_sw, tr, te, t_s))
        plot_data.append((name, None, hist))
        append_result({"experiment": "exp23c", "variant": name, "n_pop": N_POP,
                       "sigma": sigma, "rank_switch": rank_sw,
    ...
    print(f"\n{'='*60}")
    print(f"{'Variant':<20} {'σ':>7} {'sw':>4} {'Test':>7} {'Time':>8}")
    print(f"{'-'*60}")
    for name, sigma, rank_sw, tr, te, t_s in all_results:
        print(f"{name:<20} {sigma:>7.4f} {rank_sw:>4} {te:>7.4f} {t_s:>7.0f}s")
    print(f"{'='*60}")
