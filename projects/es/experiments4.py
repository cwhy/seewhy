"""
exp4 — Does larger MLP help ES? mlp=64/128/256/512 sweep.

With gradient descent (RNN project): mlp=128→98.36%, mlp=256→98.65%, mlp=512→98.81%.
For ES the tradeoff differs:
  + more capacity → better fit when optimizer can reach it
  - more parameters → same n_pop gradient budget spread thinner
ES gradient SNR per element is roughly fixed (σ² * ||∇L_layer||²_F / n_pop),
independent of layer size. So larger MLP should still help IF capacity is the limit.

Same settings as exp3 (sigma=0.001, n_pop=128, cosine LR 1e-3→1e-4, 100 epochs).

Usage:
    uv run python projects/es/experiments4.py
"""
...
if __name__ == "__main__":
    ...
    experiments = [
        ("f3(4,4,4) mlp=64  ES n_pop=128", *make_f3_bilinear(4, 4, 4,  64)),
        ("f3(4,4,4) mlp=128 ES n_pop=128", *make_f3_bilinear(4, 4, 4, 128)),
        ("f3(4,4,4) mlp=256 ES n_pop=128", *make_f3_bilinear(4, 4, 4, 256)),
        ("f3(4,4,4) mlp=512 ES n_pop=128", *make_f3_bilinear(4, 4, 4, 512)),
    ]
    ...
    for label, fwd, params, emb_keys in experiments:
        ...
        row = {
            "name": label, "experiment": "exp4",
            "family": "es_f3_mlp_sweep",
            ...
            "notes": "mlp sweep, sigma=0.001, n_pop=128, cosine LR, noise30",
        }
        append_result(row)
    ...
    print(f"\n{'='*85}")
    print(f"{'Variant':<40} {'NonEmb':>8} {'Total':>8} {'Train':>8} {'Test':>8}")
    print(f"{'-'*85}")
    for label, nonemb, ntotal, tr, te in results:
        print(f"{label:<40} {nonemb:>8,} {ntotal:>8,} {tr:>8.4f} {te:>8.4f}")
    print(f"{'='*85}")
