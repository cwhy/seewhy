"""
exp6 — Gram-free IMLE: outer product of h and pos → per-pixel logits.

The gram decoder from exp1–5 was a mistake: it carried the VAE encoder's
pseudo-gram structure (A@B.T) into the decoder, which adds complexity with
no benefit in IMLE (there is no encoder to mirror).

Correct decoder: z is the conditioning signal. Per pixel, compute the outer
product of latent h = MLP(z) and positional embedding pos, then predict logits.

  z (LATENT_DIM)
  → h = GELU(z @ W_mlp1 + b_mlp1)          # (MLP_HIDDEN,) shared across pixels

  per pixel (r, c):
  → pos = row_emb[r] ⊗ col_emb[c]          # (D_RC,)
  → f   = (re[:,None,None] * ce[None,:,None] * h[None,None,:]).reshape(-1)  # (D_R*D_C*MLP_HIDDEN,)
  → h1  = GELU(W_val1 @ f + b_val1)
  → logits = W_val2 @ h1                     # (N_VAL_BINS,)

MLP_HIDDEN=64 so f_dim = 6*6*64 = 2304 (3-way tensor product r⊗c⊗h).
No gram. No DEC_RANK. No W_A/W_B. No W_pos_dec.
Otherwise identical to exp1 (IMLE training loop, N_SAMPLES=1000, LATENT_DIM=64).

Usage:
    uv run python projects/imle-gram/experiments6.py
"""
...
D_R, D_C   = 6, 6
D_RC       = D_R * D_C   # 36  — positional embedding dim

LATENT_DIM = 64
MLP_HIDDEN = 64           # z → h dim; f_dim = D_R*D_C*MLP_HIDDEN = 6*6*64 = 2304
...
def init_params(key_gen):
    ...
    f_dim = D_R * D_C * MLP_HIDDEN   # 6*6*64 = 2304 — tensor product r⊗c⊗h
    return {
        # z → h
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  (no gram, z+pos → logits)")
    ...
    save_learning_curves(history, EXP_NAME, subtitle=f"r⊗c⊗h N_SAMPLES={N_SAMPLES} LATENT_DIM={LATENT_DIM} MLP_HIDDEN={MLP_HIDDEN}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-gramfree N_SAMPLES={N_SAMPLES} LATENT_DIM={LATENT_DIM} MLP_HIDDEN={MLP_HIDDEN}",
