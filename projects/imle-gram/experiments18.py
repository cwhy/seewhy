"""
exp18 — 49-cluster flat clustering (load-balanced, mean-split).

Same architecture as exp17c (N_VARS=1 Hebbian, load-balanced mean-split
assignment) with N_CODES=49 (= 7×7).

Baseline for the exp19 comparison: same 49-prototype budget, but flat —
all 49 prototypes are fully independent.

Usage:
    uv run python projects/imle-gram/experiments18.py
"""
...
N_VARS   = 1
N_CODES  = 49    # 7×7 — baseline for exp19
D_EMB    = 16
D_U      = 16
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster-flat N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U}",
