...
Same as exp21 but adds a periodic merge step after each epoch:
  ...
  - Reinit = fresh random embedding (same scale as original init)
...
def merge_and_reinit(params, opt_state, cum_usage, key):
    ...
    # Reinit: fresh random embedding (same scale as original init_params)
    noise_key = jax.random.PRNGKey(int(key[0]) ^ int(key[1]))
    new_cb    = np.array(params["codebook"])
    for idx in to_reinit:
        new_cb[idx] = np.array(jax.random.normal(noise_key, (D_EMB,))) * (D_EMB ** -0.5)
...
def train(params, X_tr, X_te, Y_te):
    ...
    cum_usage_total = np.zeros(N_CODES)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster-merge-reinit N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U}",
