"""
exp19b — 7×7 factored clustering, individual codebooks.

N_VARS=2, N_CODES_PER_VAR=7, N_COMBOS=49.
Two separate codebooks: codebook_0 for var 0, codebook_1 for var 1.

Each image is assigned to a pair (i, j) and decoded as:
    z_0 = codebook_0[i],  z_1 = codebook_1[j]
    M = u_0⊗v_0 + u_1⊗v_1   (sum of 2 rank-1 outer products)

Unlike exp19 (shared codebook), all 49 combinations are fully distinct:
code i in slot 0 and code i in slot 1 are different embeddings. No symmetry.
Slot 0 and slot 1 can specialize to different axes of variation.

Compare with:
  exp18  — 49-cluster flat (49 fully independent prototypes)
  exp19  — 7×7 with shared codebook (~28 unique prototypes due to symmetry)

Usage:
    uv run python projects/imle-gram/experiments19b.py
"""
...
N_VARS          = 2
N_CODES_PER_VAR = 7       # codebook size per variable
N_COMBOS        = N_CODES_PER_VAR ** 2  # 49 combination prototypes
D_EMB           = 16
D_U             = 16
...
def init_params(key_gen):
    ...
    return {
        "codebook_0": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),  # for var 0
        "codebook_1": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),  # for var 1
...
# ── Codebook lookup (individual) ──────────────────────────────────────────────
# indices: (B, 2) with (i, j) — slot 0 uses codebook_0, slot 1 uses codebook_1.

def _to_gram(Z, params):
    """Z: (B, N_VARS, D_EMB) → Gram: (B, D_U²)."""
    U = Z @ params["W_u"] + params["b_u"]   # (B, N_VARS, D_U)
    V = Z @ params["W_v"] + params["b_v"]   # (B, N_VARS, D_U)
    M = jnp.einsum("bni,bnj->bij", U, V)    # (B, D_U, D_U) — sum of rank-1 outer products
    return M.reshape(Z.shape[0], -1)         # (B, D_U²)

def hard_lookup(params, indices):
    """indices: (B, 2) int. Returns gram (B, D_U²)."""
    Z0 = params["codebook_0"][indices[:, 0], :]   # (B, D_EMB)
    Z1 = params["codebook_1"][indices[:, 1], :]   # (B, D_EMB)
    Z  = jnp.stack([Z0, Z1], axis=1)              # (B, 2, D_EMB)
    return _to_gram(Z, params)

def soft_lookup(params, indices):
    """indices: (B, 2) int. Returns gram (B, D_U²) via soft one-hot per slot."""
    oh = jax.nn.one_hot(indices, N_CODES_PER_VAR)          # (B, 2, 7)
    w  = jax.nn.softmax(oh * TEMP, axis=-1)                 # (B, 2, 7)
    Z0 = (w[:, 0:1, :] @ params["codebook_0"])             # (B, 1, D_EMB)
    Z1 = (w[:, 1:2, :] @ params["codebook_1"])             # (B, 1, D_EMB)
    Z  = jnp.concatenate([Z0, Z1], axis=1)                  # (B, 2, D_EMB)
    return _to_gram(Z, params)
...
def decode_prototypes(params):
    """Decode all N_COMBOS=49 (i,j) combination prototypes. Returns (49, 784).
    Flat index: flat = i * N_CODES_PER_VAR + j  (i=var0 code, j=var1 code)."""
    i_idx     = np.repeat(np.arange(N_CODES_PER_VAR), N_CODES_PER_VAR)
    j_idx     = np.tile(np.arange(N_CODES_PER_VAR), N_CODES_PER_VAR)
    idx_pairs = jnp.stack([jnp.array(i_idx), jnp.array(j_idx)], axis=1)  # (49, 2)
    z         = hard_lookup(params, idx_pairs)                             # (49, D_U²)
    logits    = decode(params, z)                                          # (49, 784, 32)
    probs     = jax.nn.softmax(logits, axis=-1)
    return np.array((probs * BIN_CENTERS[None, None, :]).sum(-1))         # (49, 784)

# ── Load-balanced assignment (mean-split) ─────────────────────────────────────
...
def load_balanced_assign(X_real, X_proto):
    ...
    d_t   = (X_proto**2).sum(1)
    dists = d_r + d_t - 2.0 * (X_real @ X_proto.T)   # (N, N_COMBOS)
    ...
    assignment = nn.copy()
    counts = np.bincount(nn[top_idx], minlength=N_COMBOS).astype(np.float32)

    bot_dists = dists[bot_idx]
    rank      = np.argsort(bot_dists, axis=0)

    ptr      = np.zeros(N_COMBOS, dtype=int)
...
def label_accuracy(params, X_te, Y_te):
    ...
    label_probs = np.zeros((N_COMBOS, N_MNIST_CLASSES))
...
def imle_loss(params, x, matched_indices):
    """matched_indices: (B, 2) with (i, j) per image."""
...
def eval_metrics(params, X_eval):
    ...
    for i in range(0, len(X_eval), BATCH_SIZE):
        ...
        flat_idx = np.argmin(dists, axis=1)
        match_losses.append(((x_batch - X_proto[flat_idx])**2).mean())
        i_codes  = flat_idx // N_CODES_PER_VAR
        j_codes  = flat_idx %  N_CODES_PER_VAR
        pairs    = jnp.array(np.stack([i_codes, j_codes], axis=1))
        logits   = decode(params, hard_lookup(params, pairs))
...
def train(params, X_tr, X_te, Y_te):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        usage      = np.bincount(all_assign, minlength=N_COMBOS)
        ...
        for b in range(num_batches):
            ...
            flat_idx = load_balanced_assign(x_batch, X_proto)
            e_match += ((x_batch - X_proto[flat_idx])**2).mean()
            i_codes  = flat_idx // N_CODES_PER_VAR
            j_codes  = flat_idx %  N_CODES_PER_VAR
            matched  = jnp.array(np.stack([i_codes, j_codes], axis=1))   # (B, 2)
        ...
        logging.info(f"epoch {epoch:3d}  nll={e_loss:.4f}  match_l2={e_match:.1f}"
                     f"  active={n_active}/{N_COMBOS}{eval_str}  {time.perf_counter()-t0:.1f}s")
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  "
                 f"(factored 7×7 individual codebooks N_COMBOS={N_COMBOS})")
    ...
    save_learning_curves(history, EXP_NAME,
                         subtitle=f"7×7 individual codebooks N_COMBOS={N_COMBOS} D_U={D_U}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster-7x7-indiv N_COMBOS={N_COMBOS} N_CODES_PER_VAR={N_CODES_PER_VAR} D_EMB={D_EMB} D_U={D_U}",
        ...
        "n_combos": N_COMBOS, "n_codes_per_var": N_CODES_PER_VAR, "n_vars": N_VARS,
        "codebook": "individual", "d_emb": D_EMB, "d_u": D_U,
