"""
exp19 — 7×7 factored clustering, shared codebook.

N_VARS=2, N_CODES_PER_VAR=7, N_COMBOS=49.
One codebook of 7 entries shared by both slots.

Each image is assigned to a pair (i, j) from {0..6}×{0..6} and decoded as:
    M = u_i⊗v_i + u_j⊗v_j   (sum of 2 rank-1 outer products)

Symmetry note: because W_u, W_v and the codebook are all shared, pairs (i, j)
and (j, i) produce the same Gram matrix. Of the 49 pairs, only 28 are unique
(7 diagonal + 21 upper-triangle). The 49 address slots are preserved for the
load-balanced assignment, but semantically only 28 distinct prototypes exist.

Compare with:
  exp18  — 49-cluster flat (49 fully independent prototypes)
  exp19b — 7×7 with individual codebooks (all 49 combinations unique)

Usage:
    uv run python projects/imle-gram/experiments19.py
"""
...
N_VARS          = 2
N_CODES_PER_VAR = 7       # codebook size (shared for both vars)
...
def init_params(key_gen):
    ...
    return {
        "codebook": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),  # shared
...
# ── Codebook lookup (shared) ───────────────────────────────────────────────────
# indices: (B, 2) with (i, j) — both slots index the same codebook.
...
def decode_prototypes(params):
    ...
    Flat index: flat = i * N_CODES_PER_VAR + j."""
...
# Returns flat indices 0..N_COMBOS-1. Convert to (i,j) via i=flat//K, j=flat%K.
...
def label_accuracy(params, X_te, Y_te):
    ...
    d_t     = (X_proto**2).sum(1)
    dists   = d_r + d_t - 2.0 * (X_te @ X_proto.T)   # (N_test, N_COMBOS)
    ...
    return hard_acc, soft_acc, label_probs   # label_probs: (49, 10)
...
def eval_metrics(params, X_eval):
    ...
    for i in range(0, len(X_eval), BATCH_SIZE):
        ...
        pairs    = jnp.array(np.stack([i_codes, j_codes], axis=1))   # (B, 2)
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  "
                 f"(factored 7×7 shared codebook N_COMBOS={N_COMBOS})")
    ...
    save_learning_curves(history, EXP_NAME,
                         subtitle=f"7×7 shared codebook N_COMBOS={N_COMBOS} D_U={D_U}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster-7x7-shared N_COMBOS={N_COMBOS} N_CODES_PER_VAR={N_CODES_PER_VAR} D_EMB={D_EMB} D_U={D_U}",
        ...
        "codebook": "shared", "d_emb": D_EMB, "d_u": D_U,
