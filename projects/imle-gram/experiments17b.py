"""
exp17b — exp17 (load-balanced clustering) with extended history tracking.

Adds per-epoch cluster assignment counts and per-eval label accuracy
(hard majority-vote and soft probabilistic) using the MNIST test labels.

Usage:
    uv run python projects/imle-gram/experiments17b.py
"""
...
def load_balanced_assign(X_real, X_proto):
    ...
    order   = np.argsort(dists[np.arange(N), nn])
    top_idx = order[:half]
    bot_idx = order[half:]
    n_bot   = len(bot_idx)
