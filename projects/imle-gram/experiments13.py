"""
exp13 — Position-agnostic Hebbian aggregation of codebook slots.

Instead of concat(z_0,...,z_{N-1}), each slot is projected to (u_i, v_i) and
the latent is the sum of outer products — a Hebbian-style Gram matrix M:

  z_i     = E[k_i]                           # (D_EMB,)  shared codebook
  u_i     = z_i @ W_u + b_u                  # (D_U,)
  v_i     = z_i @ W_v + b_v                  # (D_U,)
  M       = Σ_i  u_i[:,None] * v_i[None,:]   # (D_U, D_U) — sum of rank-1 matrices
  z_flat  = M.reshape(-1)                     # (D_U², LATENT_DIM=256)

Then the r⊗c⊗h tensor product decoder takes z_flat as usual.

Properties:
  - Permutation-invariant: order of k_i does not matter.
  - N_CODES doubled to 32 (no position info to distinguish slots).
  - M diagonal = Σ u_i² (self-activations); off-diagonal = Σ u_i·v_j (co-activations).
  - Same LATENT_DIM=256 as exp9-12 (D_U²=16²=256 = N_VARS*D_EMB).

Prior: P_ZERO=0.8 (best sparsity from exp11), slot 0 fixed to code 0.

Usage:
    uv run python projects/imle-gram/experiments13.py
"""
...
TEMP       = 10.0
P_ZERO     = 0.8           # prob each free slot is code 0 (~3 active slots)
...
def sample_indices(key, m):
    """Slot 0 fixed=0; free slots are 0 w.p. P_ZERO else Uniform(1, N_CODES-1)."""
    k1, k2 = jax.random.split(key)
    use_zero  = jax.random.bernoulli(k1, P_ZERO, (m, N_VARS - 1))
    rand_code = jax.random.randint(k2, (m, N_VARS - 1), 1, N_CODES)
    free  = jnp.where(use_zero, 0, rand_code).astype(jnp.int32)
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  (Hebbian N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U} P_ZERO={P_ZERO})")
    ...
    save_learning_curves(history, EXP_NAME, subtitle=f"Hebbian N_CODES={N_CODES} D_U={D_U} P_ZERO={P_ZERO}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-hebbian N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U} P_ZERO={P_ZERO}",
        ...
        "n_vars": N_VARS, "n_codes": N_CODES, "d_emb": D_EMB, "d_u": D_U, "p_zero": P_ZERO,
