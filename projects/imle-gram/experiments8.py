"""
exp8 — IMLE with VQ-style categorical latent: individual codebook per variable.

Instead of z ~ N(0,I), the latent is a vector of N_VARS independent categorical
variables, each selecting from its own codebook of N_CODES entries.

  latent = (k_1, ..., k_{N_VARS}),  k_i ~ Categorical(N_CODES)

  codebooks: (N_VARS, N_CODES, D_EMB)  — one codebook per variable

Gradient flow: use softmax relaxation during the training step.
  soft_z_i = softmax(one_hot(k_i) * TEMP) @ codebooks[i]   # (D_EMB,)
High TEMP (10) → near-hard, but gradient reaches all entries via softmax Jacobian.
Hard z (TEMP→∞) is used for pixel-space pool decoding (no gradient needed).

Decoder is the same r⊗c⊗h tensor product as exp6/exp7:
  z (LATENT_DIM=N_VARS*D_EMB=128) → h = GELU(z @ W_mlp1) → r⊗c⊗h → logits

Usage:
    uv run python projects/imle-gram/experiments8.py
"""
...
N_VARS   = 16    # number of categorical latent variables
N_CODES  = 16    # codebook size per variable (K)
D_EMB    = 8     # embedding dim per variable

LATENT_DIM = N_VARS * D_EMB   # 128 — fed into tensor product decoder
MLP_HIDDEN = 64                # f_dim = 6*6*64 = 2304
TEMP       = 10.0              # softmax temperature; high → near-hard lookup
...
def init_params(key_gen):
    ...
    return {
        # categorical codebooks: one per variable
        "codebooks": n((N_VARS, N_CODES, D_EMB), scale=D_EMB**-0.5),
        # latent → h (same as exp6)
...
def hard_lookup(params, indices):
    """indices (B, N_VARS) int → z (B, LATENT_DIM) hard embedding (for pool decoding)."""
    v = jnp.arange(N_VARS)
    z = params["codebooks"][v[None, :], indices, :]   # (B, N_VARS, D_EMB)
    return z.reshape(indices.shape[0], LATENT_DIM)

def soft_lookup(params, indices):
    """indices (B, N_VARS) int → z (B, LATENT_DIM) softmax-relaxed (for training)."""
    oh = jax.nn.one_hot(indices, N_CODES)             # (B, N_VARS, N_CODES)
    w  = jax.nn.softmax(oh * TEMP, axis=-1)           # (B, N_VARS, N_CODES) near-one-hot
    z  = (w[:, :, :, None] * params["codebooks"][None, :, :, :]).sum(axis=2)  # (B, N_VARS, D_EMB)
    return z.reshape(indices.shape[0], LATENT_DIM)
...
def sample_and_decode(params, key, m, chunk=256):
    """Sample m discrete codes; return (indices, pixel images) for IMLE pool."""
    I_parts, X_parts = [], []
    for i in range(0, m, chunk):
        bs = min(chunk, m - i)
        key, subkey = jax.random.split(key)
        idx = jax.random.randint(subkey, (bs, N_VARS), 0, N_CODES)   # (bs, N_VARS)
...
def imle_loss(params, x, matched_indices):
    z_soft = soft_lookup(params, matched_indices)   # gradient flows through codebooks
...
def eval_metrics(params, X_eval, key, m=M_EVAL):
    ...
    for i in range(0, len(X_eval), 256):
        ...
        z_hard  = hard_lookup(params, jnp.array(I_pool[idx]))
        logits  = decode(params, z_hard)
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  (VQ categorical N_VARS={N_VARS} N_CODES={N_CODES} D_EMB={D_EMB})")
    logging.info(f"Prior space: {N_CODES}^{N_VARS} = {N_CODES**N_VARS:.2e} discrete codes")
    ...
    save_learning_curves(history, EXP_NAME, subtitle=f"VQ-cat N_VARS={N_VARS} N_CODES={N_CODES} D_EMB={D_EMB}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-vqcat N_VARS={N_VARS} N_CODES={N_CODES} D_EMB={D_EMB} TEMP={TEMP}",
