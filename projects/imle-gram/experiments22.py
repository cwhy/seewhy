"""
exp22 — 8×8×8 factored clustering, individual codebooks.

N_VARS=3, N_CODES_PER_VAR=8, N_COMBOS=512.
Three separate codebooks: codebook_0, codebook_1, codebook_2 (each (8, D_EMB)).

Each image is assigned to a triplet (i, j, k) and decoded as:
    z_0 = codebook_0[i],  z_1 = codebook_1[j],  z_2 = codebook_2[k]
    M = u_0⊗v_0 + u_1⊗v_1 + u_2⊗v_2   (sum of 3 rank-1 outer products)

Flat index: flat = i * 64 + j * 8 + k

Assignment: delta_sum priority (same as exp21).

Compare with:
  exp21  — 128-cluster flat delta_sum
  exp23  — 512-cluster flat delta_sum (same capacity, no factorisation)
  exp19b — 7×7 two-variable individual codebooks

Usage:
    uv run python projects/imle-gram/experiments22.py
"""
...
N_VARS          = 3
N_CODES_PER_VAR = 8        # codebook size per variable
N_COMBOS        = N_CODES_PER_VAR ** 3   # 512 combination prototypes
D_EMB           = 16
D_U             = 16
...
def init_params(key_gen):
    ...
    return {
        "codebook_0": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),
        "codebook_1": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),
        "codebook_2": n((N_CODES_PER_VAR, D_EMB), scale=D_EMB**-0.5),
...
# ── Codebook lookup (individual, 3 vars) ─────────────────────────────────────
# indices: (B, 3) with (i, j, k) — each slot uses its own codebook.
...
def hard_lookup(params, indices):
    """indices: (B, 3) int. Returns gram (B, D_U²)."""
    Z0 = params["codebook_0"][indices[:, 0], :]
    Z1 = params["codebook_1"][indices[:, 1], :]
    Z2 = params["codebook_2"][indices[:, 2], :]
    Z  = jnp.stack([Z0, Z1, Z2], axis=1)   # (B, 3, D_EMB)
    return _to_gram(Z, params)

def soft_lookup(params, indices):
    """indices: (B, 3) int. Returns gram (B, D_U²) via soft one-hot per slot."""
    oh = jax.nn.one_hot(indices, N_CODES_PER_VAR)           # (B, 3, 8)
    w  = jax.nn.softmax(oh * TEMP, axis=-1)                  # (B, 3, 8)
    Z0 = (w[:, 0:1, :] @ params["codebook_0"])              # (B, 1, D_EMB)
    Z1 = (w[:, 1:2, :] @ params["codebook_1"])              # (B, 1, D_EMB)
    Z2 = (w[:, 2:3, :] @ params["codebook_2"])              # (B, 1, D_EMB)
    Z  = jnp.concatenate([Z0, Z1, Z2], axis=1)              # (B, 3, D_EMB)
    return _to_gram(Z, params)
...
def decode_prototypes(params):
    """Decode all N_COMBOS=512 (i,j,k) combination prototypes. Returns (512, 784).
    Flat index: flat = i * 64 + j * 8 + k"""
    K = N_CODES_PER_VAR
    i_idx = np.repeat(np.arange(K), K * K)
    j_idx = np.tile(np.repeat(np.arange(K), K), K)
    k_idx = np.tile(np.arange(K), K * K)
    idx_triplets = jnp.stack([jnp.array(i_idx), jnp.array(j_idx), jnp.array(k_idx)], axis=1)  # (512, 3)
    z      = hard_lookup(params, idx_triplets)   # (512, D_U²)
    logits = decode(params, z)                   # (512, 784, 32)
    probs  = jax.nn.softmax(logits, axis=-1)
    return np.array((probs * BIN_CENTERS[None, None, :]).sum(-1))   # (512, 784)

# ── Delta-sum assignment ───────────────────────────────────────────────────────

def load_balanced_assign(X_real, X_proto):
    N    = len(X_real)
    cidx = np.arange(N_COMBOS)
    ...
    delta_sum  = np.zeros(N_COMBOS, dtype=dists.dtype)
    ptr        = np.zeros(N_COMBOS, dtype=np.intp)
    assigned   = np.zeros(N, dtype=bool)
    assignment = np.empty(N, dtype=np.intp)

    for _ in range(N):
        cur_imgs  = rank[ptr, cidx]
        cur_costs = delta_sum + dists[cur_imgs, cidx]
...
def imle_loss(params, x, matched_indices):
    """matched_indices: (B, 3) with (i, j, k) per image."""
...
def eval_metrics(params, X_eval):
    ...
    for i in range(0, len(X_eval), BATCH_SIZE):
        ...
        i_codes  = flat_idx // (K * K)
        j_codes  = (flat_idx // K) % K
        k_codes  = flat_idx % K
        pairs    = jnp.array(np.stack([i_codes, j_codes, k_codes], axis=1))
...
def train(params, X_tr, X_te, Y_te):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for b in range(num_batches):
            ...
            j_codes  = (flat_idx // K) % K
            k_codes  = flat_idx % K
            matched  = jnp.array(np.stack([i_codes, j_codes, k_codes], axis=1))  # (B, 3)
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  "
                 f"(factored 8×8×8 individual codebooks N_COMBOS={N_COMBOS})")
    ...
    save_learning_curves(history, EXP_NAME,
                         subtitle=f"8×8×8 individual codebooks N_COMBOS={N_COMBOS} D_U={D_U}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster-8x8x8-indiv N_COMBOS={N_COMBOS} N_CODES_PER_VAR={N_CODES_PER_VAR} D_EMB={D_EMB} D_U={D_U}",
