"""
exp15 — Hebbian with N_VARS=2: exactly the capacity of 1-or-2 active slots.

N_VARS reduced from 16 to 2: no wasted slot positions.
  - n_active ~ Uniform({1, 2})
  - idx = [code_0, code_1 if n_active==2 else 0]
  - code_i ~ Uniform(1, N_CODES-1)

Usage:
    uv run python projects/imle-gram/experiments15.py
"""
...
N_VARS   = 2     # exactly the capacity of 1-or-2 active slots
...
def sample_indices(key, m):
    """N_VARS=2: n_active ~ {1,2}; idx=[code_0, code_1 if 2 else 0]."""
    k1, k2, k3 = jax.random.split(key, 3)
    n_active = jax.random.randint(k1, (m,), 1, 3)      # 1 or 2
    code_0   = jax.random.randint(k2, (m,), 1, N_CODES)
    code_1   = jax.random.randint(k3, (m,), 1, N_CODES)
    return jnp.stack([code_0, jnp.where(n_active == 2, code_1, 0)], axis=1)
...
if __name__ == "__main__":
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-hebbian N_VARS={N_VARS} N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U} n_active=1-2",
