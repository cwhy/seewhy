"""
exp16 — Pure clustering: N_VARS=1, always exactly 1 active code.

Each sample picks one code k ~ Uniform(0, N_CODES-1).
M = u_k ⊗ v_k  (single rank-1 matrix).
Equivalent to IMLE with N_CODES=32 learned cluster prototypes.

Usage:
    uv run python projects/imle-gram/experiments16.py
"""
...
N_VARS   = 1     # single slot — pure clustering over N_CODES prototypes
...
def sample_indices(key, m):
    """N_VARS=1: k ~ Uniform(0, N_CODES-1) — pure clustering."""
    return jax.random.randint(key, (m, 1), 0, N_CODES)
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  (clustering N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U})")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-cluster N_VARS={N_VARS} N_CODES={N_CODES} D_EMB={D_EMB} D_U={D_U}",
