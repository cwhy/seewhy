"""
exp10 — Shared codebook + zero-biased prior (P_ZERO=0.5 per free slot).

Changes from exp9:
  - Each free slot (1..N_VARS-1) samples code 0 with probability P_ZERO=0.5,
    otherwise samples uniformly from codes 1..N_CODES-1.
  - Sparser latent vectors: ~7.5 non-zero free slots per sample on average.
  - Decoder learns to treat code 0 as "absent" → better isolated-code interpretability.

Usage:
    uv run python projects/imle-gram/experiments10.py
"""
...
TEMP       = 10.0
P_ZERO     = 0.5   # probability each free slot is code 0
...
def sample_indices(key, m):
    """Sample m index vectors; slot 0 fixed=0; free slots are 0 w.p. P_ZERO else 1..N_CODES-1."""
...
if __name__ == "__main__":
    ...
    logging.info(f"Parameters: {n_params(params):,}  (shared-cb P_ZERO={P_ZERO} N_CODES={N_CODES} D_EMB={D_EMB})")
    ...
    save_learning_curves(history, EXP_NAME, subtitle=f"shared-cb P_ZERO={P_ZERO} N_CODES={N_CODES} D_EMB={D_EMB}")
    ...
    append_result({
        "experiment": EXP_NAME,
        "name": f"imle-shared-cb P_ZERO={P_ZERO} N_CODES={N_CODES} D_EMB={D_EMB} N_VARS={N_VARS}",
