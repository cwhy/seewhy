...
so this is idempotent and a partial regeneration only touches what moved.
    ...
    uv run --no-sync python projects/{project-name}/scripts/gen_report.py
    uv run --no-sync python projects/{project-name}/scripts/gen_report.py --into report
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Regenerate {project-name} figures from results.jsonl")
